...
async function entender({ mensaje, historial, estado, autos = null }) {
    ...
    // Determinista y quirúrgico: SOLO si trae link (firma del clic) + frase de interés.
    {
        ...
        // Boilerplate = puro relleno de interés (whitelist de palabras) + al menos una señal.
        const RELLENO = new Set(['hola', 'buenas', 'buenos', 'noches', 'tardes', 'dias', 'dia', 'buen', 'me', 'interesa', 'interesa', 'gustaria', 'recibir', 'saber', 'mas', 'informacion', 'informes', 'info', 'un', 'una', 'este', 'esta', 'el', 'la', 'auto', 'carro', 'coche', 'vehiculo', 'camioneta', 'quiero', 'necesito', 'busco', 'del', 'de', 'sobre', 'por', 'favor', 'porfavor', 'gracias', 'anuncio', 'publicacion']);
        const palabras = sl.split(' ').filter(Boolean);
        const esBoiler = palabras.length > 0 && palabras.every(w => RELLENO.has(w)) &&
            palabras.some(w => ['interesa', 'gustaria', 'informacion', 'informes', 'info'].includes(w));
        if (tieneLink && sinLinks.length <= 90 && esBoiler) {
            return {
...
module.exports = { entender, clasificar, resolverAutoDeterminista, esVendedor, candidatosDeAuto, INTENCIONES, SCHEMA };
