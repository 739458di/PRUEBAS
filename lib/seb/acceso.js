...
const SESION_INACTIVIDAD_DIAS = 30;    // sin uso → muere (orden owner 2026-09-12: "como cualquier app, la sesión queda abierta")
...
async function ensureAcceso() {
    ...
    await run(`CREATE TABLE IF NOT EXISTS contrasenas_acceso (tenant_id INTEGER PRIMARY KEY, hash TEXT, sal TEXT, creada INTEGER, cambiada INTEGER)`);
    try { await run("ALTER TABLE sesiones_vendedor ADD COLUMN via TEXT"); } catch (e) { /* ya existe */ }
    ...
    try { await run("ALTER TABLE sesiones_vendedor ADD COLUMN miembro_id INTEGER"); } catch (e) { /* ya existe */ }
    await run(`CREATE TABLE IF NOT EXISTS contrasenas_miembro (miembro_id INTEGER PRIMARY KEY, hash TEXT, sal TEXT, creada INTEGER, cambiada INTEGER)`);
    try { await run("ALTER TABLE tickets_acceso ADD COLUMN miembro_id INTEGER"); } catch (e) { /* ya existe */ }
    listo = true;
}
...
async function miembroPorTelefono(tel) {
    const t = tel521(tel); if (!t) return null;
    const r = await query(`SELECT m.id, m.nombre, m.telefono, t.id tid, t.nombre tnombre, t.telefono ttel, t.config_json FROM vendedores_universo m JOIN tenants t ON t.id = m.tenant_id
                           WHERE m.telefono = ? AND m.activo = 1 AND t.activo = 1 ORDER BY m.id DESC LIMIT 1`, [t]).catch(() => []);
    if (!r.length) return null;
    return { miembro: { id: Number(r[0].id), nombre: r[0].nombre, telefono: r[0].telefono }, tenant: { id: Number(r[0].tid), nombre: r[0].tnombre, telefono: r[0].ttel, activo: 1, config_json: r[0].config_json } };
}
/** Universo o miembro por teléfono (el universo manda si el número es de ambos). → { t, m } | null */
...
async function entrarConCodigo(telefono, codigo, ua, ip) {
    ...
    if (DEMO.esDemo(t) && String(codigo || '').replace(/\D/g, '') === DEMO.DEMO_CODIGO) return abrirSesion(t, ua, ip, 'codigo', m);
    const ahora = Date.now();
    ...
    return abrirSesion(t, ua, ip, 'codigo', m);
}
...
function inicioDe(t, miembro) {
    if (miembro) return '/fyrachat.html';
...
async function canjearTicket(token, ua, ip) {
    ...
    let mT = null;
    if (f[0].miembro_id != null) { const rm = await query("SELECT id, nombre, telefono FROM vendedores_universo WHERE id = ? AND activo = 1 AND tenant_id = ? LIMIT 1", [Number(f[0].miembro_id), Number(t.id)]).catch(() => []); if (!rm.length) return { ok: false, error: 'Ese vendedor ya no está dado de alta.' }; mT = { id: Number(rm[0].id), nombre: rm[0].nombre, telefono: rm[0].telefono }; }
    const ses = await abrirSesion(t, ua, ip, f[0].origen === 'web' ? (mT ? 'contrasena' : 'usuario') : 'ticket', mT);
...
async function sesionDe(token) {
    ...
    const r = await query(`SELECT s.id sid, s.ultimo_uso, s.creada, s.via, s.miembro_id, t.id, t.nombre, t.telefono, t.config_json,
                           ...
                           (SELECT m.nombre FROM vendedores_universo m WHERE m.id = s.miembro_id AND m.activo = 1) m_nombre, (SELECT m.telefono FROM vendedores_universo m WHERE m.id = s.miembro_id AND m.activo = 1) m_tel
    ...
    return { sid: Number(s.sid), tenant_id: s.id, tenant: { id: s.id, nombre: s.nombre, telefono: s.telefono, demo: demoS }, maestra: !mid && String(s.telefono) === OWNER_TEL,
        miembro: mid ? { id: mid, nombre: s.m_nombre, telefono: s.m_tel } : null,
...
module.exports = {
    ...
    pedirCodigo, textoCodigo, entrarConCodigo, crearTicket, crearTicketMaestra, crearTicketRemoto, canjearTicket, inicioDe,
