...
async function abrirSesion(t, ua, ip) {
    ...
    return { ok: true, token, sid: Number(ins.lastInsertRowid) || null, tenant: { id: t.id, nombre: t.nombre, telefono: t.telefono }, maestra: esOwner(t) };
}
...
async function sesionDe(token) {
    ...
    return { sid: Number(s.sid), tenant_id: s.id, tenant: { id: s.id, nombre: s.nombre, telefono: s.telefono }, maestra: String(s.telefono) === OWNER_TEL };
}
...
module.exports = {
    ...
    pedirCodigo, textoCodigo, entrarConCodigo, crearTicket, canjearTicket,
