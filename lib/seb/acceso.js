...
async function entrarConCodigo(telefono, codigo, ua, ip) {
    ...
    if (DEMO.esDemo(t) && String(codigo || '').replace(/\D/g, '') === DEMO.DEMO_CODIGO) return abrirSesion(t, ua, ip, 'codigo');
    const ahora = Date.now();
    ...
    return abrirSesion(t, ua, ip, 'codigo');
}
...
async function canjearTicket(token, ua, ip) {
    ...
    const ses = await abrirSesion(t, ua, ip, 'ticket');
    return Object.assign(ses, { destino: f[0].destino || null, origen: f[0].origen || null });
}

// ── SESIONES ───────────────────────────────────────────────────────────────────────────────────
async function abrirSesion(t, ua, ip, via) {
    const token = nuevoToken(); const ahora = Date.now();
    const ins = await run("INSERT INTO sesiones_vendedor (token_hash, tenant_id, creada, caduca, ultimo_uso, ua, ip, cerrada, via) VALUES (?,?,?,?,?,?,?,0,?)",
        [sha(token), t.id, ahora, ahora + SESION_MAX_DIAS * 86400 * 1000, ahora, String(ua || '').slice(0, 200), String(ip || '').slice(0, 64), String(via || 'codigo')]);
    ...
    const tiene = await tieneContrasena(t.id);
    return { ok: true, token, sid: Number(ins.lastInsertRowid) || null, tenant: { id: t.id, nombre: t.nombre, telefono: t.telefono, demo: DEMO.esDemo(t) }, maestra: esOwner(t), via: String(via || 'codigo'),
...
async function modoAcceso(telefono) {
    const t = await tenantPorTelefono(telefono);
    if (!t || DEMO.esDemo(t)) return { ok: true, modo: 'codigo' };
    return { ok: true, modo: (await tieneContrasena(t.id)) ? 'contrasena' : 'codigo' };
}
...
async function entrarConContrasena(telefono, contrasena, ua, ip) {
    ...
    const t = await tenantPorTelefono(tel);
    if (!t || DEMO.esDemo(t)) { hashContrasena('sin-sal', String(contrasena)); return ERR; }
    if (!(await verificarContrasena(t.id, contrasena))) return ERR;
    return abrirSesion(t, ua, ip, 'contrasena');
}
...
async function sesionDe(token) {
    ...
    await ensureAcceso();
    const ahora = Date.now();
    const r = await query(`SELECT s.id sid, s.ultimo_uso, s.creada, s.via, t.id, t.nombre, t.telefono, t.config_json, (SELECT 1 FROM contrasenas_acceso c WHERE c.tenant_id = t.id) tiene_c FROM sesiones_vendedor s JOIN tenants t ON t.id = s.tenant_id
    ...
    const demoS = DEMO.esDemo(s);
    return { sid: Number(s.sid), tenant_id: s.id, tenant: { id: s.id, nombre: s.nombre, telefono: s.telefono, demo: demoS }, maestra: String(s.telefono) === OWNER_TEL,
...
module.exports = {
    abrirSesion,
    tieneContrasena, ponerContrasena, verificarContrasena, modoAcceso, entrarConContrasena, validarContrasena, MAX_DISPOSITIVOS, CONTRA_MIN,
