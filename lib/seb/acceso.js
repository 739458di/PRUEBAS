...
async function sesionDe(token) {
    ...
    if (desv) { await run("UPDATE sesiones_vendedor SET cerrada = 1 WHERE tenant_id = ?", [s.id]).catch(() => {}); return { desvinculado: true, tenant: { id: s.id, nombre: s.nombre, telefono: s.telefono } }; }
...
module.exports = { ensureAcceso, estaDesvinculado, tel521, pedirCodigo, entrarConCodigo, crearTicket, canjearTicket, sesionDe, cerrarSesion, cerrarTodas, leerCookie, ponerCookie, borrarCookie, esOwner, OWNER_TEL, COOKIE };
