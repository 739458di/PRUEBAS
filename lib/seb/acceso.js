...
async function ponerContrasenaMiembro(mid, contrasena, opts) {
    // opts.forzar (solo servidor, alta de vendedor 2026-09-24): la contraseña inicial ES el código que le llegó por WhatsApp (dígitos); él la cambia en su perfil.
    await ensureAcceso(); const err = (opts && opts.forzar && /^\d{6,12}$/.test(String(contrasena || ''))) ? null : validarContrasena(contrasena); if (err) return { ok: false, error: err };
    const sal = crypto.randomBytes(16).toString('hex'); const ahora = Date.now();
