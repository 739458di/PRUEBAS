...
async function nombreAuto(telefono) {
    try {
        const aid = await U.autoActivoDe(0, telefono);
        if (!aid) return null;
        const a = await query("SELECT marca, modelo, anio FROM inventario_autos WHERE id=?", [aid]);
...
async function ghostScan({ duenos = new Set(), dry = false } = {}) {
    ...
    // CHATS POR UNIVERSO (orden owner 2026-09-10): el ghost scan es EXCLUSIVO del universo 0 (sale por el número de
    // Fyradrive). Candidatos, candados e inserts acotados a tenant 0; filas viejas sin etiqueta = 0 (COALESCE).
    await run("ALTER TABLE seguimientos_ghost ADD COLUMN tenant_id INTEGER DEFAULT 0").catch(() => {});
    await run("CREATE INDEX IF NOT EXISTS idx_ghost_tel ON seguimientos_ghost(telefono, id)").catch(() => {});
    ...
    const cands = await query(
        "SELECT id, channel_thread_id, telefono, nombre, ult_msg_ts FROM conversaciones WHERE ult_dir='out' AND ult_msg_ts <= ? AND ult_msg_ts >= ? AND COALESCE(tenant_id,0)=0",
    ...
    for (const c of cands) {
        ...
        const ya = await query(
            "SELECT id FROM seguimientos_ghost WHERE telefono=? AND COALESCE(tenant_id,0)=0 AND (episodio_in_ts=? OR sent_dia=?) LIMIT 1",
        ...
        const yaEsRecordatorio = await query(
            "SELECT id FROM seguimientos_ghost WHERE telefono=? AND COALESCE(tenant_id,0)=0 ORDER BY id DESC LIMIT 1", [tel]);
        ...
        if (!dry) await run(
            "INSERT INTO seguimientos_ghost (telefono, conv_id, episodio_in_ts, escenario, frase, sent_dia, created_at, tenant_id) VALUES (?,?,?,?,?,?,?,0)",
