...
const saludo = ts => { const h = L(ts).getUTCHours(); return h < 12 ? 'buen día' : (h < 19 ? 'buenas tardes' : 'buenas noches'); };
const hora = ts => { const d = L(ts); let h = d.getUTCHours(); const m = d.getUTCMinutes(); const ap = h >= 12 ? 'pm' : 'am'; h = h % 12 || 12; return 'a las ' + h + (m ? ':' + String(m).padStart(2, '0') : '') + ' ' + ap; };
const J = s => { try { return JSON.parse(s || '{}') || {}; } catch (e) { return {}; } };
async function ahoraDe(c) { return Date.now() + await CITAF.offsetDe(Number(c.chat_id)); }
...
async function cuando(c, iniTs, finTs, prec) { const now = await ahoraDe(c); return CITAF._t.cuandoCorto({ ini_ts: iniTs || c.ini_ts, fin_ts: finTs || c.fin_ts || iniTs || c.ini_ts, precision: prec || c.precision || 'hora', estado: 'viva', datos_json: '{}' }, now); }   // "mañana a las 11 am", "el sábado a las 5 pm"

// ── CANALES ──
const gram = s => String(s || '').replace(/\bde el\b/g, 'del').replace(/\ba el\b/g, 'al');
async function mandarA(t, chatId, texto, accion) { texto = gram(texto); return MSJ.enviar({ tenantId: Number(t.id), chatId: Number(chatId), origen: 'sb', clave: 'cita3:' + Number(chatId) + ':' + Date.now() + ':' + Math.random().toString(36).slice(2, 6), manual: false, accion: accion || 'cita3', texto }); }
...
async function duenoDe(t, autoInvId) {
    ...
    if (DEMO.esDemo(t)) return { tipo: 'particular', nombre: 'Juan (dueño de prueba)', telefono: '5210000000095', inv };
...
async function proponer({ tenant: t, c, ini, fin, precision, votos, por, razon, nuevo }) {
    ...
    if (!votos.v) { await guardar(c, { fase: 'espera_vendedor' }); await aMario(t, c, 'Cita 3 partes · ' + N.auto + ' · ' + cu + ' · la propuso ' + (por === 'comprador' ? 'el comprador' : 'el dueño') + '. Confirma o propón otro horario en FyraChat (botones de la cita).'); return c; }
    if (!votos.c) {
...
async function pedirDueno(t, c, cuAnt) {
    ...
    const tx = cuAnt
        ? 'Hola ' + (N.D || '') + ', ' + saludo(now) + '. Soy Seb, el asistente de ' + N.V + ' de ' + N.marca + '. La visita para ver tu ' + N.auto + ' que teníamos ' + cuAnt + ' se tendría que mover a ' + cu + '. ¿Te quedaría bien? Con un "sí" lo dejo listo; si no, con toda confianza escríbele o márcale a ' + N.V + ' aquí mismo. ¡Gracias!'
        : 'Hola ' + (N.D || '') + ', ' + saludo(now) + '. Soy Seb, el asistente de ' + N.V + ' de ' + N.marca + ', mucho gusto. Hay un comprador interesado en ver tu ' + N.auto + ' ' + cu + '. ¿Te quedaría bien a esa hora? Con un "sí" me basta para dejarlo listo. Si tienes alguna duda o prefieres otro horario, con toda confianza escríbele o márcale a ' + N.V + ' aquí mismo; este es su número. ¡Gracias!';
...
async function acreditar(t, c) {
    ...
    await ubicacion(t, c);
    if (c.dueno_chat_id) await mandarA(t, c.dueno_chat_id, '¡Confirmado' + (N.D ? ' ' + N.D : '') + '! ' + cu + ', ' + N.V + ' llega con el comprador a ver tu ' + N.auto + '. El día de la visita te aviso cuando vayan en camino. ¡Gracias!', 'cita3_dueno');
...
async function propuestaComprador({ tenant: t, chat, citaId, nuevo }) {
    ...
    if (cf.precision !== 'hora') { await bit(c, 'comprador', nuevo, 'ventana_sin_hora', 'alta', 'no arranca la ronda; se pide la hora'); await aMario(t, c, 'El comprador quiere ver el ' + (c.auto_nombre || 'auto') + ' ' + await cuando(c, cf.ini_ts, cf.fin_ts, cf.precision) + ', sin hora todavía. Ya le pedí la hora; te aviso cuando la dé.'); return c; }
    return proponer({ tenant: t, c, ini: Number(cf.ini_ts), fin: Number(cf.fin_ts), precision: 'hora', votos: { c: 1 }, por: 'comprador', nuevo });
}
...
async function entranteDueno({ tenant: t, chat, texto }) {
    ...
    if (c.fase === 'espera_dueno' && esSiLimpio(tx)) { await bit(c, 'dueno', tx, 'confirma', 'alta', 'ok_dueno'); await guardar(c, { ok_dueno: 1 }); return (c.ok_comprador && c.ok_vendedor) ? (await acreditar(t, c), { manejado: true, evento: 'acreditada' }) : { manejado: true, evento: 'confirma_dueno' }; }
    if (['confirmada', 'dia_d'].includes(c.estado)) {
        if (c.pregunta_dueno === 'dia' && !d.ok_d && esSiLimpio(tx)) { await luces(c, { ok_d: await ahoraDe(c) }); await bit(c, 'dueno', tx, 'palomita', 'alta', 'dueño confirmó el día'); await mandarA(t, chat.id, '¡Gracias' + (N.D ? ' ' + N.D : '') + '! Te aviso cuando vayan en camino.', 'cita3_dueno'); await revisarListos(t, c); return { manejado: true, evento: 'palomita' }; }
        if (c.pregunta_dueno === 'listo' && !d.listo_d && (esSiLimpio(tx) || RE_LISTO.test(n))) { await luces(c, { listo_d: await ahoraDe(c) }); await bit(c, 'dueno', tx, 'listo', 'alta', 'dueño listo'); await mandarA(t, chat.id, '¡Perfecto! En cuanto el comprador salga te aviso para que salgas tú también.', 'cita3_dueno'); await revisarLuzVerde(t, c); return { manejado: true, evento: 'listo' }; }
        ...
        if (c.pregunta_dueno === 'vispera' && esSiLimpio(tx)) { await bit(c, 'dueno', tx, 'vispera_ok', 'alta', null); await guardar(c, { pregunta_dueno: null }); return { manejado: true, evento: 'vispera_ok' }; }
    }
...
async function entranteComprador({ tenant: t, chat, texto }) {
    ...
    if (!esSiLimpio(texto)) return { manejado: false };
...
async function interceptar({ tenant: t, chat, ev, nuevo }) {
    ...
    if (c.estado !== 'pausada' && E === 'confirma' && esSiLimpio(nuevo)) {
...
async function tick({ chatId, hasta } = {}) {
    ...
    for (const c0 of rows) {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            await ejecutar(t, c, sig.k); out.push({ cita: c.cita_id, k: sig.k, due: sig.due });
        }
    }
...
module.exports = { activo, asegurar, propuestaComprador, entranteDueno, entranteComprador, interceptar, vendedor, tick, proximo, estado, porCita, porChat, porDuenoChat, abiertasDueno, borrarDeCita, duenoDe, esSiLimpio, K_TXT };
