...
async function proponer({ tenant: t, c, ini, fin, precision, votos, por, razon, nuevo }) {
    const now = await ahoraDe(c); const prev = { ini: c.ini_ts, fin: c.fin_ts, precision: c.precision, fase: c.fase, gen: c.gen, okD: !!c.ok_dueno }; const gen = (Number(c.gen) || 0) + 1;
...
async function pedirDueno(t, c, cuAnt, cuDueno) {
    ...
    const tx = cuDueno
        ? hola + 'El comprador no puede ' + cuDueno + '; ¿te quedaría bien ' + cu + ' para ver tu ' + N.auto + '? Con un "sí" lo dejo listo; si prefieres otro horario, respóndeme aquí y se lo paso a ' + N.V + '. ¡Gracias!'
        : cuAnt
...
async function propuestaComprador({ tenant: t, chat, citaId, nuevo }) {
    ...
    const marioYa = !!(await query("SELECT 1 FROM cita3_eventos WHERE cita_id = ? AND canal = 'vendedor' LIMIT 1", [Number(c.cita_id)]).catch(() => []))[0];
    return proponer({ tenant: t, c, ini: Number(cf.ini_ts), fin: Number(cf.ini_ts), precision: 'hora', votos: marioYa ? { c: 1, v: 1 } : { c: 1 }, por: 'comprador', nuevo });
}
...
async function entranteComprador({ tenant: t, chat, texto }) {
    ...
    if (!esSi(texto, c, await ahoraDe(c))) {
        ...
        // "déjame ver / luego te digo / lo checo": lo va a pensar → sin presión, la propuesta sigue esperando (owner 2026-09-26: el motor de 2 partes contestó "¿sí nos vemos el miércoles?" de una cita que nunca existió)
        if (/\b(dejame (ver|checar|revisar|pensarlo|lo pienso)|deja (lo )?(veo|checo|reviso)|lo (checo|reviso|pienso|veo)|te (aviso|confirmo|digo)( al rato| luego| en un rato| mas tarde)?|ahorita te (digo|aviso|confirmo)|luego te (digo|aviso|confirmo)|al rato te (digo|aviso)|a ver|checo|reviso|lo consulto)\b/.test(n) && !/\b(no puedo|no me queda|imposible|ya no)\b/.test(n)) {
            const N0 = await nombres(t, c); await bit(c, 'comprador', texto, 'lo_piensa', 'alta', 'propuesta sigue en pie');
            await mandarA(t, chat.id, 'Va' + N0.n + ', sin presión. Me dices si te queda ' + await cuando(c) + '.', 'cita3');
            await aMario(t, c, 'El comprador lo va a pensar ("' + String(texto).slice(0, 60) + '"). La propuesta de ' + await cuando(c) + ' sigue esperando su respuesta.');
            return { manejado: true, evento: 'lo_piensa' };
        }
