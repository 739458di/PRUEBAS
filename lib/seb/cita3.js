...
async function entranteDueno({ tenant: t, chat, texto }) {
    ...
    if (o.accion === 'acepta' && ['espera_dueno', 'pausa_dueno'].includes(c.fase)) { await guardar(c, { ok_dueno: 1 }); return (c.ok_comprador && c.ok_vendedor) ? (await acreditar(t, c), { manejado: true, evento: 'acreditada' }) : { manejado: true, evento: 'confirma_dueno' }; }
...
async function interceptar({ tenant: t, chat, ev, nuevo }) {
    ...
    if (c.estado === 'ronda' && c.fase !== 'espera_comprador') {
        const E0 = ev && ev.evento; const N0 = await nombres(t, c); const cu0 = c.ini_ts ? await cuando(c) : null;
