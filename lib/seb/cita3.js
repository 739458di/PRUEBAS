...
// ══ COORDINADOR (Opus): helpers ══
const COORD = require('./coordinador.js');
async function histChat(chatId, quienIn) { const r = await query("SELECT direccion, emisor, texto FROM mensajes WHERE conversacion_id = ? AND COALESCE(emisor,'') <> 'sistema' ORDER BY id DESC LIMIT 7", [Number(chatId)]).catch(() => []); return r.reverse().slice(0, -1).map(m => ({ quien: m.direccion === 'in' ? quienIn : 'Seb', texto: m.texto })); }
async function pendienteTxt(c) { return c.ini_ts ? await cuando(c) + ({ espera_dueno: ' (se le preguntó al dueño)', pausa_dueno: ' (se le preguntó al dueño)', espera_comprador: ' (se le preguntó al comprador)' }[c.fase] || (c.estado === 'confirmada' ? ' (YA CONFIRMADA)' : '')) : 'ninguna'; }
async function leerCoord(t, c, quien, chatId, texto) { const now = await ahoraDe(c); return COORD.interpretar({ quien, texto, foto: await fotoTexto(t, { id: c.chat_id }), pendiente: await pendienteTxt(c), historial: await histChat(chatId, quien === 'dueno' ? 'Dueño' : 'Comprador'), hoy: new Date(now - 6 * 3600000).toISOString().slice(0, 16).replace('T', ' ') + ' (' + ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'][new Date(now - 6 * 3600000).getUTCDay()] + ')' }); }
function horaDe(t, dia, hora, now) { if (!/^\d{4}-\d{2}-\d{2}$/.test(String(dia)) || !/^\d{1,2}:\d{2}$/.test(String(hora))) return null; const [h, m] = hora.split(':').map(Number); const ini = at(dia, h, m); return ini > now ? ini : null; }
// ══ FOTO DEL TRATO (etapa 3): lo que la lectura necesita ver en 1-2 líneas (no toda la historia): en qué va, qué se propuso, quién ya votó y qué se espera
const FASE_TXT = { espera_vendedor: 'se espera que Mario confirme', espera_comprador: 'se espera la respuesta del COMPRADOR a la propuesta', espera_dueno: 'se espera el sí del dueño', pausa_dueno: 'el dueño necesita a Mario' };
async function fotoTexto(t, chat) {
    const c = await porChat(t.id, chat.id).catch(() => null); if (!c) return null;
    const est = { ronda: 'en ronda (NO confirmada)', confirmada: 'CONFIRMADA', dia_d: 'es el día de la cita', pausada: 'pausada' }[c.estado] || c.estado;
    const vot = ['comprador', 'vendedor', 'dueno'].filter(k => c['ok_' + k]).map(k => k === 'vendedor' ? 'Mario' : (k === 'dueno' ? 'dueño' : k));
    return 'cita ' + est + (c.ini_ts ? ' · propuesta vigente: ' + await cuando(c) : ' · sin día ni hora') + (c.estado === 'ronda' ? ' · votos: ' + (vot.join(', ') || 'ninguno') : '') + (FASE_TXT[c.fase] ? ' · ' + FASE_TXT[c.fase] : '');
}
// ══ ESPEJO (etapa 3, owner 2026-09-26: "¿sí nos vemos el miércoles?" de una cita que nunca existió): la cita de 3 partes es la ÚNICA verdad del trato;
//    la visita del motor de 2 partes es solo su reflejo — cada propuesta la actualiza al instante (misma fila, sin notas ni recordatorios).
async function espejo(c) { try { await run("UPDATE citaf SET ini_ts = ?, fin_ts = ?, precision = ?, estado = CASE WHEN estado IN ('viva','en_camino','pospuesta') THEN 'viva' ELSE estado END, version = COALESCE(version, 1) + 1, version_ts = ? WHERE id = ?", [Number(c.ini_ts), Number(c.fin_ts || c.ini_ts), c.precision || 'hora', Date.now(), Number(c.cita_id)]); } catch (e) { } }
async function proponer({ tenant: t, c, ini, fin, precision, votos, por, razon, nuevo }) {
    const now = await ahoraDe(c); const prev = { ini: c.ini_ts, fin: c.fin_ts, precision: c.precision, fase: c.fase, gen: c.gen, okD: !!c.ok_dueno, okC: !!c.ok_comprador }; const gen = (Number(c.gen) || 0) + 1;
    await guardar(c, { gen, por, ini_ts: ini, fin_ts: fin || ini, precision: precision || 'hora', ok_comprador: votos.c ? 1 : 0, ok_vendedor: votos.v ? 1 : 0, ok_dueno: votos.d ? 1 : 0, estado: c.estado === 'confirmada' || c.estado === 'pausada' ? c.estado : 'ronda', razon: razon || null, espera_desde: now, fase: null, pregunta_comprador: null, pregunta_dueno: c.pregunta_dueno === 'confirmar' ? null : c.pregunta_dueno });
    ...
    if (!votos.c) {
        ...
        let tx;
        // CONTRAPROPUESTA CORDIAL (owner 2026-09-26): si el comprador había pedido una hora, se reconoce lo que pidió antes de sugerir otra (no suena a "propuesta inicial")
        const cuPed = prev.okC && prev.ini && Number(prev.ini) !== Number(c.ini_ts) && (!c.conf_ini || Number(prev.ini) !== Number(c.conf_ini)) ? await cuando(c, prev.ini, prev.fin, prev.precision) : null;
        const oye = N.C ? 'Oye ' + N.C + ', ' : 'Oye, '; const cierre = ' ¿Te quedaría bien? Si no, dime qué otro día y hora te acomoda y lo cuadramos.';
        const quien = por === 'dueno' ? 'quien tiene el ' + N.auto : N.V;
        if (cuPed) tx = oye + 'fíjate que ' + quien + ' no va a poder ' + cuPed + ', pero sí puede ' + (ymd(Number(prev.ini)) === ymd(Number(c.ini_ts)) ? 'ese mismo día ' + hora(c.ini_ts) : cu) + '.' + cierre;
        else if (cuAnt) tx = oye + 'fíjate que ' + (por === 'dueno' ? quien : N.V + ' tuvo un cambio y') + ' no va a poder ' + cuAnt + ', pero sí puede ' + cu + '.' + cierre;
        else tx = oye + quien + ' puede ' + cu + '.' + cierre;
    ...
    if (!votos.d) return pedirDueno(t, c, cuAnt, prev.okD && prev.ini && Number(prev.ini) !== Number(c.ini_ts) && (!c.conf_ini || Number(prev.ini) !== Number(c.conf_ini)) ? await cuando(c, prev.ini, prev.fin, prev.precision) : null);   // el dueño había dicho que sí a otra hora (no la ya confirmada) → contrapropuesta   // contrapropuesta a lo que el dueño ya había aceptado/propuesto
    return acreditar(t, c);
}
async function pedirDueno(t, c, cuAnt, cuDueno) {
    ...
    const cola = ' ¿Te quedaría bien? Con un "sí" lo dejo listo. Si prefieres otro horario, dime cuál y se lo propongo al comprador; para cualquier duda puedes hablar con ' + N.V + '. ¡Gracias!';
    const tx = cuDueno
        ? hola + 'Fíjate que al comprador no le queda ' + cuDueno + '; ¿te quedaría bien ' + cu + ' para ver tu ' + N.auto + '? Con un "sí" lo dejo listo; si prefieres otro horario, respóndeme aquí y lo cuadramos con ' + N.V + '. ¡Gracias!'
        : cuAnt
    ...
    await aMario(t, c, 'Seb está coordinando con el dueño del ' + N.auto + ' para ' + cu + '. No tienes que hacer nada; si tú no puedes a esa hora, pícale "Nuevo horario".', c.chat_id);
    return c;
}
...
async function propuestaComprador({ tenant: t, chat, citaId, nuevo }) {
    ...
    // después de confirmada no se mueve sola (lo decide el interceptor)… SALVO que se esté renegociando y la propuesta esté esperando al comprador: su contrapropuesta REBOTA a la otra parte (owner 2026-09-26: "miércoles misma hora" se quedaba en el aire)
    if (['confirmada', 'pausada', 'dia_d'].includes(c.estado) && c.fase !== 'espera_comprador') return c;
    ...
    // MARIO VOTA SOLO LA PRIMERA VEZ (owner 2026-09-26): su voto existe porque puede no estar al pendiente del teléfono; si ya picó algo en este trato, está enterado → cuenta solo y va directo al dueño
    // COORDINADOR AUTOMÁTICO (owner 2026-09-26): Mario ya no es compuerta — su "sí" va implícito y la propuesta rebota directo al dueño; él entra solo si no puede (Nuevo horario) o si lo llaman
    return proponer({ tenant: t, c, ini: Number(cf.ini_ts), fin: Number(cf.ini_ts), precision: 'hora', votos: { c: 1, v: 1 }, por: 'comprador', nuevo });
}
...
// SÍ DÉBIL (owner 2026-09-26: "no puedes asumir por UN SOLO MENSAJE"): "ok / va / sale / listo / 👍" a secas puede ser "enterado, déjame ver" → no confirma: se pregunta "¿verdad?"
const esDebil = tx => /^\W*(ok+(ey|ay|i)?|okis|va+|vale|sale|listo|lista|dale|bien|orale|simon|ok gracias|va gracias|mm+ ok|ok mm+)\W*$/.test(norm(tx).replace(/[^\p{L}\p{N}\s]/gu, ' ').replace(/\s+/g, ' ').trim()) || /^\s*(👍|👌|🙏|👍🏻|👍🏼)+\s*$/u.test(String(tx || ''));
...
async function entranteDueno({ tenant: t, chat, texto }) {
    const abiertas = await abiertasDueno(t.id, chat.id); if (!abiertas.length) return { manejado: false };
    // DUEÑO CON VARIAS CITAS (owner 2026-09-26: "no se pueden empalmar nunca"): su mensaje va a la cita que le hizo la pregunta pendiente; si hay varias, a la que
    // nombre por día u hora; si no nombra ninguna, a la pregunta más reciente (así se lee WhatsApp). Solo si no hay forma de saberlo, pausa y Mario elige.
    let c = abiertas[0], ambigua = false;
    if (abiertas.length > 1) {
        const esperan = abiertas.filter(x => ['espera_dueno', 'pausa_dueno'].includes(String(x.fase || '')) || x.pregunta_dueno);
        const nD = norm(texto); const DNx = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado'];
        const nombra = (x) => { const d = new Date(Number(x.ini_ts) - 6 * 3600000); const w = (nD.match(/\b(lunes|martes|miercoles|jueves|viernes|sabado|domingo)\b/) || [])[1]; const h = (nD.match(/\b(?:a las|alas|a la)\s*(\d{1,2})/) || [])[1]; if (!w && !h) return false; return (!w || w === DNx[d.getUTCDay()]) && (!h || Number(h) % 12 === d.getUTCHours() % 12); };
        const pool = esperan.length ? esperan : abiertas; const porNombre = pool.filter(nombra);
        if (porNombre.length === 1) c = porNombre[0];
        else if (esperan.length >= 1) c = esperan.slice().sort((a, b) => Number(b.espera_desde || b.updated || 0) - Number(a.espera_desde || a.updated || 0))[0];
        else ambigua = true;
    }
    ...
    if (ambigua) return pausa('varias citas abiertas y el mensaje no dice a cuál va');
    if (RE_HUMANO.test(n)) return pausa('pide hablar con el vendedor');
    if ((c.fase === 'espera_dueno' || c.fase === 'pausa_dueno') && esDebil(tx) && c.pregunta_dueno !== 'confirmar') { await guardar(c, { pregunta_dueno: 'confirmar' }); await bit(c, 'dueno', tx, 'si_debil', 'baja', 'se le pregunta ¿verdad?'); await mandarA(t, chat.id, 'Entonces te lo confirmo' + (N.D ? ' ' + N.D : '') + ': ' + await cuando(c) + ' para ver tu ' + N.auto + ', ¿verdad?', 'cita3_dueno'); return { manejado: true, evento: 'pide_confirmar' }; }
    if ((c.fase === 'espera_dueno' || c.fase === 'pausa_dueno') && esSi(tx, c, await ahoraDe(c))) { if (c.pregunta_dueno === 'confirmar') await guardar(c, { pregunta_dueno: null }); /* pausa_dueno: si Mario ya le devolvió el chat a Seb, el "sí" limpio del dueño vale */ await bit(c, 'dueno', tx, 'confirma', 'alta', 'ok_dueno'); await guardar(c, { ok_dueno: 1 }); return (c.ok_comprador && c.ok_vendedor) ? (await acreditar(t, c), { manejado: true, evento: 'acreditada' }) : { manejado: true, evento: 'confirma_dueno' }; }
    ...
    // ══ COORDINADOR AUTOMÁTICO del lado del DUEÑO: lo que es acordar horario se resuelve solo; lo demás escala a Mario ══
    const o = await leerCoord(t, c, 'dueno', chat.id, tx); const now0 = await ahoraDe(c);
    if (o) await bit(c, 'dueno', tx, 'coord:' + o.accion, 'alta', String(o.razon || '').slice(0, 140));
    if (!o) return pausa('no se pudo leer el mensaje');
    if (o.accion === 'cortesia') return { manejado: true, evento: 'cortesia' };
    if (o.accion === 'acepta' && ['espera_dueno', 'pausa_dueno'].includes(c.fase)) { await guardar(c, { ok_dueno: 1, pregunta_dueno: null }); return (c.ok_comprador && c.ok_vendedor) ? (await acreditar(t, c), { manejado: true, evento: 'acreditada' }) : { manejado: true, evento: 'confirma_dueno' }; }
    if (o.accion === 'acepta') return { manejado: true, evento: 'acepta_sin_pendiente' };
    if (o.accion === 'propone') {
        const ini = horaDe(t, o.dia, o.hora, now0); if (!ini) { await mandarA(t, chat.id, 'Esa hora ya pasó' + (N.D ? ' ' + N.D : '') + '. ¿Qué otro día y a qué hora te acomoda? Se lo propongo al comprador.', 'cita3_dueno'); return { manejado: true, evento: 'propone_invalida' }; }
        try { const chD = await U.chatPorId(chat.id); if (chD && VOZ.vozDe(chD) === 'humano') await VOZ.devolver({ tenant: t, chat: chD }); } catch (e) { }
        await proponer({ tenant: t, c, ini, fin: ini, precision: 'hora', votos: { v: 1, d: 1 }, por: 'dueno', razon: 'dueño propone', nuevo: tx });
        await mandarA(t, chat.id, '¡Va' + (N.D ? ' ' + N.D : '') + '! Se lo propongo al comprador y te aviso.', 'cita3_dueno');
        await aMario(t, c, 'El dueño del ' + N.auto + ' propuso ' + await cuando(c) + '; ya se lo reboté al comprador. No tienes que hacer nada.', c.chat_id); return { manejado: true, evento: 'dueno_propone' };
    }
    if (o.accion === 'lo_piensa') { await mandarA(t, chat.id, 'Va' + (N.D ? ' ' + N.D : '') + ', sin presión. Me dices si te queda ' + await cuando(c) + '.', 'cita3_dueno'); return { manejado: true, evento: 'dueno_lo_piensa' }; }
    if (o.accion === 'no_puede_sin_hora') {
        if (c.fase === 'pausa_dueno') await guardar(c, { fase: 'espera_dueno' });
        await mandarA(t, chat.id, 'Sin problema' + (N.D ? ' ' + N.D : '') + '. ¿Qué otro día y a qué hora te acomoda? Se lo propongo al comprador.', 'cita3_dueno');
        await aMario(t, c, 'El dueño no puede ' + (c.ini_ts ? await cuando(c) : 'esa hora') + ' (' + N.auto + '); ya le pedí otra hora. No tienes que hacer nada.', c.chat_id); return { manejado: true, evento: 'dueno_otra_hora' };
    }
    if (o.accion === 'ya_no_disponible') return pausa('dice que el auto ya no está disponible — si es así, pícale "Ya no disponible" en su chat');
    return pausa(o.accion === 'hablar_mario' ? 'pide hablar contigo' : 'no es de horario: ' + String(o.razon || '').slice(0, 80));
}
...
async function entranteComprador({ tenant: t, chat, texto }) {
    ...
    if (esDebil(texto) && c.pregunta_comprador !== 'confirmar') { const N0 = await nombres(t, c); await guardar(c, { pregunta_comprador: 'confirmar' }); await bit(c, 'comprador', texto, 'si_debil', 'baja', 'se le pregunta ¿verdad?'); await mandarA(t, chat.id, 'Entonces te lo confirmo' + N0.n + ': ' + await cuando(c) + ' para ver el ' + N0.auto + ', ¿verdad?', 'cita3'); return { manejado: true, evento: 'pide_confirmar' }; }
    if (!esSi(texto, c, await ahoraDe(c))) {
        // ══ COORDINADOR AUTOMÁTICO del lado del COMPRADOR (propuesta esperando su respuesta): acordar horario se resuelve solo; dudas del auto las contesta el cerebro; lo demás escala ══
        const N = await nombres(t, c); const now0 = await ahoraDe(c);
        const o = await leerCoord(t, c, 'comprador', chat.id, texto);
        if (o) await bit(c, 'comprador', texto, 'coord:' + o.accion, 'alta', String(o.razon || '').slice(0, 140));
        const escalar = async (motivo) => { await mandarA(t, chat.id, 'Va' + N.n + ', ahorita te escribe ' + N.V + '.', 'cita3'); try { await VOZ.entregar({ tenant: t, chat, motivo, avisar: false }); } catch (e) { } await aMario(t, c, '👤 Le propuse al comprador ' + await cuando(c) + ' y contestó: "' + String(texto).slice(0, 140) + '". Contéstale tú; si quedan en otro horario pícale "Nuevo horario", o si dice que sí, "Aceptó el comprador".'); return { manejado: true, evento: 'escala_propuesta' }; };
        if (!o) return escalar('no se pudo leer el mensaje');
        if (o.accion === 'pregunta_otra') return { manejado: false };   // duda del auto: la contesta el cerebro; la propuesta sigue esperando
        if (o.accion === 'cortesia') return { manejado: true, evento: 'cortesia' };
        if (o.accion === 'lo_piensa') { await mandarA(t, chat.id, 'Va' + N.n + ', sin presión. Me dices si te queda ' + await cuando(c) + '.', 'cita3'); await aMario(t, c, 'El comprador lo va a pensar. La propuesta de ' + await cuando(c) + ' sigue esperando su respuesta.'); return { manejado: true, evento: 'lo_piensa' }; }
        if (o.accion === 'no_puede_sin_hora') { await mandarA(t, chat.id, 'Sin tema' + N.n + '. ¿Qué otro día y a qué hora te acomoda? Lo cuadro con quien tiene el auto.', 'cita3'); return { manejado: true, evento: 'comprador_otra_hora' }; }
        if (o.accion === 'propone') {
            const ini = horaDe(t, o.dia, o.hora, now0); if (!ini) { await mandarA(t, chat.id, 'Esa hora ya pasó' + N.n + '. ¿Qué otro día y a qué hora te acomoda?', 'cita3'); return { manejado: true, evento: 'propone_invalida' }; }
            await proponer({ tenant: t, c, ini, fin: ini, precision: 'hora', votos: { c: 1, v: 1 }, por: 'comprador', nuevo: texto });
            await mandarA(t, chat.id, 'Va' + N.n + ', déjame confirmarlo con quien tiene el auto: ' + await cuando(c) + '. Te aviso por aquí.', 'cita3'); return { manejado: true, evento: 'comprador_propone' };
        }
        if (o.accion === 'cancela') {
            await guardar(c, { estado: 'cancelada', fase: null, razon: 'no puede el comprador' });
            try { await CITAF.vendedor({ tenant: t, chat: await U.chatPorId(c.chat_id), evento: 'descartar', razon: 'el comprador ya no va', io: ioMudo() }); } catch (e) { }
            if (c.dueno_chat_id && (c.ok_dueno || c.conf_ini)) await mandarA(t, c.dueno_chat_id, 'Hola' + (N.D ? ' ' + N.D : '') + ', el comprador ya no va a poder; queda sin efecto la visita que te comenté. ¡Gracias!', 'cita3_dueno');
            await mandarA(t, chat.id, 'Entendido' + N.n + ', gracias por avisar. Cualquier cosa aquí andamos.', 'cita3'); await aMario(t, c, '❌ El comprador canceló (' + N.auto + '): "' + String(texto).slice(0, 80) + '".'); return { manejado: true, evento: 'cancela' };
        }
        if (o.accion !== 'acepta') return escalar(o.accion === 'hablar_mario' ? 'pide hablar con Mario' : 'comprador ante la propuesta: "' + String(texto).slice(0, 60) + '"');
    }
    await bit(c, 'comprador', texto, 'acepta', 'alta', 'ok_comprador'); await guardar(c, { ok_comprador: 1, pregunta_comprador: null });
...
async function interceptar({ tenant: t, chat, ev, nuevo }) {
    const c = await porChat(t.id, chat.id); if (!c) return null;
    // ══ TABLA DE LA RONDA (etapa 3): antes de confirmar, la cita de 3 partes también decide — el motor de 2 partes no puede dar por hecha ni por muerta una visita que no es suya
    if (c.estado === 'ronda' && c.fase !== 'espera_comprador') {
        const E0 = ev && ev.evento; let E0x = null; const N0 = await nombres(t, c); const cu0 = c.ini_ts ? await cuando(c) : null;
        const avisarDueno = async (tx) => { if (c.dueno_chat_id && (['espera_dueno', 'pausa_dueno'].includes(c.fase) || c.ok_dueno)) await mandarA(t, c.dueno_chat_id, tx, 'cita3_dueno'); };
        // COORDINADOR también mientras se espera al dueño/Mario (owner 2026-09-26: "yo te aviso" → el motor viejo dijo "nos vemos mañana, me avisas cuando vengas" y le borró la hora):
        // si ya hay una propuesta con hora, todo lo del comprador que no sea un cuándo nuevo claro lo lee el coordinador; el motor de 2 partes no toca la visita
        if (c.ini_ts && c.precision === 'hora' && !['agenda_o_cambio', 'ya_voy', 'ya_llegue', 'cancela', 'se_complico', 'cambia_auto', 'no_es_de_cita'].includes(E0)) {
            const o = await leerCoord(t, c, 'comprador', chat.id, nuevo); if (o) await bit(c, 'comprador', nuevo, 'coord:' + o.accion, 'alta', String(o.razon || '').slice(0, 140));
            if (o && o.accion === 'pregunta_otra') return null;
            if (o && o.accion === 'cortesia') return { manejado: true, silencio: true };
            if (o && o.accion === 'propone') { const ini = horaDe(t, o.dia, o.hora, await ahoraDe(c)); if (ini) { await proponer({ tenant: t, c, ini, fin: ini, precision: 'hora', votos: { c: 1, v: 1 }, por: 'comprador', nuevo }); await mandarA(t, chat.id, 'Va' + N0.n + ', déjame confirmarlo con quien tiene el auto: ' + await cuando(c) + '. Te aviso por aquí.', 'cita3'); return { manejado: true, evento: 'comprador_propone' }; } }
            if (o && (o.accion === 'lo_piensa' || o.accion === 'acepta')) { await mandarA(t, chat.id, 'Va' + N0.n + ', sin presión. Mientras, lo sigo confirmando con quien tiene el auto: ' + cu0 + '. Te aviso por aquí.', 'cita3'); return { manejado: true, evento: 'espera_confirmacion' }; }
            if (o && o.accion === 'no_puede_sin_hora') E0x = 'se_complico';
            else if (o && o.accion === 'cancela') E0x = 'cancela';
            else if (o && o.accion === 'hablar_mario') { try { await VOZ.entregar({ tenant: t, chat, motivo: 'pide hablar con Mario', avisar: false }); } catch (e) { } await mandarA(t, chat.id, 'Va' + N0.n + ', ahorita te escribe ' + N0.V + '.', 'cita3'); await aMario(t, c, '👤 El comprador pide hablar contigo (' + N0.auto + ', ' + (cu0 || 'sin hora') + ').'); return { manejado: true, evento: 'hablar_mario' }; }
            else if (!o) return { manejado: true, silencio: true };
        }
        if (E0 === 'ya_voy' || E0 === 'ya_llegue') { await bit(c, 'comprador', nuevo, E0, 'alta', 'antes de confirmar'); await mandarA(t, chat.id, 'Todavía no queda confirmada tu visita' + N0.n + '; en cuanto la confirme te aviso por aquí para que salgas.', 'cita3'); await aMario(t, c, '⚠️ El comprador dice "' + String(nuevo).slice(0, 60) + '" y la cita todavía no está confirmada (' + (cu0 || 'sin hora') + '). Márcale si hace falta.'); return { manejado: true, evento: 'antes_de_confirmar' }; }
        if (E0 === 'cancela' || E0x === 'cancela') {
            await bit(c, 'comprador', nuevo, 'cancela', 'alta', 'ronda cancelada por el comprador');
            await guardar(c, { estado: 'cancelada', fase: null, razon: 'no puede el comprador' });
            try { await CITAF.vendedor({ tenant: t, chat: await U.chatPorId(c.chat_id), evento: 'descartar', razon: 'el comprador ya no va', io: ioMudo() }); } catch (e) { }
            await avisarDueno('Hola' + (N0.D ? ' ' + N0.D : '') + ', el comprador ya no va a poder; queda sin efecto la visita que te comenté. ¡Gracias!');
            await mandarA(t, chat.id, 'Entendido' + N0.n + ', gracias por avisar. Cualquier cosa aquí andamos.', 'cita3');
            await aMario(t, c, '❌ El comprador canceló antes de confirmar (' + (cu0 || 'sin hora') + ', ' + N0.auto + '): "' + String(nuevo).slice(0, 80) + '".'); return { manejado: true, evento: 'cancela_ronda' };
        }
        if (E0 === 'se_complico' || E0x === 'se_complico') {
            await bit(c, 'comprador', nuevo, 'se_complico', 'alta', 'ronda sin fecha');
            await guardar(c, { fase: null, ok_comprador: 0, ok_vendedor: 0, ok_dueno: 0 });
            try { await run("UPDATE citaf SET estado = 'pospuesta', version = COALESCE(version, 1) + 1, version_ts = ? WHERE id = ? AND estado IN ('viva','en_camino')", [Date.now(), Number(c.cita_id)]); } catch (e) { }
            await avisarDueno('Hola' + (N0.D ? ' ' + N0.D : '') + ', hubo un cambio en el horario que te comenté; en cuanto quede te vuelvo a escribir. ¡Gracias!');
            await mandarA(t, chat.id, 'Va' + N0.n + ', sin tema. Cuando sepas qué día y hora te acomoda me dices y lo confirmo.', 'cita3');
            await aMario(t, c, '👉 Cuadra el horario con el comprador: se le complicó ' + (cu0 || 'la visita') + ' (' + N0.auto + ') y se quedó sin hora. Cuando quede, pícale "Nuevo horario" (o si él la dice, yo la rebotaré al dueño).'); return { manejado: true, evento: 'se_complico_ronda' };
        }
        if (E0 === 'cambia_auto') { await bit(c, 'comprador', nuevo, 'cambia_auto', 'alta', 'escala'); try { await VOZ.entregar({ tenant: t, chat, motivo: 'cambia de auto a media ronda: "' + String(nuevo).slice(0, 60) + '"', avisar: false }); } catch (e) { } await aMario(t, c, '👤 El comprador quiere cambiar de auto a media ronda: "' + String(nuevo).slice(0, 100) + '". Contéstale tú.'); return { manejado: true, evento: 'cambia_auto_ronda' }; }
        return null;   // un cuándo nuevo lo aplica el motor (→ nueva propuesta del comprador); lo demás sale por la puerta que no deja decir "nos vemos" antes de los 3 votos
    }
    if (!['confirmada', 'pausada', 'dia_d'].includes(c.estado)) return null;
    ...
    // ══ COORDINADOR AUTOMÁTICO con la cita CONFIRMADA (owner 2026-09-26: "automático hasta el día D"): mover, no poder, cancelar o dudar se resuelve solo; la cita sigue en pie mientras se renegocia ══
    if (c.estado !== 'pausada') {
        const o = await leerCoord(t, c, 'comprador', chat.id, nuevo);
        if (o) await bit(c, 'comprador', nuevo, 'coord:' + o.accion, 'alta', String(o.razon || '').slice(0, 140));
        const cuV = await cuando(c, c.conf_ini, c.conf_fin, c.conf_precision);
        if (o && o.accion === 'pregunta_otra') return null;
        if (o && o.accion === 'cortesia') return { manejado: true, silencio: true };
        if (o && o.accion === 'acepta') { await mandarA(t, chat.id, '¡Perfecto' + N.n + ', gracias!', 'cita3'); return { manejado: true, evento: 'confirma' }; }
        if (o && o.accion === 'lo_piensa') { await mandarA(t, chat.id, 'Va' + N.n + ', sin presión. Tu cita sigue en pie ' + cuV + '; cualquier cambio me dices.', 'cita3'); return { manejado: true, evento: 'lo_piensa' }; }
        if (o && o.accion === 'propone') {
            const ini = horaDe(t, o.dia, o.hora, await ahoraDe(c));
            if (ini) { await proponer({ tenant: t, c, ini, fin: ini, precision: 'hora', votos: { c: 1, v: 1 }, por: 'comprador', razon: 'comprador', nuevo });
                await mandarA(t, chat.id, 'Va' + N.n + ', déjame ver con quien tiene el auto si se puede mover a ' + await cuando(c) + '. Mientras, tu cita sigue en pie ' + cuV + '.', 'cita3');
                await aMario(t, c, 'El comprador pidió mover la cita (' + cuV + ') a ' + await cuando(c) + '; ya se lo estoy preguntando al dueño. No tienes que hacer nada.'); return { manejado: true, evento: 'mover' }; }
        }
        if (o && o.accion === 'no_puede_sin_hora') { await bit(c, 'comprador', nuevo, 'no_puede', 'alta', 'se le pide otra hora'); await mandarA(t, chat.id, 'Sin tema' + N.n + '. ¿Qué otro día y a qué hora te acomoda? Lo muevo con quien tiene el auto.', 'cita3'); await aMario(t, c, 'El comprador ya no puede ' + cuV + ' (' + N.auto + '); le pedí otra hora. Te aviso cuando la dé.'); return { manejado: true, evento: 'no_puede' }; }
        if (o && o.accion === 'cancela') {
            await guardar(c, { estado: 'cancelada', fase: null, razon: 'no puede el comprador' });
            try { await CITAF.vendedor({ tenant: t, chat: await U.chatPorId(c.chat_id), evento: 'descartar', razon: 'el comprador ya no va', io: ioMudo() }); } catch (e) { }
            if (c.dueno_chat_id) await mandarA(t, c.dueno_chat_id, 'Hola' + (N.D ? ' ' + N.D : '') + ', el comprador ya no va a poder ' + cuV + '; queda sin efecto la visita. ¡Gracias!', 'cita3_dueno');
            await mandarA(t, chat.id, 'Entendido' + N.n + ', gracias por avisar. Cualquier cosa aquí andamos.', 'cita3'); await aMario(t, c, '❌ El comprador canceló la cita de ' + cuV + ' (' + N.auto + '). Ya le avisé al dueño.'); return { manejado: true, evento: 'cancela' };
        }
    }
...
async function vendedor({ tenant: t, chat, evento, datos }) {
    ...
    if (evento === 'nuevo_horario' || evento === 'proponer') {
        ...
        let votos = desdeDueno ? { v: 1, d: 1 } : ((!post || razon === 'vendedor') ? { v: 1 } : { v: 1, c: 1 });
        if (!desdeDueno && !post && c.ok_comprador && Number(c.ini_ts) === Number(v.ini)) votos = { c: 1, v: 1 };   // propuso la MISMA hora que el comprador ya pidió = Mario confirma: va al dueño, no de regreso al comprador
...
async function tickHora({ chatId, hasta, out }) {
    const rows = await query("SELECT f.* FROM citaf f LEFT JOIN cita3_hora h ON h.cita_id = f.id WHERE h.cita_id IS NULL AND f.estado IN ('viva') AND f.precision IN ('dia','dias') AND NOT EXISTS (SELECT 1 FROM cita3 x WHERE x.cita_id = f.id AND x.precision = 'hora' AND x.ini_ts IS NOT NULL AND x.estado IN ('ronda','confirmada','dia_d','pausada'))" + (chatId ? ' AND f.chat_id = ?' : ''), chatId ? [Number(chatId)] : []).catch(() => []);
    for (const f of rows) {
...
async function proximo(chatId) {
    ...
    const f = (await query("SELECT f.* FROM citaf f LEFT JOIN cita3_hora h ON h.cita_id = f.id WHERE h.cita_id IS NULL AND f.estado = 'viva' AND f.precision IN ('dia','dias') AND NOT EXISTS (SELECT 1 FROM cita3 x WHERE x.cita_id = f.id AND x.precision = 'hora' AND x.ini_ts IS NOT NULL AND x.estado IN ('ronda','confirmada','dia_d','pausada')) AND f.chat_id = ?", [Number(chatId)]).catch(() => []))[0];
    if (f) P.push({ k: 'pide_hora', due: Math.max(at(masDias(ymd(Number(f.ini_ts)), -1), 9, 0), now + 60000) });   // si las 9 am del día anterior ya pasaron, se pide ya
    return P.sort((a, b) => a.due - b.due);
}
...
async function estado(citaId) {
    ...
    const P = (['ronda', 'confirmada', 'dia_d'].includes(c.estado) ? await proximo(c.chat_id) : []).map(p => ({ k: p.k, que: K_TXT[p.k.split(':')[0]] || p.k, due: p.due, cuando: CITAF.fechaCorta(p.due) }));   // misma lista que usa el reloj: incluye "pedirle la hora" de una ventana (owner: poder simularlo)
...
module.exports = { activo, asegurar, propuestaComprador, entranteDueno, entranteComprador, interceptar, vendedor, tick, proximo, estado, porCita, porChat, porDuenoChat, abiertasDueno, borrarDeCita, duenoDe, esSiLimpio, esSi, fotoTexto, K_TXT };
