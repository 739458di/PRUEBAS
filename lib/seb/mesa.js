...
const RE_DETALLES = /(detalles?|informaci[oó]n|info\b|ficha|caracter[ií]sticas|cu[eé]ntame m[aá]s|m[aá]s datos|todo del)/i;
...
function detectarAccion(t, clasif) {
    ...
    if (/cotiza|cr[eé]dito|financia|mensualidad|enganche|a meses/i.test(t) || int === 'cotizar_credito') return 'cotizar';
...
async function ejecutarAccionMesa({ rows, accion, clasif, texto, tel, ej, mesa }) {
    ...
    if (accion === 'disponibilidad') {
        return { tipo: 'mesa_accion_disponibilidad', segmentos: [uno ? 'Sí, sigue disponible 👍' : `Sí, los ${rows.length === 2 ? 'dos' : rows.length} siguen disponibles 👍`, '¿Te late venir a verlo y manejarlo? Te agendo de una vez'] };
    }
...
async function responderMesa({ tel, texto, clasif, convId }) {
    try {
        ...
        let nuevosMulti = [], preguntaAmb = null;
        {
            ...
            if (frasesM.length > 1) {
                ...
                for (const f of frasesM) {
                    ...
                    if (r && r.auto && r.via === 'nombre' && !nuevosMulti.includes(r.auto.id) && !ap.esNegado(f, r.auto.nombre)) nuevosMulti.push(r.auto.id);
        ...
        if (!mesa.length) {
            ...
            if (RE_UBICACION.test(t) || (clasif && clasif.intencion_principal === 'cita_ubicacion'))
        ...
        if (pend === 'cotizar' && rows.length >= 1) {
            ...
            if (eng) {
                ...
                delete ej.mesa_pregunta; delete ej.mesa_pregunta_auto; delete ej.mesa_pregunta_autos;
        ...
        if (objetivo) {
            ...
            if (accObj && accObj !== 'detalles') {
                ...
                const idsAcc = clarosInv.concat([objetivo]).filter((v, i, a) => a.indexOf(v) === i).slice(-3);
...
async function entradaMultiple({ tel, texto, nombre }) {
    try {
        ...
        for (const f of frasesM) {
            const r = ap.resolverEleccion(f, items);
            if (r && r.auto && r.via === 'nombre' && !ids.includes(r.auto.id)) ids.push(r.auto.id);
        }
...
module.exports = { responderMesa, cargarMesa, guardarMesa, entradaMultiple, alinearAuto, nombradoEnTexto, infoEnPosesion, herramientaEnPosesion, paqueteAuto, detectarAccion, ejecutarAccionMesa, preguntaFamilia };
