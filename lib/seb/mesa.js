...
async function responderMesa({ tel, texto, clasif, convId }) {
    try {
        ...
        if (Array.isArray(ej.mesa_familia) && ej.mesa_familia.length) {
            ...
            if (rFam && rFam.auto) {
                delete ej.mesa_familia;
                if (!mesa.includes(rFam.auto.id)) { mesa.push(rFam.auto.id); while (mesa.length > 3) mesa.shift(); }
                const rowF = autos.find(a => a.id === rFam.auto.id);
                await guardarMesa(tel, ej, mesa, rFam.auto.id);
                const segsF = [`Sí, claro — el ${rowF.nombre} 👍`, await ap.descripcionDe(rowF)];
                const pF = await ap.puntoHablado(rowF.id);
                if (pF) segsF.push(`Lo tenemos en ${pF}, para que lo veas cuando gustes`);
                segsF.push('¿Qué día te queda bien para venir a verlo y manejarlo? Te agendo de una vez');
                const f2F = await ap.fotosAuto(rowF.fyradrive_web_id, 2);
                return { tipo: 'mesa_entra_auto', segmentos: segsF, fotos: f2F.length ? f2F : null, fotos_after_index: 1 };
            }
        }
        ...
        if (!objetivo && familiaAmb) {
            ...
            return { tipo: 'mesa_pregunta_cual', segmentos: ['¿Cuál de estos? ' + familiaAmb.map(x => x.nombre).join(' o ') + ' 🤔'] };
        }
        ...
        if (pend === 'cotizar' && rows.length >= 1) {
            ...
            if (eng) {
                ...
                const objsCot = targetId ? rows.filter(r => r.id === targetId) : rows;
                delete ej.mesa_pregunta; delete ej.mesa_pregunta_auto;
...
async function infoEnPosesion({ tel, texto }) {
    try {
        ...
        const f2 = await ap.fotosAuto(row.fyradrive_web_id, 2);
        return { tipo: 'posesion_info', segmentos: segs, fotos: f2.length ? f2 : null, fotos_after_index: 1, universo: 'info_auto' };
    } catch (e) { return null; }
}

module.exports = { responderMesa, cargarMesa, entradaMultiple, alinearAuto, nombradoEnTexto, infoEnPosesion };
