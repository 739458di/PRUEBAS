...
const U = require('./universo.js');   // ETAPA 2: la puerta única universo → chat → estado/delegación

const normz = s => String(s || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').trim();
// PLURAL con sus letras → la acción va para TODOS los de la mesa (LA ESCENA no aplica)
const RE_PLURAL_ESCENA = /(los dos|las dos|ambos|ambas|\btodos\b|\btodas\b|los tres|esos (dos|tres)|cotizamelos|cotizalos|cotizame los|mandamelos|mandalos|mandame los|pasamelos|pasalos|pasame los|enviamelos|correlos|de los \d|los quiero ver)/;
...
const RE_PRECIO_FICHA = /(precio total|cu[aá]nto (cuesta|sale|vale|est[aá]|es|cobran)|qu[eé] precio|en cu[aá]nto (est[aá]n?|sale|(lo|la) (dan|das|tienen|tienes|manejan|vendes|venden|dejas))|^precios?\??$)/i;
...
async function cargarMesa(tel) {
    const st = await U.leerEstado(0, tel);   // (tenant 0, tel) → chat → estado del bot
    const ej = st.ej;
    ...
    if (!mesa.length && st.auto_id_activo) mesa = [Number(st.auto_id_activo)];
    const activo = st.auto_id_activo || null;
    return { existe: st.existe, ej, mesa, mesaExplicita, activo };
}
...
async function guardarMesa(tel, ej, mesa, activo, opts) {
    ...
    if (!(opts && opts.reemplazar)) {
        try {
            const ejPrev = (await U.leerEstado(0, tel)).ej;
    ...
    await U.guardarEstado(0, tel, { estado_json: ej, auto_id_activo: act, estado: 'mesa' }).catch(() => { });
}
...
const RE_CREDITO_GENERAL = /((manejan|manejas|dan|das|tienen|tienes|tiene|trabajan con|trabajas con|aceptan|aceptas|ofrecen|hay|hacen|haces|manejamos)[^?]{0,25}(cr[eé]dito|financ|a meses|mensualidades))|((cr[eé]dito|financiamiento)[^?]{0,25}(manejan|tienen|hay|ofrecen|dan))|c[oó]mo (funciona|es|va) (el|lo del|lo de)? ?(cr[eé]dito|financ)|qu[eé] (banco|financiera)|(qu[eé]|cu[aá]l) (tasa|inter[eé]s)|(tasa|inter[eé]s)(es)? (manejan|tienen|es|son|cobran)|a cu[aá]ntos meses|plazos? (manejan|tienen|hay)/i;
async function dudaGeneral({ texto, nombre, clasif, convId, tel }) {
    try {
        const t = String(texto || '');
        // ══ LEY DEL OPENER (owner 2026-07-22): si preguntamos "¿comprar o vender?",
        // la respuesta lo mete DIRECTO al cajón correcto (y la marca se borra).
        if (tel) {
            try {
                const ejR = (await U.leerEstado(0, tel)).ej;
                if (ejR.pregunta_rol) {
                    delete ejR.pregunta_rol;
                    await U.guardarEstado(0, tel, { estado_json: ejR }).catch(() => { });
                    if (/\bvend/i.test(t) && !/\bcompr/i.test(t)) {
                        // cajón VENDEDOR → Ignacio despierta (forzado: NOSOTROS preguntamos)
                        const rec = require('./recepcion.js');
                        const rIg = await rec.turnoIgnacio({ telefono: tel, convId, forzarVenta: true }).catch(() => null);
                        if (rIg && rIg.activo && rIg.segmentos && rIg.segmentos.length) return { tipo: 'rol_vendedor', segmentos: rIg.segmentos };
                        return { tipo: 'rol_vendedor', segmentos: ['Perfecto 👍 ¿Me pasas los datos de tu auto? Marca, modelo, año y kilometraje, y le echamos un ojo'] };
                    }
                    if (/\bcompr|\bbusco|interesa (un|el) (auto|carro)/i.test(t) && !/\bvend/i.test(t)) {
                        return { tipo: 'rol_comprador', segmentos: ['Perfecto 👍 ¿Qué auto andas buscando? Para pasarte lo que tenemos'] };
                    }
                    // contestó otra cosa (quizá nombró un auto) → el flujo normal lo atiende
                }
            } catch (e) { }
        }
        try { const { RE_RIESGO, RE_CONTADO } = require('./doctrina.js'); if (RE_RIESGO.test(normz(t)) || RE_CONTADO.test(normz(t))) return null; } catch (e) { }
        const RE_PREGUNTA_ENGANCHE = /\benganche\b/i;   // '¿cuánto es el enganche?' / '¿de cuánto es el enganche mínimo?' SÍ se contesta (enganche mínimo real del auto) — orden owner 2026-09-22
        if (!RE_CREDITO_GENERAL.test(t) && !RE_REQUISITOS.test(t) && !(RE_PREGUNTA_ENGANCHE.test(t) && !/\d/.test(t))) return null;
        if (engancheDeTexto(t)) return null;                 // trae cifra → es cotización
        let autoCtx = null;
        if (tel) {
            try {
                const st = await cargarMesa(tel);
                autoCtx = st.activo || (st.mesa.length ? st.mesa[st.mesa.length - 1] : null);
                // si en la misma frase NOMBRA un auto ("el Yaris también lo tienes a
                // crédito?"), ese auto ENTRA A LA MESA aunque la respuesta sea la duda
                const ap = require('./aparador.js');
                const autos = await ap.inventarioActivo();
                const rN = ap.resolverEleccion(t, autos.map(a => ({ n: 0, id: a.id, nombre: a.nombre, color: '' })));
                if (rN && rN.auto && ['nombre', 'anio'].includes(rN.via) && !ap.esNegado(t, rN.auto.nombre)) {
                    autoCtx = rN.auto.id;
                    const m2 = st.mesa.slice(); if (!m2.includes(rN.auto.id)) m2.push(rN.auto.id);
                    st.ej.escena = [rN.auto.id];
                    await guardarMesa(tel, st.ej, m2.slice(-3), rN.auto.id);
                }
            } catch (e) { }
        }
        ...
        if (r && Array.isArray(r.segmentos) && r.segmentos.length && !r.escalar && !r.silencio) {
            // pregunta DOBLE precio + crédito (agentes 2026-09-26): el precio primero, jamás se brinca
            const nT = String(t || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
            if (autoCtx && /(precio|cuanto (cuesta|sale|vale|piden|esta|es)|en cuanto (la|lo) )/.test(nT) && !r.segmentos.some(x => /\$\s?\d/.test(String(x)))) { const rp = await require('./db.js').query('SELECT precio FROM inventario_autos WHERE id=?', [Number(autoCtx)]).catch(() => []); if (rp[0] && rp[0].precio != null) return { tipo: 'duda_general_credito', segmentos: ['Son $' + Number(rp[0].precio).toLocaleString('es-MX')].concat(r.segmentos.map((x, i) => i === 0 ? String(x).replace(/^(s[ií],? as[ií] es|claro que s[ií])\.?$/i, 'Y sí, manejamos crédito') : x)), universo: r.universo || 'cotizacion' }; }
            return { tipo: 'duda_general_credito', segmentos: r.segmentos, universo: r.universo || 'cotizacion' };
        }
        // respaldo determinista del PROCESO de crédito (machote del manual)
        if (RE_CREDITO_GENERAL.test(t)) {
            return { tipo: 'duda_general_credito', segmentos: [
                'Claro que sí',
                'Es por medio de ' + (await require('./herramientas.js').bancoDeAuto(autoCtx)) + ', con muy buenas tasas. Mandas tus documentos, te cotizo, solicitamos el crédito, te dicen si apruebas en menos de 2 horas, ya aprobado tú decides cuándo firmas, y el enganche lo das a la entrega — no pagas nada hasta que se te entregue el auto',   // el banco sale del AUTO (≤2017 → Renueva Car)
                'Gustas que te mande un ejercicio para que veas cómo quedaría?'
            ], universo: 'cotizacion' };
        }
        ...
        if (/(requisit|papeler[ií]a|qu[eé] (documentos|papeles|necesito|necesitas|piden|ocupan))/i.test(t)) {
            ...
            if (REQUISITOS) return { tipo: 'duda_general_credito', segmentos: [String(REQUISITOS).trim(), 'En cuanto me los mandes por aquí, arrancamos con tu solicitud 👍'], universo: 'cotizacion' };
        }
        return null;
    } catch (e) { return null; }
}
...
function detectarAccion(t0, clasif) {
    ...
    if (/enganche/i.test(t) && !/\d/.test(t)) return null;   // '¿cuánto es el enganche?' = duda de crédito (enganche mínimo real), no precio ni cotización
...
// dato de ficha que puede viajar JUNTO con otra acción (jamás se traga)
const RE_KM_Q = /(cu[aá]nt[oa]s? (km|kil[oó]metros)|qu[eé] (km|kms|kilometraje)|kilometraje|km trae|kms? trae|trae de (km|kilometraje))/i;
function datoRapido(texto, row) {
    if (RE_KM_Q.test(String(texto || ''))) return row && row.kilometraje ? `Trae ${Number(row.kilometraje).toLocaleString('es-MX')} km` : null;
    return null;
}
...
async function ejecutarAccionMesa({ rows, accion, clasif, texto, tel, ej, mesa }) {
    ...
    // ══ LA ESCENA (owner 2026-07-22): lo que se le está enseñando AHORITA queda
    // grabado — la próxima acción implícita ("cotízame") cae sobre esto.
    if (ej && tel) { try { ej.escena = rows.map(r => r.id); await guardarMesa(tel, ej, mesa, rows[rows.length - 1].id); } catch (e) { } }
    ...
    if (accion === 'cotizar') {
        ...
        // LA RÁFAGA DE DOS PREGUNTAS (owner 2026-07-23, caso Ezequiel): "Enganche y
        // mensualidad? Qué km trae" — el dato de ficha NO se traga: viaja adelante.
        const dq = uno ? datoRapido(texto, rows[0]) : null;
        if (!eng) {
            ej.mesa_pregunta = 'cotizar';
            ej.mesa_pregunta_autos = rows.map(r => r.id);
            await guardarMesa(tel, ej, mesa, rows[rows.length - 1].id);
            const segsC = [uno ? `Va, te corro los números del ${rows[0].nombre} — ¿con cuánto de enganche le hacemos?` : `Va, te corro los números de los ${rows.length === 2 ? 'dos' : rows.length} — ¿con cuánto de enganche le hacemos?`];
            if (dq) segsC.unshift(dq);
            return { tipo: 'mesa_cotizar_pregunta', segmentos: segsC };
        }
        const outC = await cotizarMesa(rows, eng, (clasif && clasif.datos && clasif.datos.plazo_meses) || null);
        if (outC && outC.segmentos && dq) outC.segmentos.unshift(dq);
        return outC;
    }
    if (accion === 'precio') {
        const segs = [];
...
async function responderMesa({ tel, texto, clasif, convId }) {
    try {
        // 🔴 OBJECIÓN DE PAGO (owner 2026-07-23): "se me hace alta la mensualidad" NO es
        // petición de cotizar — la mesa se hace a un lado y el perro/doctrina ESCALAN.
        try { const { RE_RIESGO, RE_CONTADO } = require('./doctrina.js'); const tG = normz(String(texto || '')); if (RE_RIESGO.test(tG) || RE_CONTADO.test(tG)) return null; } catch (e) { }
        ...
        if (Array.isArray(ej.mesa_familia) && ej.mesa_familia.length) {
            // ⚠️ si NOMBRA un auto que no está entre los candidatos, la pregunta vieja
            // se descarta: lo nombrado manda (caso: pregunta de Chevrolets viva y el
            // comprador escribe "la información del sentra 2021")
            const itemsTodo = autos.map(a => ({ n: 0, id: a.id, nombre: a.nombre, color: '' }));
            const rFuera = ap.resolverEleccion(t, itemsTodo);
            if (rFuera && rFuera.auto && rFuera.via === 'nombre' && !ej.mesa_familia.includes(rFuera.auto.id)) {
                delete ej.mesa_familia; delete ej.pregunta; delete ej.mesa_familia_accion; delete ej.mesa_familia_ya;
                await guardarMesa(tel, ej, mesa, activo);
            }
        }
        if (Array.isArray(ej.mesa_familia) && ej.mesa_familia.length) {
        ...
        if (!objetivo && familiaAmb) {
            ...
            ej.pregunta = familiaAmb.map(x => x.id);   // libreta ÚNICA: el aparador la respeta
        ...
        let nuevosMulti = [], preguntaAmb = null;
        {
            ...
            if (nuevosMulti.length >= 1 && preguntaAmb) {
                ...
                if (accM) {
                    ...
                    ej.pregunta = preguntaAmb.map(x => x.id);
        ...
        // auto nuevo + DUDA GENERAL en el mismo mensaje ("el Yaris también lo tienes a
        // crédito?") → se sienta el auto y se contesta LA DUDA, no la ficha completa
        if (recienSentado && objetivo && (RE_CREDITO_GENERAL.test(t) || RE_REQUISITOS.test(t))) {
            await guardarMesa(tel, ej, mesa, objetivo);
            const dg = await dudaGeneral({ texto: t, nombre: null, clasif, convId, tel });
            if (dg) return dg;
        }
        if (recienSentado) {
        ...
        if (RE_PRECIO_FICHA.test(t) && !RE_NEGOCIA.test(t) && !/\benganche\b/i.test(t)) {   // 'enganche' no es precio
            // (bug cazado 2026-07-23, caso Brenda "este en cuánto lo tienes": objs no
            // existía → ReferenceError mudo → el turno se iba al perro como cotización)
            const escP = Array.isArray(ej.escena) ? ej.escena.map(Number).filter(Boolean) : [];
            const objs = objetivo ? rows.filter(r => r.id === objetivo)
                : (escP.length === 1 && rows.some(r => r.id === escP[0]) && !RE_PLURAL_ESCENA.test(normz(t))) ? rows.filter(r => r.id === escP[0])
                    : rows;
            if (objs.length) {
                await guardarMesa(tel, ej, mesa, objetivo || objs[objs.length - 1].id);
        ...
        if (rows.length >= 2 && !objetivo) {
            // ══ LA ESCENA (owner 2026-07-22, caso Dodge): "me podrías cotizar" sin
            // nombrar auto cae sobre LO QUE SE ESTÁ VIENDO — el último auto enseñado
            // (escena de UNO). El PLURAL con sus letras ("cotízamelos", "los dos",
            // "ambos", "todos") sí va para TODOS los de la mesa. Escena de varios o
            // desconocida → regla de oro de siempre (todos).
            const escI = Array.isArray(ej.escena) ? ej.escena.map(Number).filter(Boolean) : [];
            if (escI.length === 1 && rows.some(r => r.id === escI[0]) && !RE_PLURAL_ESCENA.test(normz(t))) {
                const accI = detectarAccion(t, clasif)
                    || (int === 'cotizar_credito' || engancheDeTexto(t) ? 'cotizar' : null)
                    || ((int === 'cita_ubicacion' || RE_UBICACION.test(t)) && !RE_FORANEO_MSG.test(t) ? 'ubicacion' : null)
                    || (int === 'fotos_videos' ? 'fotos' : null);
                if (accI) {
                    const rowVivo = rows.find(r => r.id === escI[0]);
                    const outVivo = await ejecutarAccionMesa({ rows: [rowVivo], accion: accI, clasif, texto: t, tel, ej, mesa });
                    if (outVivo) return outVivo;
                }
            }
            ...
            if ((int === 'cotizar_credito' || engancheDeTexto(t)) && detectarAccion(t, clasif) === 'cotizar') {
                ...
                if (rows.length >= 3) {
                    ej.mesa_familia = rows.map(r => r.id); ej.pregunta = rows.map(r => r.id); ej.mesa_familia_accion = 'cotizar';
                    await guardarMesa(tel, ej, mesa, mesa[mesa.length - 1]);
...
async function entradaMultiple({ tel, texto, nombre }) {
    try {
        ...
        // ⚖️ EL JUEZ DE DISPARO (owner 2026-07-22): esto es PRIMER CONTACTO — si el
        // matcher de letras propone autos, el juez razona si de verdad los nombró.
        // "No los nombró" → null (el opener genérico/variedad atiende). Sin juez o
        // confirmado → determinista de siempre. Memo: no se paga doble en el turno.
        const juezOpener = async (cands) => {
            try {
                const { juezNombroAuto } = require('./juez.js');
                const j = await juezNombroAuto({ texto: t, candidatos: cands });
                return !j || j.nombro !== false;
            } catch (e) { return true; }
        };
        if (ids.length >= 2) {
            const candsM = ids.map(id => autos.find(a => a.id === id)).filter(Boolean);
            if (!(await juezOpener(candsM))) return null;
        }
        ...
        if (ids.length < 2) {
            ...
            if (rSolo && (rSolo.auto || (rSolo.pregunta && rSolo.via === 'nombre_ambiguo'))) {
                if (!(await juezOpener(rSolo.auto ? [rSolo.auto] : rSolo.pregunta))) return null;
            }
            if (rSolo && rSolo.auto && rSolo.via === 'nombre' && !ap.esNegado(t, rSolo.auto.nombre)) {
                const rowS = autos.find(a => a.id === rSolo.auto.id);
                const ejS = (await U.leerEstado(0, tel)).ej;
            ...
            // ══ LEY DEL OPENER (owner 2026-07-22): un error ortográfico JAMÁS acredita
            // que "no lo tenemos" — "cabalier" ES el Cavalier → su flujo completo;
            // typo de una familia ("mazada") → la pregunta de cuál.
            const rT = ap.resolverConTypos(t, autos);
            if (rT && rT.auto && !ap.esNegado(t, rT.auto.nombre)) {
                const rowT = rT.auto;
                const ejT = (await U.leerEstado(0, tel)).ej;
                ejT.interes = (Array.isArray(ejT.interes) ? ejT.interes : []).concat([rowT.id]).filter((v, i, a) => a.indexOf(v) === i);
                ejT.escena = [rowT.id];
                await guardarMesa(tel, ejT, [rowT.id], rowT.id);
                const pkT = await paqueteAuto(rowT, { confirma: `Claro que sí, te paso todo del ${rowT.nombre}:` });
                return { tipo: 'mesa_entra_auto', segmentos: saludos.concat(pkT.segmentos), fotos: pkT.fotos, fotos_after_index: (pkT.fotos_after_index != null ? pkT.fotos_after_index : 0) + saludos.length };
            }
            if (rT && rT.familia && rT.familia.length > 1) {
                const ejF = (await U.leerEstado(0, tel)).ej;
                ejF.mesa_familia = rT.familia.map(x => x.id);
                await guardarMesa(tel, ejF, [], null);
                return { tipo: 'mesa_pregunta_cual', segmentos: saludos.concat(['De esos tenemos estos — ¿cuál te interesa?\n' + rT.familia.map((x, i) => `${i + 1}) ${x.nombre}`).join('\n')]) };
            }
            return null;
        }
        const mesa = ids.slice(0, 3);
        const stM = await U.leerEstado(0, tel); const ej = stM.ej;
        ej.interes = (Array.isArray(ej.interes) ? ej.interes : []).concat(mesa).filter((v, i, a) => a.indexOf(v) === i);
        if (stM.existe) await guardarMesa(tel, ej, mesa, mesa[mesa.length - 1]);
        else { ej.mesa = mesa; await U.guardarEstado(0, tel, { estado_json: ej, auto_id_activo: mesa[mesa.length - 1], estado: 'mesa' }).catch(() => { }); }
...
async function alinearAuto({ tel, texto, clasif }) {
    try {
        const stored = await U.autoActivoDe(0, tel);
        const cl = Number((clasif && clasif.auto_id) || 0) || null;
        if (!cl) return stored;
        const ap = require('./aparador.js');
        const autos = await ap.inventarioActivo();
...
async function infoEnPosesion({ tel, texto }) {
    try {
        ...
        if (!row) {
            const act = await U.autoActivoDe(0, tel);
...
module.exports = { responderMesa, cargarMesa, guardarMesa, engancheDeTexto, dudaGeneral, datoRapido, ofertaDeSegmentos, sinCortesia, entradaMultiple, alinearAuto, nombradoEnTexto, infoEnPosesion, herramientaEnPosesion, paqueteAuto, detectarAccion, ejecutarAccionMesa, preguntaFamilia };
