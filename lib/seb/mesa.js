...
async function dudaGeneral({ texto, nombre, clasif, convId, tel }) {
    try {
        ...
        if (tel) {
            try {
                const curR = await query("SELECT estado_json FROM wa_conversations WHERE telefono=?", [tel]);
                let ejR = {}; try { ejR = JSON.parse((curR[0] && curR[0].estado_json) || '{}'); } catch (e) { }
                if (ejR.pregunta_rol) {
                    delete ejR.pregunta_rol;
                    await run("UPDATE wa_conversations SET estado_json=?, updated_at=? WHERE telefono=?", [JSON.stringify(ejR), Date.now(), tel]).catch(() => { });
    ...
    try {
        ...
        if (ids.length < 2) {
            ...
            if (rT && rT.auto && !ap.esNegado(t, rT.auto.nombre)) {
                const rowT = rT.auto;
                const curT = await query("SELECT estado_json FROM wa_conversations WHERE telefono=?", [tel]);
                let ejT = {}; try { ejT = JSON.parse((curT[0] && curT[0].estado_json) || '{}'); } catch (e) { }
            ...
            if (rT && rT.familia && rT.familia.length > 1) {
                const curF = await query("SELECT estado_json FROM wa_conversations WHERE telefono=?", [tel]);
                let ejF = {}; try { ejF = JSON.parse((curF[0] && curF[0].estado_json) || '{}'); } catch (e) { }
