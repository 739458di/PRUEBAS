...
async function responderMesa({ tel, texto, clasif, convId }) {
    try {
        ...
        // 1.2) RESOLUCIÓN PROPIA de nombre (owner 2026-07-21, caso "me interesa el
        //      cavalier"): si la IA no resolvió el auto, la mesa lo resuelve SOLA con
        //      hechos duros contra TODO el inventario — jamás depender del clasificador.
        if (!objetivo) {
            const itemsInv = autos.map(a => ({ n: 0, id: a.id, nombre: a.nombre, color: '' }));
            const rInv = ap.resolverEleccion(t, itemsInv);
            if (rInv && rInv.auto && !ap.esNegado(t, rInv.auto.nombre)) {
                if (!mesa.includes(rInv.auto.id)) { mesa.push(rInv.auto.id); while (mesa.length > 3) mesa.shift(); }
                recienSentado = recienSentado || !mesaExplicita.includes(rInv.auto.id);
