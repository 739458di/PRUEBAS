...
// ══ ¿OBJETA EL COSTO O PIDE NÚMEROS? (owner 2026-07-23, caso JHONNY) ══════════
// Solo se llama cuando hay palabras de dinero y el regex determinista NO decidió.
// LA LEY: ante la duda → 'duda' (y el caller ESCALA). Jamás adivinar petición.
async function juezObjecion(texto) {
    const key = 'obj|' + String(texto || '').slice(0, 200);
    if (memo.has(key)) return memo.get(key);
    const j = await haikuJson(
        'Eres el JUEZ DE OBJECIÓN de un sistema de ventas de autos. Lee el mensaje del comprador (puede traer faltas de ortografía o palabras pegadas, p.ej. "sevmvhacevla" = "se me hace la") y decide: ¿está OBJETANDO el costo (le parece cara/alta la mensualidad, el enganche o el precio) o está PIDIENDO números con datos claros ("y si te doy 150 mil, ¿cuánto queda la mensualidad?")? peticion_clara SOLO cuando pide cotización/números con claridad. Ante cualquier duda → duda. Responde SOLO el JSON.',
        { type: 'object', properties: { razon: { type: 'string' }, veredicto: { type: 'string', enum: ['peticion_clara', 'objecion', 'duda'] } }, required: ['razon', 'veredicto'], additionalProperties: false },
        'MENSAJE: ' + String(texto).slice(0, 400),
        150);
    memo.set(key, j || null);
    return j || null;
}

module.exports = { juzgar, juezNombroAuto, juezRol, juezObjecion };
