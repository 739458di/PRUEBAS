...
async function marcarOwner(telRaw) {
    ...
    if (!tel) return false;
    try { await run("ALTER TABLE wa_conversations ADD COLUMN canal TEXT", []); } catch (e) { }
    try {
        const u = await run("UPDATE wa_conversations SET canal=COALESCE(canal,'owner') WHERE telefono=?", [tel]);
        if (!(u.rowsAffected > 0)) await run("INSERT INTO wa_conversations (telefono, canal) VALUES (?,'owner')", [tel]);
        return true;
...
async function detectarYRegistrar(telRaw) {
    ...
    try { await run("ALTER TABLE wa_conversations ADD COLUMN canal TEXT", []); } catch (e) { }
    try {
        const u = await run("UPDATE wa_conversations SET canal='messenger' WHERE telefono=?", [tel]);
        if (!(u.rowsAffected > 0)) await run("INSERT INTO wa_conversations (telefono, canal) VALUES (?,'messenger')", [tel]);
        out.canal_ok = true;
    } catch (e) { }
    ...
    try {
        ...
        if (auto) {
            ...
            if (!wc.length || !wc[0].auto_id_activo) {
                const u2 = await run("UPDATE wa_conversations SET auto_id_activo=? WHERE telefono=?", [focoId, tel]);
                if (!(u2.rowsAffected > 0)) await run("INSERT INTO wa_conversations (telefono, auto_id_activo, canal) VALUES (?,?,'messenger')", [tel, focoId]);
            }
...
async function esMessenger(telRaw) {
    const tel = String(telRaw || '').replace(/\D/g, '');
    try {
        const r = await query("SELECT canal FROM wa_conversations WHERE telefono=?", [tel]);
        return r.length && (r[0].canal === 'messenger' || r[0].canal === 'owner');
    } catch (e) { return false; }
}
