...
async function tickEspejo() {
    ...
    const tocados = [];   // ids web afectados → re-proyección puntual en fyradrive.com
    const idsDe = async (sql) => { try { return (await query(sql, [])).map(x => Number(x.id)); } catch (e) { return []; } };
    // 1) fantasmas fuera
    tocados.push(...await idsDe(`SELECT id FROM autos WHERE estado='activo' AND id NOT IN (SELECT COALESCE(fyradrive_web_id,-1) FROM inventario_autos WHERE estado='activo')`));
    ...
    tocados.push(...await idsDe(`SELECT id FROM autos WHERE estado='eliminado' AND id IN (SELECT COALESCE(fyradrive_web_id,-1) FROM inventario_autos WHERE estado='activo')`));
    ...
    for (const p of px) {
        ...
        r.precios++; tocados.push(Number(p.fw));
    }
    if (tocados.length) r.proyeccion = await require('./catalogo-web.js').proyectarWeb(tocados);
