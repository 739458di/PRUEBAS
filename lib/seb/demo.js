...
function cfgDe(tenant) { let cfg = tenant && tenant.config; if (!cfg || typeof cfg !== 'object') { try { cfg = JSON.parse((tenant && tenant.config_json) || '{}') || {}; } catch (e) { cfg = {}; } } return cfg; }
// SANDBOX (orden owner 2026-09-18): un universo real (sus autos, su IA automática) cuya tubería de WhatsApp se sustituye por el chat de la app:
// lo que sale se simula en el hilo, lo del comprador lo teclea el owner. Comparte TODA la tubería del demo (esDemo=true) pero sin
// etiquetas "[prueba]", sin reinicio automático y con autos reales.
function esSandbox(tenant) { return !!tenant && Number(cfgDe(tenant).sandbox) === 1; }
function esDemo(tenant) {
    if (!tenant) return false;
    if (tenant.demo === true) return true;
    const cfg = cfgDe(tenant);
    return Number(cfg.demo) === 1 || Number(cfg.sandbox) === 1;
}
const etq = (tenant) => esSandbox(tenant) ? '' : ' (prueba)';
...
function telComprador(tel) {
    ...
    return '52100000000' + String(10 + (h % 40)).padStart(2, '0');   // 5210000000010 … 5210000000049 (50–89 = dueños de prueba de 3 partes, 90–99 = Mario simulado y pruebas de Claude: antes un comprador podía caer en el número de un dueño y se juntaban los chats)
}
...
// RELOJ POR CHAT del sandbox (película de citas): citaf_reloj guarda el desfase de CADA cliente de prueba bajo la llave 1e9 + chat_id.
const RELOJ_BASE = 1000000000;
async function offsetChat(chatId) { try { const r = (await query('SELECT offset_ms FROM citaf_reloj WHERE tenant_id = ?', [RELOJ_BASE + Number(chatId)]))[0]; return r ? Number(r.offset_ms) || 0 : 0; } catch (e) { return 0; } }
async function ahoraChat(chatId) { return Date.now() + await offsetChat(chatId); }
// HORA SIEMPRE CRECIENTE POR CHAT (owner 2026-09-26: mensajes que saltaban hacia arriba o ni aparecían): ningún mensaje nuevo queda con hora anterior al último del chat
// (el hilo se ordena y se pagina por hora; uno "del pasado" se pintaba arriba o el refresco incremental nunca lo pedía)
async function tsMono(chatId, ts) { try { const r = (await query('SELECT MAX(ts) m FROM mensajes WHERE conversacion_id = ?', [Number(chatId)]))[0]; return Math.max(Number(ts) || 0, (Number(r && r.m) || 0) + 1); } catch (e) { return Number(ts) || Date.now(); } }
async function registrar({ tenant, tel, texto, direccion, emisor, ai, tipo, nombre, ts }) {
    ...
    const when = await tsMono(chat.id, Number(ts) || await ahoraChat(chat.id));   // hora de la PRUEBA de este chat (reloj propio por cliente; sin reloj movido = hora real)
...
async function salida(tenant, tel, texto, emisor, extra) {
    ...
    return registrar(Object.assign({ tenant, tel, texto: (esSandbox(tenant) ? '' : PREFIJO) + String(texto || ''), direccion: 'out', emisor: esBot ? 'asistente' : 'dueno', ai: esBot ? 1 : 0, tipo: 'text' }, extra || {}));
}
...
async function delegar({ tenant, tel, auto_id, auto_nombre, nombre, opener, opener_emisor }) {
    ...
    if (!yaDelegado) await sistema(tenant, p, '🤝 Chat delegado' + etq(tenant) + (auto_nombre ? ' · ' + auto_nombre : ''), nombre || null);   // como el puente: la nota nace una vez
...
async function soltar({ tenant, tel }) {
    ...
    await sistema(tenant, p, 'Chat soltado' + etq(tenant));
...
async function reset(tenant) {
    ...
    const rep = {};
    // memoria del cerebro por TELÉFONO (tablas sin universo): se limpia SOLO para los compradores de ESTE universo
    try { const tels = (await query('SELECT telefono FROM conversaciones WHERE tenant_id = ?', [tId])).map(r => String(r.telefono)).filter(Boolean); if (tels.length) { const ph = tels.map(() => '?').join(','); for (const tb of ['escalas_log', 'ad_por_telefono', 'prueba_reset']) { try { await run('DELETE FROM ' + tb + ' WHERE telefono IN (' + ph + ')', tels); } catch (e) { } } } } catch (e) { }
    ...
    rep.subir_chat = await cuenta('DELETE FROM subir_chat_msgs WHERE tenant_id = ?', [tId]) ;   // el chat de "Subir auto" también nace en cero
    rep.subir_chat_sesiones = await cuenta('DELETE FROM subir_chat_sesiones WHERE tenant_id = ?', [tId]);
    return { ok: true, borrado: rep };
}

module.exports = { tsMono, RELOJ_BASE, offsetChat, ahoraChat, esSandbox, DEMO_TEL, DEMO_NOMBRE, DEMO_CODIGO, DEMO_COMPRADOR, PREFIJO, esDemo, esTelPrueba, telComprador, asegurarTenantDemo, registrar, salida, salidaMedia, sistema, delegar, soltar, responder, reset };
