...
async function registrar({ tenant, tel, texto, direccion, emisor, ai, tipo, nombre, ts }) {
    ...
    const when = Number(ts) || await ahoraChat(chat.id);   // hora de la PRUEBA de este chat (reloj propio por cliente; sin reloj movido = hora real)
...
module.exports = { RELOJ_BASE, offsetChat, ahoraChat, esSandbox, DEMO_TEL, DEMO_NOMBRE, DEMO_CODIGO, DEMO_COMPRADOR, PREFIJO, esDemo, esTelPrueba, telComprador, asegurarTenantDemo, registrar, salida, salidaMedia, sistema, delegar, soltar, responder, reset };
