...
function telComprador(tel) {
    ...
    return esTelPrueba(d) ? (d.length === 10 ? '521' + d : d) : DEMO_COMPRADOR;
}
...
/** delegar({ tenant, tel, auto_id, auto_nombre, nombre, opener }) → { ok, chat_id, delegacion_id, opener_enviado, simulado:true } */
async function delegar({ tenant, tel, auto_id, auto_nombre, nombre, opener }) {
    const p = telComprador(tel);
    ...
    const d = await U.delegar(chat, { auto_id: auto_id || null, auto_nombre: auto_nombre || null, activado_por: 'fyrachat-demo', opener_texto: opener || null, opener_pendiente: 0 });
    if (auto_id) await run('UPDATE conversaciones SET auto_id_activo=? WHERE id=?', [Number(auto_id), chat.id]).catch(() => { });
    await sistema(tenant, p, '🤝 Chat delegado (prueba)' + (auto_nombre ? ' · ' + auto_nombre : ''), nombre || null);
    let opener_enviado = false;
    if (opener && String(opener).trim()) { const s = await salida(tenant, p, opener, 'asistente'); opener_enviado = !!s.ok; }
    return { ok: true, chat_id: Number(chat.id), delegacion_id: d && d.id ? Number(d.id) : null, opener_enviado, simulado: true, telefono: p };
}
