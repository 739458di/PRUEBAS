...
const TRUST = 'Único dueño, factura de agencia, inspección mecánica y legal';
...
async function construir({ tipo, texto, nombre, auto_id }) {
    ...
    if (tipo === 'interesa') {
        const segs = [...inicio, `Claro que sí, el ${a.nombre} sigue disponible`, TRUST];
    ...
    // INPUT 1/2 — genérico "quiero más información / detalles".
    const segs = [...inicio, `Claro, déjame te mando la información del ${a.nombre}`, `Sigue disponible. ${TRUST}`];
    if (ubic) segs.push(ubic);
    segs.push(GANCHO);
    return segs;
}
