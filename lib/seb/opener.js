...
const { fotosDeAuto, reglasFin } = require('./herramientas.js');
...
// gancho() con A/B test: rota entre 3 cierres (te interesa / prueba de manejo / combo)
// para MEDIR cuál jala más respuesta. Aleatorio, como el "Como estas?".
const INVITES = [
    'Te interesa venir a verlo?',
    'Gustas venir a hacer una prueba de manejo?',
    'Gustas venir a verlo y manejarlo?'
];
function gancho() { return INVITES[Math.floor(Math.random() * INVITES.length)]; }
// El marco de financiamiento sale de las REGLAS DEL AUTO (año → banco/enganche/plazo).
// Antes decía "Enganche desde 30%, hasta 60 meses con HEY Banco" FIJO — mentía en los
// ≤2017 (van por Renueva Car, 45% y 48 meses) y en los 2018-2021 (35%). #591 Tacoma 2014.
function financieraDe(a) {
    const r = reglasFin(a && a.anio);
    return `Enganche desde ${r.minPct}%, hasta ${r.plazoMax} meses con ${r.banco}`;
}
...
const RESP_OBJ = {
    // garantia: ESCALA (decisión del owner 2026-07-06: no hay política fija de garantía)
...
const NOMBRES_COMUNES = new Set([
    ...
    // más nombres comunes que faltaban (México)
    'lucio','moises','nestor','dario','dante','reynaldo','teodoro','fidel','leandro','eleazar','eliseo',
    'isidro','macario','melchor','pascual','urbano','zacarias','filiberto','florencio','primitivo','rosendo',
    'montserrat','monserrat','dolores','soledad','refugio','ines','ofelia','raquel','sofia','itzia','damaris','noelia'
]);
...
// VARÍA: de repente sí, de repente no (~50%) le pega un "Como estas?" (estilo del owner).
function saludo(nm) {
    const base = `Qué tal${nm ? ' ' + nm : ''} ${saludoHora()}!`;
    return Math.random() < 0.5 ? base + ' Como estas?' : base;
}
...
function tipoOpener(texto, intencion) {
    ...
    // "tarjeta" = PAGO, no financiamiento HEY (antes "aceptan tarjeta de credito?" caía a crédito).
    if (/\btarjeta/.test(t) && !/tarjeta de circulacion/.test(t)) return 'pago';
    const pideCredito = /\b(financ|credit|engan|engach|mensualidad|mensualidades|a meses|cuantos meses|a que plazo|pagos mensuales)/.test(t);
    ...
    // INFO GENERAL + dato que YA está en la ficha (km/dueños/factura/ubicación) → familia INFO
    // (la ficha "Único dueño, factura, [km], ubicación" contesta las DOS). Color/precio/transmisión
    // no están en la ficha → caen abajo en su preg_ específica.
    if (/\b(informacion|info|mas datos|más datos|mas info|más info)\b/.test(t)) {
        const si = subInfo(texto);
        if (si === 'km' || si === 'duenos' || si === 'papeleria' || /\b(donde|dónde|ubicacion|ubicación|ubicado|en que parte|en qué parte)\b/.test(t)) return 'info';
    }
    ...
    if (/\b(precio|cuesta|cuanto vale|cuánto vale|cuanto piden|que precio|en cuanto|en cuánto|valor)\b/.test(t)) return 'preg_precio';
    ...
    // INFORMACIÓN DEL AUTO (sub-áreas que aún no caza arriba: dueños, papelería, motor).
    if (subInfo(texto)) return 'info_dato';
    // INPUT 3 — me interesa / sigue disponible (caliente).
    if (/\b(me interesa|interesado|interesada)\b/.test(t)) return 'interesa';
    if (/(sigue disponible|aun disponible|aún disponible|esta disponible|está disponible|todavia.*disponible|disponible\?|a la venta|sigue en venta)/.test(t)) return 'interesa';
...
async function datosAuto(auto_id) {
    const r = await query(
        "SELECT marca, modelo, anio, precio, kilometraje, color, transmision, estado, vendido_externo, vendido_fyradrive, fyradrive_web_id FROM inventario_autos WHERE id = ?",
        [Number(auto_id)]);
    if (!r.length) return null;
    const a = r[0];
    return {
        anio: a.anio || null,                                             // para las reglas de financiamiento
        fyradrive_web_id: a.fyradrive_web_id || null,                     // para leer dueños/factura reales (tabla autos)
        ...
        color: (a.color && !/^(no especificad[oa]|n\/a|na|sin especificar|por confirmar|-+)$/i.test(String(a.color).trim())) ? cap(a.color) : null
    };
}

// Familias de PREGUNTA específicas presentes en el texto (para decidir si el opener
// puede solo o conviene el cerebro). No cuenta info/interesa genéricos.
const FAM_RES = [
    ['precio', /\b(precio|cuesta|cuanto vale|cuanto piden|que precio|en cuanto)\b/],
    ['km', /\b(km|kilometraje|kilometros|caminado|recorrido)\b/],
    ['color', /\bcolor\b/],
    ['ubic', /\b(donde|ubicacion|ubicado|en que parte|como llego)\b/],
    ['garantia', /garant/],
    ['credito', /\b(financ|credit|engan|engach|mensualidad|a meses|cuantos meses|a que plazo)\b/],
    ['foraneo', /otra ciudad|otro estado|foraneo|envi[oa]|envian|mandan|toda la republica|tracking/],
    ['pago', /tarjeta|forma de pago|transferencia|efectivo/],
    ['transmision', /estandar|automatic|transmision/],
    ['estado', /chocad|golpe|choque|tiene detalle|algun detalle|en buen estado/],
    ['mecanico', /mecanic/],
    ['separacion', /\bapart|separa|reservar/],
    ['fotos', /foto/],
    ['cuenta', /a cuenta|a cambio|toman mi|tomas mi/],
    ['buro', /\bburo\b|historial cred/]
];
function familiasDetectadas(texto) {
    const t = norm(texto);
    return FAM_RES.filter(([, re]) => re.test(t)).map(([f]) => f);
}
// ¿El opener NO basta y conviene el CEREBRO (loop, voz del owner)? Sí cuando hay 2+
// preguntas distintas (multi-pregunta) o una pregunta específica que no cae en ninguna
// familia (long-tail). NO para info/saludo/disponibilidad simple.
function necesitaCerebro(texto) {
    const t = norm(texto);
    const fams = familiasDetectadas(t);
    if (fams.length >= 2) return true;
    if (fams.length === 0) {
        const generica = /\b(informacion|info|detalles|mas datos|mas informacion|saber mas|me interesa|interesad|disponible|sigue disp|hola|buenas|buen dia|buenos dias|gustas|cita)\b/.test(t);
        const pareceQuestion = /\?/.test(texto) || /^(es |tiene |trae |viene |sirve|funciona|acepta|cuenta con|que tal|como es|de que|cual es)/.test(t);
        if (!generica && pareceQuestion && t.trim().split(/\s+/).length >= 2) return true;
    }
    return false;
}
...
async function infoAuto(a) {
    const h = await require('./aparador.js').hechosDe(a.fyradrive_web_id || a.web || a.id);   // verdad de la base: sin dato, no se afirma (antes: 'Único dueño, factura de agencia' de cualquier auto)
    const p = []; if (h.duenos) p.push(h.duenos.charAt(0).toUpperCase() + h.duenos.slice(1)); if (h.factura === 'ok') p.push('factura de agencia');
...
async function lineaUbicacion(auto_id) {
    const pe = await query("SELECT name, maps_link FROM punto_envio WHERE auto_id = ?", [Number(auto_id)]);
    let name = (pe.length && pe[0].name) ? pe[0].name : null;
    let mapsLink = pe.length ? pe[0].maps_link : null;
    // Fallback al PIN de puntos.html si el auto no tiene fila en punto_envio.
    if (!name) {
        const inv = await query("SELECT puntos_venta FROM inventario_autos WHERE id = ?", [Number(auto_id)]);
        if (inv.length) {
            try {
                const p = JSON.parse(inv[0].puntos_venta || '[]');
                if (p[0] && p[0].name) {
                    name = String(p[0].name).trim();
                    if (!mapsLink && p[0].lat != null && p[0].lng != null) mapsLink = `https://maps.google.com/?q=${p[0].lat},${p[0].lng}`;
                }
            } catch (e) { /* sin pin válido */ }
        }
    }
    if (!name) return null;
    const zona = zonaDeMaps(mapsLink);
    return zona ? `La tenemos en ${name}, por ${zona}` : `La tenemos en ${name}`;
}
...
async function construir({ tipo, texto, nombre, auto_id }) {
    ...
    // Si ADEMÁS pidió info general (ej. "quiero info" + "tiene garantía"), mete la ficha
    // antes de la respuesta → contesta LAS DOS cosas, en ≤5.
    // INFORMACIÓN DEL AUTO (dueños/papelería/motor) → ritual opener + el dato, en la voz del owner.
    if (tipo === 'info_dato') {
        const ia = await infoAccion(texto, auto_id);
        if (!ia || !ia.accion) return null;   // motor / sin dato → no autopilot (lo ve el owner)
        return [...inicio, ia.accion, gancho()];
    }

    if (tipo === 'transmision' || RESP_OBJ[tipo]) {
        const ans = (tipo === 'transmision')
            ? (a.transmision ? `Es ${a.transmision}` : 'Con gusto te confirmo la transmisión')
            : RESP_OBJ[tipo];
        if (/\b(informacion|info|detalles|mas datos|más datos)\b/.test(norm(texto))) {
            const infoL = ubic ? `${await infoAuto(a)}. ${ubic}` : await infoAuto(a);
            return [...inicio, infoL, ans, gancho()];
        }
        return [...inicio, ans, gancho()];
    }
    ...
    if (tipo === 'info_credito') {
        const fin = /\b(buro|buró|historial)\b/.test(norm(texto))
            ? `El financiamiento queda así: ${financieraDe(a)}, sujeto a aprobación bancaria`
            : `El financiamiento queda así: ${financieraDe(a)}`;
        return [...inicio, `Claro, el ${a.nombre} sigue disponible. ${await infoAuto(a)}`, fin, gancho()];
    }

    // INPUT 4 — financiamiento: SIN km, SIN ubicación. (queda así + términos en UN mensaje)
    if (tipo === 'financiamiento') {
        const segs = [...inicio, `Claro que sí, el financiamiento del ${a.nombre} queda así: ${financieraDe(a)}`];
    ...
    if (tipo === 'interesa') {
        const info = ubic ? `${await infoAuto(a)}. ${ubic}` : await infoAuto(a);
...
async function responder({ texto, nombre, auto_id, intencion }) {
    ...
    if (tipo === 'garantia') return null;     // garantía → ESCALA (sin política fija; lo ve el owner)
    ...
    const out = { tipo, segmentos };
    // FOTOS: si pidieron fotos, adjunta las URLs (el bridge las descarga y manda). Van
    // después del "déjame te las mando" y antes del gancho (último segmento).
    if (tipo === 'fotos') {
        const urls = await fotosDeAuto(auto_id, 5);
        if (urls.length) { out.fotos = urls; out.fotos_after_index = segmentos.length - 2; }
    }
    return out;
}

module.exports = { responder, SENTINEL, tipoOpener, nombreReal, saludoHora, zonaDeMaps, necesitaCerebro, familiasDetectadas };
