...
async function construir({ tipo, texto, nombre, auto_id }) {
    ...
    // COMBO — info + crédito en el mismo mensaje: disponibilidad + marco de crédito, en 5.
    if (tipo === 'info_credito') {
        ...
        return [...inicio, `Claro, el ${a.nombre} sigue disponible`, fin, GANCHO];
    }
    ...
    // INPUT 3 — me interesa / disponible (caliente). Info + ubicación JUNTAS.
    if (tipo === 'interesa') {
        const disp = ubic ? `Claro que sí, el ${a.nombre} sigue disponible. ${ubic}` : `Claro que sí, el ${a.nombre} sigue disponible`;
        return [...inicio, disp, GANCHO];
    }

    // INPUT 1/2 — genérico "quiero más información / detalles" (5 mensajes exactos).
    const infoUbic = ubic ? `Sigue disponible. ${ubic}` : 'Sigue disponible';
    return [...inicio, `Claro, déjame te mando la información del ${a.nombre}`, infoUbic, GANCHO];
}
