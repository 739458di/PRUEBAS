...
async function barrer({ tel, ahora, ultimoInTs, standby }) {
    try {
        ...
        const enviar = [];
        // VETO DE CITA VIVA (diseño firmado): cita agendada → CITAS VIVAS gobierna y
        // esta máquina SUELTA el folio — jamás dos máquinas sobre el mismo lead.
        const nowMx = (() => { const d = mx(ahora); return d.toISOString().slice(0, 19).replace('T', ' '); })();
        for (const r of rows) {
            try {
                const t10 = String(r.telefono).slice(-10);
                const cv = await query("SELECT estado, fecha_hora FROM citas WHERE comprador_telefono LIKE ? AND estado IN ('agendada','confirmada') ORDER BY id DESC LIMIT 3", ['%' + t10]);
                if (cv.some(c => String(c.fecha_hora || '') >= nowMx)) { await cerrar(r, 'cita viva — gobierna Citas Vivas', ahora); continue; }
            } catch (e) { }
