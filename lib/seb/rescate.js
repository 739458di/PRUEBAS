...
async function tuvoCita(tel) {
    try {
        const t10 = String(tel).replace(/\D/g, '').slice(-10);
        if (!t10) return false;
        const r = await query("SELECT id FROM citas WHERE comprador_telefono LIKE ? LIMIT 1", ['%' + t10]);
        return r.length > 0;
...
async function dedupVivos(tel, ahora) {
    try {
        const r = await query("SELECT id FROM rescates WHERE telefono=? AND estado='vivo' ORDER BY id DESC", [tel]);
...
async function barrer({ tel, ahora, ultimoInTs, standby }) {
    try {
        ...
        for (const r of rows) {
            ...
            try {
                const prog = await query("SELECT id FROM mensajes_programados WHERE telefono LIKE ? AND estado='pendiente' LIMIT 1", ['%' + String(r.telefono).slice(-10)]);
...
async function preAvisos({ ahora }) {
    try {
        await ensureTabla();
        ahora = ahora || Date.now();
        let rows = await query("SELECT * FROM rescates WHERE estado='vivo' AND avisado=0 AND proxima_ts>? AND proxima_ts<=?", [ahora, ahora + 40 * 60000]);
