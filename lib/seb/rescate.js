...
async function registrarSalidaManual({ tel, texto, ahora }) {
    try {
        await ensureTabla();
        ahora = ahora || Date.now();
        const r = await vivo(tel);
        if (r && r.tipo === 'promesa') return { accion: 'promesa_intacta' };
        const conGancho = /\?/.test(String(texto || ''));
        // EL CARRIL SE ACREDITA CON HECHO DURO, no con la palabra (owner 2026-07-23:
        // "¿viste lo del enganche?" NO es una cotización): caliente SOLO si en la
        // conversación reciente existe una TARJETA real ("Enganche:" formato exacto).
        let carril = 'tibio';
        try {
            ...
            if (cv.length) {
                ...
                if (outs.some(m => /Enganche:/.test(String(m.texto || '')))) carril = 'caliente';
            }
        } catch (e) { }
        if (r && r.tipo === 'ghost') {
            ...
            await run("UPDATE rescates SET etapa=0, carril=?, pendiente=?, proxima_ts=?, historial=?, updated=? WHERE id=?",
                [carril, 'manual', enVentana(ahora + 60 * 60000), await anotar(r, 're_armado_manual'), ahora, r.id]);
            return { accion: 'ghost_rearmado' };
        }
        if (!conGancho) return { accion: 'sin_folio' };
        await run("INSERT INTO rescates (telefono, tipo, estado, etapa, carril, escala, pendiente, proxima_ts, historial, created, updated) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            [tel, 'ghost', 'vivo', 0, carril, '', 'manual', enVentana(ahora + 60 * 60000), JSON.stringify([{ ts: ahora, ev: 'armado', bala: 'manual' }]), ahora, ahora]);
