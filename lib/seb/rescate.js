...
function machote(r, ctx) {
    ...
    if (etapa === 0) {   // toque 1 — revividor por carril (dictado del owner)
        if (bala === 'cotizacion') return `Qué onda${nom}, ¿cómo te pareció la cotización? ¿Qué pensaste — le damos? 👍`;
        if (bala === 'ubicacion') return `¿Qué piensas${nom}? ¿Te queda cerca, o qué horario te queda bien? Pendiente si te interesa venir, para coordinarte 👍 Es una ventaja comprar un auto con nuestros filtros de calidad`;
        return autoNom
            ? `¿Qué piensas${nom}? El ${autoNom} está en excelente estado, la verdad te recomiendo venir a manejarlo — te separo la hora y el día sin tema 👍`
            : `¿Cómo la ves${nom}? ¿Te late o te busco otra opción? 👍`;
    }
    if (etapa === 1) return `Qué tal${nom}, ${saludoMx(ahora)}! ¿Qué has pensado?`;   // + portada si hay auto
    return `Qué tal${nom}, ${saludoMx(ahora)}! ¿Qué has pensado? ¿Sigues teniendo interés en un auto? Me avisas para ayudarte — te conviene comprar con Fyradrive 👍`;   // + portada si hay auto
}
...
async function ctxDe(tel, etapa, ahora) {
    ...
    try {
        ...
        if (aid) {
            const a = await query("SELECT id, marca, modelo, version, anio, fyradrive_web_id FROM inventario_autos WHERE id=? AND estado='activo'", [aid]);
            if (a.length) {
                ctx.auto = { id: a[0].id, nombre: [a[0].marca, a[0].modelo, a[0].version, a[0].anio].filter(Boolean).join(' ') };
...
async function barrer({ tel, ahora, ultimoInTs, standby }) {
    try {
        ...
        for (const r of rows) {
            ...
            enviar.push({ folio: folioDe(r), telefono: r.telefono, texto, foto: ctx.foto || null, tipo: r.tipo, etapa });
        }
        return enviar;
