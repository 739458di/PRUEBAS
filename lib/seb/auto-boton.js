...
async function correr({ tenant, chat, texto, msgId, puerta, rastro, dry }) {
    ...
    if (accion === 'fotos' && !RE_PIDE_FOTOS.test(t)) return log(Object.assign(base, { accion, resultado: 'sin_peticion_explicita' }));
    if (accion === 'ubicacion' && !RE_PIDE_UBIC.test(t)) return log(Object.assign(base, { accion, resultado: 'sin_peticion_explicita' }));
