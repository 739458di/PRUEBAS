...
// GUION (modo video, orden owner 2026-09-15): burbujas "humanas" ANTES y DESPUÉS del botón, dictadas por el owner (no las redacta la IA).
// { antes: { fotos: ['Va, aquí están'] }, despues: { fotos: ['¿Qué te parece?', 'Como nuevo 😎'] }, enganche_pct: 0.35, pausa_ms: 1200, sin_dedupe: true }
async function decir(puerta, cid, textos, clave, pausa) {
    const out = [];
    for (let i = 0; i < textos.length; i++) {
        if (i > 0 && pausa) await new Promise(r => setTimeout(r, pausa));
        try { out.push(await puerta('enviar', { chat_id: cid, texto: textos[i], clave: clave + ':txt' + i })); } catch (e) { out.push({ ok: false, error: e.message }); }
    }
    return out;
}
async function correr({ tenant, chat, texto, msgId, puerta, rastro, guion, dry }) {
    await ensure();
    ...
    if (accion === 'cotizar' && !(Number(d.enganche) > 0)) {
        if (guion && guion.enganche_pct && auto && Number(auto.precio) > 0) d.enganche = Math.round(Number(auto.precio) * guion.enganche_pct);   // modo video: enganche por defecto
        else return log(Object.assign(base, { accion, resultado: 'sin_enganche' }));
    }
    // REPETIR: solo si el comprador lo pide de nuevo explícitamente, y jamás dentro de 2 min (eso es bug, no petición)
    if (ya.includes(accion) && !(guion && guion.sin_dedupe)) {
    ...
    if (dry) return log(Object.assign(base, { accion, resultado: 'dry', detalle: { enganche: d.enganche, plazo: d.plazo_meses, fecha_iso: d.fecha_iso, hora: d.hora } }));
    // ── APRETAR EL BOTÓN por la misma puerta que la UI (idempotente por clave) ──
    const clave = 'auto:' + cid + ':' + accion + ':' + (msgId || Date.now());
    const pausa = (guion && guion.pausa_ms) || 0;
    if (guion && guion.antes && guion.antes[accion] && guion.antes[accion].length) await decir(puerta, cid, guion.antes[accion], clave + ':a', pausa);
    let r;
    try {
    ...
    const ok = !!(r && r.ok);
    if (ok && guion && guion.despues && guion.despues[accion] && guion.despues[accion].length) { if (pausa) await new Promise(x => setTimeout(x, pausa)); await decir(puerta, cid, guion.despues[accion], clave + ':d', pausa); }
