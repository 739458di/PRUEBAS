...
async function decidir({ texto, historial, auto, yaMandadas }) {
    ...
    const system =
        ...
        'CONTEXTO PRIMERO: lee el mensaje NUEVO como RESPUESTA a lo último que dijo Seb/el vendedor. Si le acaban de preguntar algo ("¿a qué hora te coordinamos una cita?") y contesta algo corto que NO pide nada ("déjame ver", "deja checo", "ahorita te digo", "lo veo con mi esposa") → nada. "Ver" en "déjame ver" es "déjame pensarlo", no "quiero ver fotos". ' +
...
async function correr({ tenant, chat, texto, msgId, puerta, rastro, dry }) {
    await ensure();
    ...
    // FOTOS / UBICACIÓN mandan archivos: solo con petición EXPLÍCITA en el mensaje (compuerta de código después del modelo)
    if (accion === 'fotos' && !/(foto|im[aá]gen|video|v[ií]deo|por dentro|por fuera|interior|exterior|c[oó]mo (se ve|luce|est[aá] de)|ens[eé][ñn]a|mu[eé]stra)/i.test(t)) return log(Object.assign(base, { accion, resultado: 'sin_peticion_explicita' }));
    if (accion === 'ubicacion' && !/(d[oó]nde|ubicaci|direcci|domicilio|zona|colonia|sucursal|mapa|\bpin\b|c[oó]mo llego|por d[oó]nde|queda (lejos|cerca)|en qu[eé] parte)/i.test(t)) return log(Object.assign(base, { accion, resultado: 'sin_peticion_explicita' }));
