...
//   info · fotos · ubicacion · cotizar (con enganche) · cita (con día+hora) · cita_propuesta (quiere verlo, sin horario)
...
const ACCIONES = ['info', 'fotos', 'ubicacion', 'cotizar', 'cita', 'cita_propuesta'];
const ETIQUETA = { info: 'Información', fotos: 'Fotos', ubicacion: 'Ubicación', cotizar: 'Cotizar', cita: 'Agendar cita', cita_propuesta: 'Cita · proponer horario' };
...
const SCHEMA = {
    type: 'object', properties: {
        ...
        intencion: { type: 'string', enum: ['info', 'fotos', 'ubicacion', 'cotizar', 'cita', 'cita_propuesta', 'nada'] },
    ...
    }, required: ['razon', 'intencion', 'confianza', 'enganche', 'plazo_meses', 'fecha_iso', 'hora'], additionalProperties: false
};
...
async function decidir({ texto, historial, auto, yaMandadas }) {
    ...
    const system =
        ...
        '· cita = agenda la cita. SOLO si dio día concreto Y hora concreta (o se deducen sin duda: "mañana a las 5" = mañana 17:00).\n' +
        '· cita_propuesta = le pregunta qué día y hora le queda. Cuando claramente quiere ir a verlo pero no dio día u hora.\n' +
        '· nada = no apretar nada: saludos, gracias, dudas de dinero/negociación/descuento/trade-in, preguntas que un botón no contesta, comentarios, o cuando la acción YA se le mandó y no la vuelve a pedir explícitamente.\n' +
    ...
    if (!r || !ACCIONES.concat('nada').includes(r.intencion)) return { intencion: 'nada', confianza: 'baja', razon: r ? 'salida inválida' : 'sin IA disponible', enganche: null, plazo_meses: null, fecha_iso: null, hora: null };
    return r;
}
...
async function correr({ tenant, chat, texto, msgId, puerta, rastro }) {
    ...
    const ya = (await query("SELECT DISTINCT accion FROM auto_boton_log WHERE chat_id = ? AND resultado = 'ok' AND ts > ?", [cid, desde]).catch(() => [])).map(r => r.accion);
    ...
    if (accion === 'cita' && !(/^\d{4}-\d{2}-\d{2}$/.test(String(d.fecha_iso || '')) && /^\d{1,2}:\d{2}$/.test(String(d.hora || '')))) accion = 'cita_propuesta';   // sin día+hora firme → propuesta
    if (accion === 'cotizar' && !(Number(d.enganche) > 0)) return log(Object.assign(base, { accion, resultado: 'sin_enganche' }));
    if (ya.includes(accion)) return log(Object.assign(base, { accion, resultado: 'repetida_6h' }));
