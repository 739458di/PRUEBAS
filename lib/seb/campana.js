...
async function liberar(tel) {
    ...
    try { require('./memo.js').olvidar('mudos'); } catch (e) { }
}
...
async function setMudos() {
    try {
        await ensure();
        // CUOTA TURSO (2026-09-08): cacheado 30 s — barrer() y preAvisos() lo piden en el MISMO
        // ghost_scan; callar()/liberar() lo invalidan al escribir.
        return await require('./memo.js').memo('mudos', 30000, async () => {
