...
function planRecordatorios(match_ts, cita_ts, ctx) {
    ...
    if (!mismoDia) {
        ...
        add('vispera_vendedor', Date.UTC(c.getUTCFullYear(), c.getUTCMonth(), c.getUTCDate() - 1, 20, 0) + MTY_OFF + 120000, 'vendedor',
            `Qué tal ${ctx.dueno}, buenas noches. Te recuerdo que mañana a las ${ctx.hora} es la cita de tu ${ctx.auto} 👍 De favor tenlo listo y disponible — yo te aviso en cuanto el comprador vaya en camino.`);
...
async function tickRecordatorios() {
    ...
    return { ok: true, matches: rows.length, enviados, senales, cierres, cita_jobs: jobs, cargas };
}
...
module.exports = {
    ...
    ejecutarCierre, ejecutarCitaJobs, respuestasCierre, resolverAutoCierre };
