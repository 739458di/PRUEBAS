...
async function ejecutarMatch(M, { notificarVendedor = false, avisarComprador = true } = {}) {
    ...
    if (avisarComprador) await enviarWA(M.comprador_tel, MSJ.confianza(ctx));
...
async function matchDirectoCalendar({ comprador_tel, comprador_nombre, dueno_tel, dueno, auto_id, auto_nombre, fecha, hora, cita_ts, avisar }) {
    await ensureCitasMatch();
    ...
    const vivos = await query("SELECT * FROM citas_match WHERE estado='match'");
    ...
    const ins = await run(`INSERT INTO citas_match (comprador_tel, comprador_nombre, dueno_tel, dueno, auto_id, auto_nombre, fecha, hora, cita_ts, estado, updated)
               VALUES (?,?,?,?,?,?,?,?,?,'solicitud',?)`,
        [telC, comprador_nombre || null, String(dueno_tel || ''), dueno || 'Vendedor', auto_id || null, auto_nombre || null, fecha || '', hora || '', Number(cita_ts), Date.now()]);
...
async function manejarMensajeStaff(tel, texto) {
    ...
    const filas = await query("SELECT * FROM cita_vendedores WHERE estado IN ('invitado','confirmado') AND updated > ? ORDER BY updated DESC", [Date.now() - 30 * 86400000]);
