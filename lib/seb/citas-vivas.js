...
function planRecordatorios(match_ts, cita_ts, ctx) {
    ...
    if (!mismoDia) {
        ...
        if (selloTs && selloTs > match_ts + 3 * 3600000) {
            ...
            // UNIVERSOS DE VENDEDOR (orden owner 2026-09-10): el sello "enviado por SEB, agente de IA de Fyradrive" NO sale
            // por el número de un vendedor — nadie eligió que el bot se presente ahí. Solo tenant 0.
            const tenantIdPlan = Number(ctx && ctx.tenant_id) || 0;
            if (!esDiaVispera && !tenantIdPlan) add('sello', selloTs, 'comprador',
...
function ensureDireccionCitas() {
    ...
    _ensDir = (async () => {
        ...
        // CITAS POR UNIVERSO (orden owner 2026-09-10): las lecturas de respaldo por teléfono (filas viejas sin chat)
        // van acotadas al universo → índice (comprador_telefono, tenant_id, estado): exacto por tel, luego el universo.
        await run('CREATE INDEX IF NOT EXISTS idx_citas_tel_tenant ON citas(comprador_telefono, tenant_id, estado)').catch(() => { });
...
// origen (opcional, 2026-09-12): etiqueta que el puente exige en universos ≠0 ('casilla' | 'maquina_cita' | …); default 'maquina_cita'.
// NÚMERO DE CASA (orden owner 2026-09-23): el WhatsApp principal (5659423834) pasó a ser de TERRA (universo 9). Lo que antes salía
// "desde Fyradrive" (universo 0: códigos de acceso, avisos) sale por el universo que hoy tiene ese número: tenants[0].config.casa_tenant.
let _casa = { t: 0, id: 0 };
async function casaTenant() {
    if (Date.now() - _casa.t < 60000) return _casa.id;
    let id = 0; try { const r = await query("SELECT telefono, config_json FROM tenants WHERE id = 0"); if (r.length && !r[0].telefono) { const c = JSON.parse(r[0].config_json || '{}'); id = Number(c.casa_tenant) || 0; } } catch (e) { }
    _casa = { t: Date.now(), id }; return id;
}
async function enviarWA(tel, texto, tenantId, origen) {
    if (esTelPrueba(tel)) return { ok: true, simulado: true };
    let tId = Number(tenantId) || 0;                        // 0 = número de Fyradrive; N = universo del vendedor
    if (!tId) tId = await casaTenant();                     // sin número en el 0 → sale por el universo de casa
    ...
    try {
        const r = await fetch(url, {
            ...
            body: JSON.stringify(Object.assign({ phone: String(tel), text: texto, origen: String(origen || 'maquina_cita') }, tId ? { tenant_id: tId } : {}))
        });
...
async function casillaEjecutar(idOrRow, opts) {
    ...
    if (C.staff_id != null) {
        ...
        if (C.para === 'vendedor') tenantId = (M && Number(M.tenant_id)) ? Number(M.tenant_id) : 0;   // ORDEN owner 2026-09-10: al vendedor le habla SU propio universo (t0 solo para dueños de Fyradrive)
        else if (M) tenantId = Number(M.tenant_id) || 0;  // al comprador, por el universo de la cita
        // compuerta determinista (2026-09-10): una casilla 'sello' vieja de un universo de vendedor jamás sale (el sello se presenta como IA de Fyradrive)
        if (!salto && String(C.tipo) === 'sello' && M && Number(M.tenant_id)) salto = 'sello_universo';
    }
    if (salto) {
    ...
    // ══ SOLO POR BOTÓN (auditoría H, 2026-09-12): en un universo de vendedor, si ÉL escribió a mano en ese chat hace
    //    <15 min, la casilla al COMPRADOR no se salta: se POSPONE 15 min (due_ts += 15 min) para no pisarle la conversación.
    if (tenantId && C.para === 'comprador' && C.chat_id != null && !opts.forzar) {
        const POSPONER_MS = 15 * 60000;
        let ultManual = 0;
        try { const r = await query("SELECT ts FROM mensajes WHERE conversacion_id=? AND direccion='out' AND COALESCE(ai_generated,0)=0 AND COALESCE(emisor,'')<>'sistema' ORDER BY ts DESC LIMIT 1", [Number(C.chat_id)]); ultManual = r.length ? Number(r[0].ts) : 0; } catch (e) { }
        if (ultManual && now - ultManual < POSPONER_MS) {
            const nuevoDue = now + POSPONER_MS;
            await run("UPDATE cita_casillas SET due_ts=?, error=?, updated=? WHERE id=? AND estado IN ('pendiente','enviando')", [nuevoDue, 'pospuesta: vendedor activo hace <15 min', now, C.id]);
            try { await programarEnPuente([{ id: C.id, due_ts: nuevoDue, tel: C.tel }]); } catch (e) { }
            await T('casilla', C.id, 'pospuesta');
            return { ok: true, pospuesta: true, due_ts: nuevoDue, motivo: 'vendedor_activo' };
        }
    }
    ...
    const res = await enviarWA(C.tel, C.texto, tenantId, 'casilla');
    if (res.ok) {
...
async function ejecutarMatch(M, { notificarVendedor = false, avisarComprador = true } = {}) {
    ...
    const ctx = { nombre: (M.comprador_nombre || '').split(/\s+/)[0] || 'amigo', dueno: (M.dueno || 'amigo').split(/\s+/)[0], auto: M.auto_nombre || 'auto', hora: M.hora, fecha: M.fecha, tenant_id: Number(M.tenant_id) || 0 };
    const recs = planRecordatorios(matchTs, Number(M.cita_ts), ctx);   // tenant≠0 → sin casilla 'sello' (2026-09-10)
...
// ── CANCELACIÓN DETERMINISTA (universos ≠0, sin IA): claro = cancel|ya no|no voy|no podr; ambiguo = suena a no-asistir/mover sin ser claro
function clasificarCancelacionDeterminista(texto) {
    const t = norm(texto);
    const proponeOtra = /(mejor (a las|el)|puedo (a las|el)|que sea (a las|el)|cambiamos? a|a las \d{1,2}\b)/.test(t);
    const claro = !proponeOtra && /(cancel|ya no\b|no voy\b|no podr)/.test(t);
    const suena = /(no (voy a |vamos a )?(poder|podre|podremos)|no (llego|logro llegar|alcanzo|alcanzare)|no (ire|asistire|vamos)\b|se me complico|me surgio|no me va a dar tiempo|imposible (llegar|ir)|no va a poderse|no se va a poder|otra hora|otro dia|reagendar|mover la|cambiar la)/.test(t);
    return { cancela: claro, ambiguo: !claro && (suena || proponeOtra) };
}
// ── A QUIÉN SE LE AVISA (2026-09-12): universo de vendedor → al VENDEDOR (M.dueno_tel) por SU universo; universo 0 → al owner
async function avisarResponsable(M, texto) {
    try {
        const tId = Number(M.tenant_id) || 0;
        if (tId) {
            const telV = String(M.dueno_tel || '').replace(/\D/g, '');
            if (!telV) return { ok: false, error: 'sin teléfono del vendedor' };
            return await enviarWA(telV, texto, tId, 'maquina_cita');
        }
        return await enviarWA(OWNER_TEL, texto);
    } catch (e) { return { ok: false, error: e.message }; }
}
async function _comprador(M, texto) {
    ...
    if (Number(M.cita_ts) && Date.now() > Number(M.cita_ts)) {
        ...
        // universo de vendedor (2026-09-12): el aviso es para ÉL, por SU universo — jamás al owner de Fyradrive
        await avisarResponsable(M, `💬 POST-CITA de ${M.comprador_nombre || M.comprador_tel} (${M.auto_nombre || 'auto'}, ${M.fecha} ${M.hora} — ya pasó): «${String(texto).slice(0, 140)}» — NO la cancelo. Márcala en Calendar: 🤝 CITA REALIZADA + resultado.`);
        return null;
    }
    if (Number(M.tenant_id)) {
        // ══ UNIVERSO DE VENDEDOR — SOLO POR BOTÓN (auditoría H, 2026-09-12): cancelación DETERMINISTA, sin IA.
        //    claro → se cancela la máquina; ambiguo (suena a cancelar/mover pero no es claro) → se le avisa al vendedor y el bot calla.
        const cd = clasificarCancelacionDeterminista(texto);
        if (cd.cancela) {
            await ejecutarCancelacion(M, { por: 'comprador', texto });
            return MSJ.canceladaComprador();
        }
        if (cd.ambiguo) {
            await ACC.registrar(Object.assign({}, dirM, { tipo: 'cita_duda', ref_id: M.id, meta: { texto: String(texto).slice(0, 120) }, actor: 'comprador' }));
            await avisarResponsable(M, `⚠️ ${M.comprador_nombre || M.comprador_tel} escribió algo sobre la cita del ${M.auto_nombre || 'auto'} (${M.fecha} ${M.hora}) que no me queda claro: «${String(texto).slice(0, 140)}» — la cita sigue en pie; revísalo tú y, si va, cancélala o muévela desde tu FyraChat.`);
            return null;
        }
    } else {
...
async function pausarPorDesvio(M, quien, texto) {
    ...
    if (Number(M.tenant_id)) {   // universo de vendedor (2026-09-12): el aviso es para ÉL, por SU universo — jamás al owner de Fyradrive
        await avisarResponsable(M, aviso.replace('en el Calendar (o "cita confirmada ✅" + día + hora + auto)', 'desde tu FyraChat'));
        return true;
    }
    let alStaff = 0;
    try {
...
async function ejecutarCancelacion(M, opts) {
    ...
    if (opts.avisar === false) return true;   // SOLO POR BOTÓN (2026-09-12): el vendedor canceló desde SU pantalla → no se le avisa a él mismo
    const ctx = { dueno: (M.dueno || '').split(/\s+/)[0], fecha: M.fecha, hora: M.hora };
    if (String(M.dueno_tel || '').replace(/\D/g, '')) await enviarWA(M.dueno_tel, MSJ.canceladaVendedor(ctx), Number(M.tenant_id) || 0, 'maquina_cita');   // al vendedor le habla SU universo (orden 2026-09-10)
    return true;
}
        ...
        try {
            ...
            if (fh && (fh.fecha || fh.hora)) {
                ...
                if (nts && Math.abs(nts - Number(M.cita_ts)) > 60000) {
                    ...
                    await registrarCitaCanonica({ telefono: M.comprador_tel, fecha: nf, hora: nh, tenant_id: Number(M.tenant_id) || 0 });
                    ...
                    try {
                        ...
                        // CITAS POR UNIVERSO (2026-09-10): el respaldo por teléfono también va acotado al universo de la máquina
                        // (idx_citas_tel_tenant); jamás mueve la cita de ESTE comprador en OTRO universo.
                        const cr = M.chat_id != null
                            ...
                            : await query("SELECT id FROM citas WHERE comprador_telefono=? AND COALESCE(tenant_id,0)=? AND estado='agendada' ORDER BY id DESC LIMIT 1", [String(M.comprador_tel), Number(M.tenant_id) || 0]);
                        if (cr.length) {
...
async function ejecutarCierre({ tel, texto, ts, origen, tenant_id }) {
    tel = String(tel || '').replace(/\D/g, '');
    if (!tel || esTelPrueba(tel)) return { ok: false, motivo: 'tel_prueba' };
    // universo ≠ 0 → no es cierre del owner (el vendedor agenda desde SU FyraChat: citaGenesis con tenant_id)
    if (Number(tenant_id)) return { ok: false, motivo: 'universo_' + Number(tenant_id) + ' — la máquina del cierre es del universo 0' };
    ...
    // CITAS POR UNIVERSO (2026-09-10): la máquina del cierre es EXCLUSIVA del universo 0 (el owner cierra desde el
    // número de Fyradrive; cita_jobs/canónica/avisos a OWNER_TEL). Candado y canónica acotados a tenant 0 —
    // filas viejas sin etiqueta cuentan como 0 (COALESCE). La cita de este comprador en OTRO universo no se toca.
    const ya = await query("SELECT id FROM cita_canonica WHERE COALESCE(tenant_id,0)=0 AND created > ? AND telefono LIKE ?", [msgTs, '%' + tel.slice(-10)]);
    if (ya.length) return { ok: false, motivo: 'ya_ejecutado' };
    await registrarCitaCanonica({ telefono: tel, fecha: cierre.fecha, hora: cierre.hora, tenant_id: 0 });
...
async function cierresOwner() {
    // CITAS POR UNIVERSO (2026-09-10): solo chats del universo 0 — un "cita confirmada" manual de un VENDEDOR en su
    // universo no es cierre del owner (su cita nace por citaGenesis con tenant_id desde su FyraChat).
    const rows = await query(
        "SELECT m.id, m.texto, m.ts, c.telefono FROM mensajes m JOIN conversaciones c ON c.id = m.conversacion_id WHERE m.direccion='out' AND COALESCE(m.ai_generated,0)=0 AND m.ts > ? AND lower(m.texto) LIKE '%cita confirmada%' AND m.msg_id NOT LIKE 'sbx_%' AND COALESCE(c.tenant_id,0)=0 ORDER BY m.ts ASC",
        [Date.now() - 30 * 60000]);
    let ejecutados = 0;
    for (const r of rows) {
        try {
...
async function senalesTelefono() {
    ...
    for (const M of rows) {
        try {
            ...
            // CITAS POR UNIVERSO (2026-09-10): el chat del DUEÑO vive en el universo 0 → primero dueno_chat_id (0 lecturas
            // extra); el respaldo por teléfono se acota a tenant 0 para no leer el chat de ese tel en otro universo.
            const cv = M.dueno_chat_id != null ? [{ id: Number(M.dueno_chat_id) }]
                : await query("SELECT id FROM conversaciones WHERE (channel_thread_id IN (?,?,?) OR telefono IN (?,?,?)) AND COALESCE(tenant_id,0)=0 LIMIT 1",
...
async function ensureCanonica() {
    ...
    // CITAS POR UNIVERSO (2026-09-10): la canónica lleva el universo de la cita (0 = cierre del owner); las lecturas
    // (ejecutarCierre, cita-extractor del SB) filtran por él. Mismo DDL en SALES-BRAIN/lib/direccion.js.
    await run("ALTER TABLE cita_canonica ADD COLUMN tenant_id INTEGER").catch(() => {});
    await run("CREATE INDEX IF NOT EXISTS idx_canonica_tenant_created ON cita_canonica(tenant_id, created)").catch(() => {});
}
...
// tenant_id (2026-09-10): universo de la cita; sin dato = 0 (cierre del owner / carril de siempre)
async function registrarCitaCanonica({ telefono, fecha, hora, lugar, auto_id, tenant_id }) {
    const ts = resolverCitaTs(fecha, hora);
    if (!ts) return null;
    await ensureCanonica();
    const { fecha_iso, hora_hhmm } = tsAIsoHora(ts);
    await run("INSERT INTO cita_canonica (telefono, fecha_texto, hora_texto, cita_ts, fecha_iso, hora_hhmm, lugar, auto_id, created, tenant_id) VALUES (?,?,?,?,?,?,?,?,?,?)",
        [String(telefono), String(fecha || ''), String(hora || ''), ts, fecha_iso, hora_hhmm, lugar || null, auto_id || null, Date.now(), Number(tenant_id) || 0]);
...
module.exports = {
    ...
    clasificarVendedor, clasificarCancelacion, clasificarCancelacionDeterminista, MSJ,
    // producción
