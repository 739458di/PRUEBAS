...
async function responderEtapa3Core({ texto, auto_id, conv_id, clasif }) {
    ...
    if (u === 'credito') {
        ...
        if (/(minimo|mínimo|lo menos|cuanto (de |es el )?(minimo|enganche minimo)|enganche minimo|con cuanto (menos|minimo)|desde cuanto|cuanto (es |seria |piden de )?(el )?enganche\b|cual es el enganche|que enganche (piden|manejan|es)|de cuanto (es )?el enganche)/.test(t) && !/\d/.test(t)) {
