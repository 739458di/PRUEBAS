...
async function estadoConv(conv_id) {
    return require('./memo.js').memo('estadoConv:' + conv_id, 20000, () => _estadoConv(conv_id));
}
async function _estadoConv(conv_id) {
    const ms = await query("SELECT direccion, texto, tipo FROM mensajes WHERE conversacion_id=? AND COALESCE(emisor,'') <> 'sistema' ORDER BY ts ASC, id ASC", [conv_id]);   // los renglones grises del sistema no son palabra de Seb
