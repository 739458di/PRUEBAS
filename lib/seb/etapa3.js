...
// ══════════════════════════════════════════════════════════════════════
// CUOTA TURSO (2026-09-08): la conversación completa se lee UNA vez por request (memo
// 'estadoConv:<id>'); responderEtapa3 la pedía hasta 3 veces por turno (estado, gancho,
// gobernador). Los handlers (seb-panel / seb-sandbox) llaman olvidar('estadoConv:') al
// entrar, así un mensaje nuevo SIEMPRE se recalcula de los mensajes (fuente de verdad).
async function estadoConv(conv_id, filas) {   // filas = mensajes ya leídos por quien llama (evita releer todo el chat); se cachean 20 s para el resto del turno
    return require('./memo.js').memo('estadoConv:' + conv_id, 20000, () => _estadoConv(conv_id, filas));
}
async function _estadoConv(conv_id, filas) {
    const ms = Array.isArray(filas) ? filas.filter(m => String(m.emisor || '') !== 'sistema') : await query("SELECT direccion, texto, tipo FROM mensajes WHERE conversacion_id=? AND COALESCE(emisor,'') <> 'sistema' ORDER BY ts ASC, id ASC", [conv_id]);   // los renglones grises del sistema no son palabra de Seb
    ...
    let citas_pedidas = 0;
    outs.forEach(m => {
        if (esGancho(String(m.texto || ''))) citas_pedidas++;   // mismo detector que el gobernador
    });
    ...
    // ¿VISITA VIVA en el motor de citas flexible? (citaf.chat_id = id de la conversación). Con visita viva no se vuelve a empujar a cita.
    let cita_viva = cita_confirmada;
    try { const cv = await query("SELECT 1 x FROM citaf WHERE chat_id = ? AND estado IN ('viva','en_camino','pospuesta') LIMIT 1", [conv_id]); if (cv.length) cita_viva = true; } catch (e) { }
    // burbujas nuestras desde el último gancho (Infinity = nunca hubo): el gancho sale eventualmente, no en cada turno
    let outs_desde_gancho = Infinity; for (let i = outsTx.length - 1; i >= 0; i--) if (esGancho(outsTx[i])) { outs_desde_gancho = outsTx.length - 1 - i; break; }
    return { cita_viva, outs_desde_gancho, ya_cotizado, fotos_enviadas, pin_enviado, citas_pedidas, ultima_pregunta, credito_tocado, proactivo_usado, ultimos_out, gancho_abierto, cita_confirmada };
}
...
function detectarUniverso(t, clasif) {
    ...
    // DISPONIBILIDAD (🚩841): "¿sigue disponible?" es lectura de ficha — lista blanca,
    // también en etapa 3 (el comprador re-pregunta antes/el día de su cita).
    if (require('./regex-comunes.js').RE_DISPONIBLE.test(t)) return 'disponibilidad';
    ...
    if (int === 'disponibilidad') return 'disponibilidad';
...
async function responderEtapa3Core({ texto, auto_id, conv_id, clasif }) {
    ...
    const traeDudaConcreta = /(precio|cuanto|donde|\bdnd\b|foto|video|banco|buro|tasa|mensualidad|enganche|adeudo|choque|factura|kilometr|horario|que dias|a que hora|abren|atienden|estacionamiento|mecanico|cotiz|simulaci|simulam|ejercicio|financia|credito|numeros|corrida|\?)/.test(t);
    ...
    // 🚩919: la PETICIÓN concreta le gana a la señal de interés — "si me late, mándame
    // una SIMULACIÓN" es cotización, no coordinación de cita.
    const preguntaCredito = /(tiene[ns]?|hay|manejan|aceptan|dan|cuentan con|se puede|habra)[^?]{0,28}(financiamiento|financiado|credito|a meses|mensualidades)|financiamiento\s*\?|(^|\s)financian|simulaci|simulam|ejercicio|corrida|(?:cotiza|coitza|cotisa|cotza|cotica)/.test(t);
    ...
    // 🚩 "me PODRÍAS cotizar/mandar los números": un condicional suelto NO es titubeo
    // de cita cuando el mensaje trae una PETICIÓN concreta — la petición manda.
    const esTitubeo = cc === 'titubeo' || preguntaSiPuede || (!cc && esSuaveRe && !traeDudaConcreta);
    ...
    // ── DISPONIBILIDAD (🚩841): lectura de ficha, lista blanca. El dato sale de la BASE
    // (jamás afirmar disponible un auto vendido/pausado — eso escala: la jugada es tuya).
    if (u === 'disponibilidad') {
        ...
        if (!disp) return { escalar: true, motivo: 'pregunta si sigue disponible y el auto ya NO está activo (vendido/pausado) — jugada tuya', puente: 'Déjame lo confirmo y aquí mismo te digo' };
        if (est.cita_confirmada) return { universo: 'disponibilidad', segmentos: [rot(['Sí, sigue disponible 👍 Aquí te lo tenemos para tu cita', 'Sí, claro — ahí te esperamos en tu cita 👍'])] };
        return { universo: 'disponibilidad', segmentos: [rot(['Sí, sigue disponible 👍', 'Sí, aún disponible 👍']), ...fin(ctaEstado(est, 'producto'))] };
    }
    ...
    if (u === 'credito') {
        ...
        if (/(requisito|papeleria|que (documentos|papeles|datos|piden|necesito|me piden)|(datos|documentos|papeles)[^?]{0,25}(enviar|mandar|pasar|llevar|ocupo|necesito))/.test(t)) {
            ...
            return { universo: 'credito', segmentos: [fusion(v.c, 'te los paso:'), REQUISITOS3, est.ya_cotizado ? 'Cuándo te viene bien venir a verlo y manejarlo?' : 'Gustas que te cotice mientras?'] };
        }
        ...
        if (/(tienen|manejan|aceptan|hay|dan|cuentan con|se puede|puedo)[^?]{0,30}(financia|credito|a meses|mensualidades|financiado)/.test(t)) {
            // pregunta DOBLE ("¿cuánto piden y aceptan crédito?"): primero el precio, luego el sí (agentes 2026-09-26: se contestaba solo lo del crédito)
            if (/(precio|cuanto (cuesta|sale|vale|piden|esta|es)|en cuanto (la|lo) )/.test(t)) { const rp = await query('SELECT precio FROM inventario_autos WHERE id=?', [Number(auto_id)]).catch(() => []); if (rp[0] && rp[0].precio != null) return { universo: 'credito', segmentos: ['Son $' + Number(rp[0].precio).toLocaleString('es-MX'), `Y sí, manejamos crédito con ${BANCO}`, ...fin(ctaEstado(est, 'credito'))] }; }
        ...
        const esCotizar = /((?:cotiza|coitza|cotisa|cotza|cotica|cotizc)|simulaci|simulam|ejercicio|corrida|(mandame|hazme|armame|pasame|dame|me (mandas|haces|armas|pasas|das))[^?]{0,10}numeros|(los|unos) numeros\b|como (queda|quedaria)|mensualidad|pago mensual|cuanto (pago|pagaria|seria|me sale|quedaria|es el pago)|al mes|otro plazo|menos enganche|mas enganche|a (48|60|36)|\d+ ?meses|me interesa (a|el|con|por|en) credito|(lo|la) (quiero|llevo|llevaria|agarro|compraria) (a|con) credito|seria (a|con) credito|quiero financiarl|me interesa financiarl|(a|en) credito me interesa)/.test(t) || ((eng || plazo) && /\d/.test(t));
        if (esCotizar) {
    ...
    if (u === 'abierto') {
        return { universo: 'abierto', segmentos: ['Sí atendemos, pero es únicamente con cita previa; es lo que pedimos para tener el auto y el dueño listos y darte toda la seguridad y eficiencia', ...fin(ctaEstado(est, 'logistica'))] };   // una sola pregunta al final
    }
    ...
    if (u === 'ubicacion') {
        ...
        // SUS horarios ≠ los del lote (owner 2026-09-26: "necesito ver con mis horarios" → se le mandó el horario del lote)
        if (/\b(mis? horarios?|mi agenda|mis tiempos|ver con mis|chec\w* (con )?mis|revis\w* (con )?mis|cuadrar mis|acomodar mis)\b/.test(t)) {
            return { universo: 'freno', segmentos: ['Va, sin presión. Cuando tengas tu horario me dices qué día y a qué hora te acomoda y te agendo'] };
        }
        if (/(horario|que dias|a que hora|que hora(s)?|hasta que hora|\babren\b|\bcierran\b|\batienden\b|(abren|atienden|cierran|es) (el|los)? ?(sabado|domingo|fin de semana))/.test(t)) {
            const hReal = await require('./continuacion.js').horarioDelUniverso();   // horario REAL del universo (tenants.config_json.horario); el texto fijo solo para el número principal
            return { universo: 'horario', segmentos: [hReal || 'De 9am a 7pm, pero como es con cita es flexible al horario que te acomode, incluso sábado o domingo sin problema', ...fin(ctaEstado(est, 'logistica'))] };
        }
