...
function detectarUniverso(t, clasif) {
    ...
    if (/((mas|toda) (la )?informacion|informacion (completa|del (auto|carro|coche))|manda(r|s|rme)?(me)? (mas )?(la )?informacion|pasa(me|r)? (la )?informacion|puedes mandar (mas )?informacion|la ficha( tecnica| completa)?\s*\??$)/.test(t)) return 'info_general';
