...
async function _estadoConv(conv_id, filas) {
    ...
    try { const cv = await query("SELECT 1 x FROM citaf WHERE chat_id = ? AND estado IN ('viva','en_camino') LIMIT 1", [conv_id]); if (cv.length) cita_viva = true; } catch (e) { }
...
function detectarUniverso(t, clasif) {
    ...
    if (/(sigue disponible|aun disponible|todavia disponible|esta disponible|disponible (todavia|aun|verdad|\?)|todavia (lo|la) (tienes|tienen|venden|esta)|sigue (en|a la) venta|no (lo|la) han vendido|ya (lo|la) vendieron|ya se vendio|lo tienen todavia)/.test(t)) return 'disponibilidad';
