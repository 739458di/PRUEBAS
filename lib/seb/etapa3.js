...
function detectarUniverso(t, clasif) {
    ...
    if (/((mas|toda) (la )?informacion|informacion (completa|del (auto|carro|coche))|manda(r|s|rme)?(me)? (mas )?(la )?informacion|pasa(me|r)? (la )?informacion|puedes mandar (mas )?informacion|la ficha( tecnica| completa)?\s*\??$|(me puedes (dar|pasar|mandar)|dame|pasame|me das|quiero) (mas )?detalles|mas detalles de(l| la| el)|que mas me (puedes (decir|contar)|cuentas) del)/.test(t)) return 'info_general';
...
async function responderEtapa3Core({ texto, auto_id, conv_id, clasif }) {
    ...
    if (u === 'info_general') {
        ...
        return { universo: 'info_general', segmentos: [fusion(v.c, 'te la mando:'), m, ...fin(ctaEstado(est, 'producto'))] };
    }
