...
async function responderEtapa3Core({ texto, auto_id, conv_id, clasif }) {
    ...
    if (u === 'credito') {
        ...
        // ── DOCTRINA (2026-07-09): en crédito el bot solo dice HECHOS, manda LA LISTA
        // y calcula LA TARJETA. Todo lo que explique el PROCESO o empuje la decisión
        // (buró, tasa, aprobación, "cómo funciona") es VENTA → escala con puente. ──
        if (/(buro|historial)/.test(t)) return { escalar: true, motivo: 'pregunta BURÓ (decisión de crédito — tuya)', puente: 'Déjame te confirmo bien ese punto del crédito y aquí mismo te digo' };
        if (/(que|cual|de cuanto|cuanto (es|manejan|cobran)|a que|k) ?(es )?(la )?tasa|tasa de interes|que tasa|porcentaje (de interes|anual)|cuanto de interes/.test(t)) {
            return { escalar: true, motivo: 'pregunta la TASA (política de crédito — tuya)', puente: 'Déjame te confirmo bien ese punto del crédito y aquí mismo te digo' };
        }
        // 📤 REQUISITOS = mandar archivo (la lista es la lista, sin CTA de venta).
        if (/(requisito|papeleria|que (documentos|papeles|datos|piden|necesito|me piden)|(datos|documentos|papeles)[^?]{0,25}(enviar|mandar|pasar|llevar|ocupo|necesito))/.test(t)) {
            return { universo: 'credito', segmentos: ['Te los paso:', REQUISITOS3] };
        }
        // 📖 MÍNIMO DE ENGANCHE = número de las reglas (sin el pitch del procesito).
        if (/(minimo|mínimo|lo menos|cuanto (de |es el )?(minimo|enganche minimo)|enganche minimo|con cuanto (menos|minimo)|desde cuanto)/.test(t) && !/\d/.test(t)) {
            ...
            if (r.ok) return { universo: 'credito', segmentos: [`El enganche mínimo para este auto es de ${r.placeholders.enganche_minimo}`] };
        }
        // 📖 QUÉ BANCO = hecho seco.
        if (/(que banco|cual banco|con que banco)/.test(t)) return { universo: 'credito', segmentos: [`Es con ${BANCO}`] };
        if (/apr(o|ue)b/.test(t)) return { escalar: true, motivo: 'pregunta la APROBACIÓN del crédito (proceso de cierre — tuyo)', puente: 'Déjame te confirmo bien ese punto del crédito y aquí mismo te digo' };
        // 📖 "¿Financian?" (sí/no) = hecho: SÍ + banco. El CÓMO del proceso ya es venta.
        if (/(tienen|manejan|aceptan|hay|dan|cuentan con|se puede|puedo)[^?]{0,30}(financia|credito|a meses|mensualidades|financiado)/.test(t)) {
            return { universo: 'credito', segmentos: [rot(['Sí, así es', 'Claro que sí']), `Es con ${BANCO}`] };
        }
        ...
        if (esCotizar) {
            ...
            return { universo: 'cotizacion', segmentos: [rot(['Va, así queda:', 'Claro, mira:', 'Sale, te la ajusto:']), card] };
        }
        // Duda de crédito genérica ("¿cómo funciona el crédito?") = PROCESO → tuya.
        return { escalar: true, motivo: 'duda del PROCESO de crédito (explicarlo es venta — tuya)', puente: 'Déjame te explico bien cómo quedaría tu crédito y en un momento te escribo' };
    }
...
async function responderEtapa3(args) {
    ...
    // riesgo, 🔥 comprador caliente, 💰 proceso de pago — NI SE PROCESAN: puente + owner.
    const gol = puertaEntrada(args && args.texto);
    if (gol) return gol;
    let r = await responderEtapa3Core(args);
    // PUERTA DE SALIDA (doctrina): universos de política/persuasión → escala con puente.
    r = puertaSalida(r);
