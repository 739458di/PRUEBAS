...
async function responderEtapa3(args) {
    ...
    // 📝#10 (owner): IMPOSIBLE agendar sin mandar LA UBICACIÓN — toda cita confirmada
    // lleva el pin del punto (si no se le había mandado ya en la conversación).
    if (g && g.cita_confirmada && args && args.auto_id && g.ubicacion_auto_id == null && !(est && est.pin_enviado)) {
        g.ubicacion_auto_id = args.auto_id;
        g.pin_after_index = 0;   // el pin cae justo después del "Listo, entonces…"
    }
    return g;
}
