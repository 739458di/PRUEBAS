...
async function responderEtapa3Core({ texto, auto_id, conv_id, clasif }) {
    ...
    const esAfirmacion = /^(si+|sii+|claro( que si)?|si por ?favor|por ?favor|va+|vale|sale( va)?|dale|andale|simon|ok+|okey|de acuerdo|correcto|exacto|asi es|esta bien|me parece( bien)?|perfecto|adelante|si gracias|hazlo|mandalo|mandala|mandamelo|mandamela|echale|arre)[\s.,!]*$/.test(t);
