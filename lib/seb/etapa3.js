...
const { datosPunto } = require('./continuacion.js');
...
function detectarUniverso(t, clasif) {
    ...
    if (/(ubicacion|\b(a ?)?d?onde\b|\bdnd\b|se encuentran|se ubican|direccion|como llego|de donde (eres|son)|hacen envio|envian|horario|que dias|a que hora|que hora(s)?|hasta que hora|\babren\b|\bcierran\b|\batienden\b|(hay|atienden|abren|cierran)[^?]{0,12}(sabado|domingo|fin de semana))/.test(t)) return 'ubicacion';
    if (/(cotiza|corrida|como (queda|quedaria)|mensualidad|de enganche|\d+ ?meses|a (48|60|36)|enganche de|otro plazo|menos enganche|mas enganche|requisito|papeleria|que (datos|documentos|papeles)|que me piden|buro|historial|apr(o|ue)b|tasa\b|que banco|financia|financiado|\bcredito\b|a meses)/.test(t)) return 'credito';
...
async function responderEtapa3Core({ texto, auto_id, conv_id, clasif }) {
    ...
    // ── MULTI-PREGUNTA: dos dudas en un mensaje → se contestan LAS DOS, dinero primero,
    // cita/acción después (va ANTES de escala3 para que el miedo no gane si hay un pedido
    // concreto que lo disuelve — p.ej. "no sé si me alcance, cuánto con 60 mil?"). ──
    {
        const engPago = /(cuando se paga|cuando (doy|pago) el enganche|el enganche (cuando|se paga|donde|es al|va al))/.test(t);
        const miedoAlcance = /(no se si (me alcance|pueda|calific)|no me alcanza|no creo (poder|calificar))/.test(t);
        const pideMensualidad = /(mensualidad|cuanto (seria|pago|pagaria|me sale|quedaria)|al mes|el pago)/.test(t);
        const engTxt = d.enganche || (String(t).match(/(\d+)\s*mil/) ? Number(RegExp.$1) * 1000 : null);
        const citaSig = /(puedo (ir|pasar|verl)|voy (a )?(ir|verl)|nos vemos|el (sabado|domingo|lunes|martes|miercoles|jueves|viernes)|manana|\bhoy\b|a las \d)/.test(t);
        // A) proceso del enganche + intención de cita
        if (engPago && citaSig && auto_id && conv_id) {
            const dinero = 'El enganche lo das hasta la entrega; el día de la cita no llevas nada';
            const fM = d.fecha || sacaFecha(t);
            let hM = d.hora || sacaHora(t); if (hM && /(antes|despues|luego) de|por la/.test(norm(String(hM)))) hM = null;
            if (fM && hM) { const dp = await datosPunto(auto_id).catch(() => null); const cc = citaConfirmadaSeg(fM, hM, dp && dp.dir ? dp.dir : null); return { ...cc, segmentos: [dinero, ...cc.segmentos] }; }
            return { universo: 'multi', segmentos: [dinero, `Y va que va${fM ? ', ' + fM : ''}, a qué hora te queda para dejarlo en firme?`] };
        }
        // B) miedo al alcance + pide mensualidad con enganche → el NÚMERO disuelve el miedo
        if (miedoAlcance && pideMensualidad && engTxt && auto_id) {
            const r = await cotizar({ auto_id, enganche: engTxt });
            if (r.ok) return { universo: 'cotizacion', segmentos: ['Tranquilo, muchos aprueban aunque no lo crean; mira cómo quedaría:', r.placeholders.cotizacion, 'Cómo la ves?'] };
        }
    }
    ...
    const esAfirmacion = /^(si+|sii+|claro( que si)?|si por ?favor|por ?favor|va+|vale|sale( va)?|dale|andale|simon|ok+|okey|de acuerdo|esta bien|me parece( bien)?|perfecto|adelante|si gracias|hazlo|mandalo|mandala|mandamelo|mandamela|echale|arre)[\s.,!]*$/.test(t);
    ...
    const traeDudaConcreta = /(precio|cuanto|donde|\bdnd\b|foto|video|banco|buro|tasa|mensualidad|enganche|adeudo|choque|factura|kilometr|horario|que dias|a que hora|abren|atienden|estacionamiento|mecanico|\?)/.test(t);
    if (esInteres && !traeDudaConcreta && auto_id && conv_id) {
    ...
    if (/^(ok+(ey)?|okok|va+(le)?|sale|dale|listo|perfecto|excelente|de acuerdo|entendido|muy amable|gracias|muchas gracias|mil gracias|grax|thx|si gracias|ok gracias|va gracias|👍|🙏|❤️)([\s.,!]*(ok+|va+|gracias|listo|perfecto|👍|🙏))*[\s.,!]*$/.test(t)) {
        ...
        if (conv_id) {
            ...
            const citaPend = lo.some(m => m.direccion === 'out' && /(a que hora|que dia|hora te queda|dia te queda|te agendo|agendarte|prueba de manejo|te coordino)/.test(norm(m.texto)));
            ...
            if (citaPend && !yaConf) {
                ...
                if (dia) { const art = /^(hoy|manana|mañana|pasado)/i.test(dia) ? '' : 'el '; return { universo: 'cita', segmentos: [`Va, entonces a qué hora nos vemos ${art}${dia}?`] }; }
    ...
    const esPromesa = /(te aviso|les aviso|le aviso|me comunico|te confirmo|les confirmo|lo platico|platicarlo|lo consulto|deja(me)? chec|lo checo|voy a ver si|lo pienso|dame (chance|oportunidad|tiempo)|aguantame|esperame|deja (programar|cuadrar|acomodar|organizar)|me programo|programar(me)? (la|mi) (ida|visita|vuelta|salida)|checo mi (agenda|semana)|dejame (programar|organizarme|acomodarme))/.test(t);
    ...
    const preguntaDonde = /(\b(a ?)?d?onde\b|\bdnd\b|ubicacion|direccion|en que parte|por que (zona|rumbo)|en donde)/.test(t);
    ...
    const preguntaSiPuede = /\bpuedo (pasar|ir|llegar|acudir|verl|darme (la|una) vuelta|caer|asistir|conocer)/.test(t);
    const esTitubeo = cc === 'titubeo' || preguntaSiPuede || (!cc && esSuaveRe);
    const esFirme = cc === 'firme';
    const hayObjecion = cc === 'objecion' || !!obj;
    const fecha = d.fecha || sacaFecha(t);
    ...
    if (contextoCita && auto_id) {
        ...
        // ── MARCHA 2: OBJECIÓN → BAJAR A REALIDAD (poner de nuestra parte y proponer) ──
        if (hayObjecion) {
            if (obj === 'distancia' || esForaneoObjecion(t)) {
                return { universo: 'cita', segmentos: ['No hay bronca por la distancia', 'Somos de Monterrey y manejamos envío a todo el país: mandas a un mecánico o conocido tuyo aquí en Mty a que la revise y te dé luz verde, y te la enviamos con garantía de viaje', 'Te late que la revise alguien de tu confianza?'] };
            }
            if (obj === 'trabajo') return { universo: 'cita', segmentos: ['Sin tema, por el trabajo no te apures', 'Abrimos sábado y domingo también, y te esperamos a la hora que salgas; la prueba de manejo es rápida', 'Qué te acomoda mejor, el sábado o el domingo?'] };
            if (obj === 'dinero') {
                // "El enganche a la entrega" SOLO aplica a CRÉDITO. De contado se maneja distinto.
                const e = await estadoConv(conv_id);
                if (e.credito_tocado) return { universo: 'cita', segmentos: ['Tranquilo, el enganche lo das hasta la ENTREGA, no necesitas tenerlo ya', 'Ven a verlo y manejarlo sin compromiso primero, y ya con el auto apartado juntas el enganche con calma', 'Te late venir mañana o el sábado?'] };
                return { universo: 'cita', segmentos: ['Para verlo y manejarlo no necesitas traer nada, ven sin compromiso', 'Y si te late, lo apartamos con un depósito chico mientras juntas el resto, así no se te va', 'Te late venir mañana o el sábado?'] };
            }
            if (obj === 'tercero') return { universo: 'cita', segmentos: ['Claro, mejor lo ven juntos', 'Si gustan pueden venir ambos a la cita y la prueba de manejo, lo checan ahí mismo; o si prefieres te paso un resumen para que lo revisen', 'Qué día les acomoda, entre semana o el fin?'] };
            return { universo: 'cita', segmentos: [acc, 'Cuéntame qué te detiene y lo resolvemos', 'Qué día te late que lo veas, mañana o el sábado?'] };
        }
        ...
        if (fecha && !hora) return { universo: 'cita', segmentos: [`${acc}, ${fecha} entonces`, 'Como a qué hora te queda? Para agendarte en firme'] };
        if (hora && !fecha) {
            let fechaCtx = null;
            if (conv_id) {
            ...
            if (fechaCtx && esTitubeo) return { universo: 'cita_firme', segmentos: [acc, `Entonces te agendo en firme: ${cap(fechaCtx)} a las ${hora}${lugar ? ' en ' + lugar : ''}, va?`] };
        ...
        if (esTitubeo || (!esFirme && cc && cc !== 'firme')) {
            ...
            return { universo: 'cita_firme', segmentos: [acc, `Entonces te agendo en firme: ${cuando}${lugar ? ' en ' + lugar : ''}, va?`] };
        }
    ...
    if (u === 'credito') {
        ...
        // Gancho de crédito con FALLBACK: si el CTA normal se repetiría (freno de
        // "nunca repetir la última pregunta"), igual damos un gancho (🚩 buró sin gancho).
        const gCred = ctaEstado(est, 'credito') || (est.ya_cotizado ? 'Cómo ves los números?' : rot(['Gustas que te cotice para que veas los números?', 'Te late venir a verlo y de una vez dejamos el crédito aprobado?']));
        // Las PREGUNTAS específicas van PRIMERO: Haiku arrastra el enganche de la
        // cotización pasada en sus datos ("pegajoso"), y si "¿hay enganche? → cotiza"
        // se evalúa antes, un "checan buró?" re-manda la MISMA tarjeta (visto en sandbox).
        if (/(buro|historial)/.test(t)) return { universo: 'credito', segmentos: [`Es sujeto a aprobación con ${BANCO}; estés en buró o no lo vemos, y con buen historial mejora`, gCred] };
        // TASA (duda directa "¿qué tasa manejan?") — se contesta el número (HEY); en Renueva Car NO se dice la tasa.
        if (/(que|cual|de cuanto|cuanto (es|manejan|cobran)|a que|k) ?(es )?(la )?tasa|tasa de interes|que tasa|porcentaje (de interes|anual)|cuanto de interes/.test(t)) {
            if (BANCO === 'HEY Banco') return { universo: 'credito', segmentos: ['La tasa ronda el 13.99% anual con HEY Banco, y mejora entre más enganche des', gCred] };
            return { universo: 'credito', segmentos: ['La tasa te la afino en tu cotización exacta, según tu enganche y plazo', gCred] };
        }
        if (/(requisito|papeleria|que (documentos|papeles|datos|piden|necesito|me piden)|(datos|documentos|papeles)[^?]{0,25}(enviar|mandar|pasar|llevar|ocupo|necesito))/.test(t)) {
            return { universo: 'credito', segmentos: ['Te los paso:', REQUISITOS3, est.ya_cotizado ? 'En cuanto los tengas me los mandas y arrancamos' : 'Gustas que te cotice mientras?'] };
        }
        // MÍNIMO DE ENGANCHE — se dice el número + el procesito (owner: "se les dice pero con el procesito").
        if (/(minimo|mínimo|lo menos|cuanto (de |es el )?(minimo|enganche minimo)|enganche minimo|con cuanto (menos|minimo)|desde cuanto)/.test(t) && !/\d/.test(t)) {
            const r = await enganche_minimo({ auto_id });
            if (r.ok) return { universo: 'credito', segmentos: [`El enganche mínimo para este auto es de ${r.placeholders.enganche_minimo}`, `De ahí para arriba el que gustes; mandas tus documentos, te cotizo con ${BANCO} y en menos de 2 horas te dicen si apruebas`, ...fin(ctaEstado(est, 'credito'))] };
        }
        if (/(que banco|cual banco|con que banco)/.test(t)) return { universo: 'credito', segmentos: [`Es con ${BANCO}, con muy buenas condiciones`, ...fin(ctaEstado(est, 'credito'))] };
        ...
        // Pregunta SÍ/NO ("¿tienen financiamiento?") → AFIRMACIÓN FUNDIDA primero
        // (🚩 training: contestaba el proceso sin decir nunca el "sí").
        if (/(tienen|manejan|aceptan|hay|dan|cuentan con|se puede|puedo)[^?]{0,30}(financia|credito|a meses|mensualidades|financiado)/.test(t)) {
            return { universo: 'credito', segmentos: [rot(['Sí, así es', 'Claro que sí', 'Sisi, claro']), `Tenemos financiamiento con ${BANCO}; mandas tus documentos y en menos de 2 horas te dicen si apruebas`, ...fin(ctaEstado(est, 'credito'))] };
        }
        ...
        // Cotizar: por fraseo explícito, o por datos de Haiku SOLO si el mensaje
        // actual trae números (si no, es dato arrastrado de turnos viejos, no un pedido).
        const eng = d.enganche || null, plazo = d.plazo_meses || null;
        const esCotizar = /(cotiza|corrida|como (queda|quedaria)|mensualidad|pago mensual|cuanto (pago|pagaria|seria|me sale|quedaria|es el pago)|al mes|otro plazo|menos enganche|mas enganche|a (48|60|36)|\d+ ?meses)/.test(t) || ((eng || plazo) && /\d/.test(t));
        if (esCotizar) {
            ...
            if (!card) return { escalar: true, motivo: MOTIVO_COTIZA(errCot), puente: 'Déjame te armo la cotización exacta y en un momento te la paso' };
            // REMATE por contexto (🚩 training): si el comprador ya mostró AGRADO
            // ("me agrada 50% a 36m") o la última pregunta ya fue "Cómo la ves?",
            // no se repite — se empuja a la CITA de manejo (freno: 2+ citas pedidas → sin pregunta).
            ...
            if (agrado || yaPregunto) ctaCot = est.citas_pedidas >= 2 ? '' : rot([
                ...
                'Te queda bien venir a manejarlo? Aquí mismo te explicamos el proceso del crédito'
            ]);
            return { universo: 'cotizacion', segmentos: [rot(['Va, así queda:', 'Claro, mira:', 'Sale, te la ajusto:']), card, ...fin(ctaCot)] };
        }
        return { universo: 'credito', segmentos: [`Es por medio de ${BANCO}: mandas tus documentos, te cotizo, solicitamos y te dicen en menos de 2 horas; el enganche lo das hasta la entrega`, ...fin(ctaEstado(est, 'credito'))] };
    }
    ...
    // ── PRECIO directo — dato del inventario, registro SECO (turno limpio, #591):
    // se contesta el número; si lleva gancho o no lo decide el gobernador (termostato).
    if (u === 'precio') {
        const rp = await query("SELECT precio FROM inventario_autos WHERE id=?", [Number(auto_id)]);
        if (!rp.length || rp[0].precio == null) return { escalar: true, motivo: 'pregunta el precio y el auto no tiene precio en inventario', puente: 'Dame un momento y te confirmo el precio exacto' };
        return { universo: 'precio', segmentos: ['Son $' + Number(rp[0].precio).toLocaleString('es-MX'), ...fin(ctaEstado(est, 'producto'))] };
    }
    ...
    // ── FORÁNEO COMO OBJECIÓN — "eso no nos frena": envío + persona de confianza que da luz verde ──
    if (u === 'foraneo') {
        const c = est.citas_pedidas < 2 ? 'Te late que la revise alguien de tu confianza?' : '';
        return {
            universo: 'foraneo', segmentos: [
                ...
                'Y si gustas la apartamos desde antes para que no se te vaya',
                ...fin(c)
            ]
        };
    }
    ...
    if (u === 'ubicacion') {
        ...
        const seg1 = dir ? `Aquí es el punto, ${dir}` : 'Aquí es nuestro punto de venta';
        return { universo: 'ubicacion', segmentos: [seg1, ...fin(ctaEstado(est, 'logistica'))], ubicacion_auto_id: auto_id, pin_primero: true };
    }
...
async function responderEtapa3(args) {
    const r = await responderEtapa3Core(args);
    if (!r || r.escalar || r.silencio || !r.segmentos || !r.segmentos.length) return r;
    let est = null;
    if (args && args.conv_id) { try { est = await estadoConv(args.conv_id); } catch (e) { /* sin estado → gobierna solo dentro de la ráfaga */ } }
    return gobernar(r, { texto: args && args.texto, est });
}
