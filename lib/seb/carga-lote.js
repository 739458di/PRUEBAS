...
async function ordenarFotos(fotos) {
    ...
    try {
            ...
            try {
                const r = await fetch('https://api.anthropic.com/v1/messages', {
                    ...
                    body: JSON.stringify({
                        ...
                        system: 'Clasificas fotos de autos usados para ordenar una publicación de venta. Responde SOLO el JSON.',
                        messages: [{ role: 'user', content: [
                            { type: 'image', source: { type: 'url', url } },
                            { type: 'text', text: 'tipo: exterior_diagonal (toma 3/4 DIAGONAL: se ve el frente Y un costado del auto completo — LA toma de portada) | exterior_frontal (frente directo del auto completo) | exterior (lateral puro/trasera/otro exterior) | interior (cabina/asientos/tablero) | detalle (motor/rin/odómetro/pantalla/acercamiento). portada: 1-10, qué tan buena PORTADA sería (auto completo, bien iluminado, fondo limpio, sin recortes).' }
                        ] }],
                        output_config: { format: { type: 'json_schema', schema: { type: 'object', properties: { tipo: { type: 'string', enum: ['exterior_diagonal', 'exterior_frontal', 'exterior', 'interior', 'detalle'] }, portada: { type: 'integer' } }, required: ['tipo', 'portada'], additionalProperties: false } } }
                    })
                });
                if (!r.ok) return { url, tipo: 'exterior', portada: 5 };
                const d = await r.json();
                const tb = (d.content || []).find(b => b.type === 'text');
                const j = JSON.parse(tb.text);
                return { url, tipo: j.tipo, portada: Number(j.portada) || 5 };
            } catch (e) { return { url, tipo: 'exterior', portada: 5 }; }
        }));
        ...
        const prio = { exterior_diagonal: 0, exterior_frontal: 1, exterior: 2, interior: 3, detalle: 4 };
        clasif.sort((a, b) => (prio[a.tipo] - prio[b.tipo]) || (b.portada - a.portada));
        let bestIdx = 0, best = -Infinity;
        clasif.forEach((c, i) => {
            const score = c.portada + (c.tipo === 'exterior_diagonal' ? 6 : c.tipo === 'exterior_frontal' ? 2 : c.tipo === 'exterior' ? 1 : -8);
...
async function publicarCarga(c) {
    ...
    try {
        ...
        if (okPub) {
            ...
            await enviarWA(OWNER_CARGA, `✅ Publicado: ${f.marca} ${modelo} ${f.anio} — $${Number(f.precio).toLocaleString('en-US')} — ${fotos.length} fotos${tpl ? ' — diseño verificada ✓' : ' — ⚠️ diseño pendiente'} (${c.code || CODE_DEFAULT})`);
            return true;
        }
