...
async function ordenarFotos(fotos) {
    ...
    try {
                ...
                const r = await fetch('https://api.anthropic.com/v1/messages', {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [{ role: 'user', content: [
                            ...
                            { type: 'text', text: 'tipo: exterior_frontal (frente o 3/4 frontal del auto completo) | exterior (lateral/trasera/otro exterior) | interior (cabina/asientos/tablero) | detalle (motor/rin/odómetro/pantalla/acercamiento). portada: 1-10, qué tan buena PORTADA sería (auto completo, bien iluminado, fondo limpio, sin recortes).' }
                        ] }],
                        output_config: { format: { type: 'json_schema', schema: { type: 'object', properties: { tipo: { type: 'string', enum: ['exterior_frontal', 'exterior', 'interior', 'detalle'] }, portada: { type: 'integer' } }, required: ['tipo', 'portada'], additionalProperties: false } } }
                    })
                });
        ...
        const prio = { exterior_frontal: 0, exterior: 1, interior: 2, detalle: 3 };
        clasif.sort((a, b) => (prio[a.tipo] - prio[b.tipo]) || (b.portada - a.portada));
        let bestIdx = 0, best = -Infinity;
        clasif.forEach((c, i) => {
            const score = c.portada + (c.tipo === 'exterior_frontal' ? 3 : c.tipo === 'exterior' ? 1 : -8);
