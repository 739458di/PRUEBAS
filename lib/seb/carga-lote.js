...
function armarBody(c, f, fotos) {
    ...
    if (dest.tipo === 'particular') {
        const dtel = String(f.dueno_telefono || '').replace(/\D/g, '');
        const partes = String(f.dueno_nombre || '').trim().split(/\s+/).filter(Boolean);
        return {
    ...
    try {
        ...
        if (okPub) {
            ...
            const etiqueta = arm.destino === 'particular'
                ...
                : arm.destino;
            await enviarWA(c.remitente || OWNER_CARGA, `✅ Publicado: ${f.marca} ${modelo} ${f.anio} — $${Number(f.precio).toLocaleString('en-US')} — ${fotos.length} fotos${tpl ? ' — diseño verificada ✓' : ' — ⚠️ diseño pendiente'} (${etiqueta})`);
...
async function pieza({ remitente, tipo, texto, url }) {
    ...
    // ══ "PUBLÍCALO" (solo el OWNER): publica el auto de Ignacio-recepción que esté
    // EN REVISIÓN (el más reciente) — el "va" del human in the loop.
    if (tipo === 'texto' && tel === OWNER_CARGA && /^publ[ií]ca(lo|r|la)?[!. ]*$/.test(tN)) {
        try {
            const recepcion = require('./recepcion.js');
            const r = await recepcion.publicarPendiente();
            if (r.ok) {
                const a = r.auto || {};
                ...
                if (r.sesion && r.sesion.telefono) await enviarWA(r.sesion.telefono, `¡Listo! Tu auto ya quedó publicado en fyradrive.com ✅ Cualquier interesado nosotros te coordinamos las citas por aquí. 🤝`);
            } else {
...
module.exports = { pieza, barrerCargas, publicarCarga, parseFicha, ensureCarga, ordenarFotos, destinoDe, armarBody, faltanDe };
