...
async function publicarCarga(c) {
    ...
    try {
        ...
        if (okPub) {
            ...
            await enviarWA(c.remitente || OWNER_CARGA, `✅ Publicado: ${f.marca} ${modelo} ${f.anio} — $${Number(f.precio).toLocaleString('en-US')} — ${fotos.length} fotos${tpl ? ' — diseño verificada ✓' : ' — ⚠️ diseño pendiente'} (${c.code || CODE_DEFAULT})`);
...
async function pieza({ remitente, tipo, texto, url }) {
    ...
    if (tipo === 'texto') {
        ...
        if (/^carga (lozano|lote|autos)/.test(tN)) {
            await enviarWA(tel, `🚗 Listo — mándame las FOTOS de cada auto y al final su TEXTO con la ficha (marca, modelo, año, precio). El texto publica el auto.`);
...
module.exports = { pieza, barrerCargas, publicarCarga, parseFicha, ensureCarga, ordenarFotos };
