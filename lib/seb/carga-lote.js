...
function destinoDe(c) {
    ...
    // OJO (caso David E. LOZANO Porte 2026-08-05): "lozano" a secas dentro de una
    // ficha puede ser el APELLIDO del dueño particular. El atajo a AUTOS LOZANO
    // solo vale con "marcelo"/"autos lozano" explícito, o como mención CORTA del
    // owner ("lozano", "es de lozano") — jamás por un apellido enterrado en la ficha.
    const t = String(c.texto || '');
    if (/marcelo|autos\s+lozano/i.test(t)) return { tipo: 'agencia', code: 'AUTOS LOZANO' };
    if (/lozano/i.test(t) && t.trim().length <= 20) return { tipo: 'agencia', code: 'AUTOS LOZANO' };
...
async function armarBody(c, f, fotos) {
    ...
    if (dest.tipo === 'particular') {
        ...
        return {
            ...
            body: {
                tipo: 'particular', key: process.env.KEY_VIEJA || undefined,   // transición (body); la llave real va por header K_PANEL en publicar()
...
// inventario_autos (Sales Brain) NO es confiable como espejo: el sync
// register_new_auto puede llegar TARDE (caso GLE 2026-07-18: pisó la
// reacreditación 31s después) o NO llegar (Mazda, Mini). La verdad es autos →
// upsert directo aquí, y el sanador del barredor re-cura cualquier pisada.
async function upsertInventarioAcred(autoId, campos) {
    const { tipo, agencia, nombre, tel } = campos;
    const r = await run("UPDATE inventario_autos SET tipo_vendedor=?, agencia_nombre=?, dueno_nombre=?, dueno_telefono=?, synced_at=? WHERE fyradrive_web_id=?", [tipo, agencia, nombre, tel, Date.now(), autoId]).catch(() => null);
    if (r && Number(r.rowsAffected) > 0) return;
    const rows = await query('SELECT marca, modelo, año AS anio, precio, kilometraje FROM autos WHERE id=?', [autoId]);
    if (!rows.length) return;
    const a = rows[0]; const now = Date.now();
    await run("INSERT INTO inventario_autos (fyradrive_web_id, marca, modelo, anio, precio, kilometraje, estado, sb_convs_count, synced_at, created_at, dueno_nombre, dueno_telefono, vendido_externo, vendido_fyradrive, ads_activo, agencia_nombre, tipo_vendedor, dueno_urgency_level, pricing_paused, proactividad, publicado_ig, publicado_fb, publicado_mp) VALUES (?,?,?,?,?,?,'activo',0,?,?,?,?,0,0,0,?,?,2,0,1,0,0,0)", [autoId, a.marca, a.modelo, a.anio, a.precio, a.kilometraje, now, now, nombre, tel, agencia, tipo]).catch(e => console.error('[upsertInv]', autoId, e.message));
}

async function rehornearPortada(autoId) {
    const im = await query('SELECT id FROM imagenes_autos WHERE auto_id=? AND es_principal=1', [autoId]);
    if (!im.length) return false;
    // acotada a 60s (el horno tarda ~40-80s): si se cuelga NO puede matar la
    // función entera (caso Mini 2026-07-18: murió aquí y ni el recibo salió).
    // Un timeout aquí no cancela el horno del lado del servidor — la portada
    // suele aterrizar sola un momento después.
    const resp = await fetch(WEB + '/api/generate-template', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ autoId, imgId: im[0].id, zoom: 1.0 }), signal: AbortSignal.timeout(60000) });
    return resp.ok;
}
...
async function reacreditar(autoId, dest) {
    ...
    if (dest.tipo === 'agencia') {
        ...
        await upsertInventarioAcred(autoId, { tipo: 'agencia', agencia: dest.code, nombre: 'Marcelo Lozano', tel: '5218129405001' });
        ...
        await upsertInventarioAcred(autoId, { tipo: 'particular', agencia: null, nombre: String(dest.nombre).trim(), tel });
    ...
    await require('./catalogo-web.js').proyectarWeb([autoId]);   // vendedor/UID → card
    let portadaOk = false;
    try { portadaOk = await rehornearPortada(autoId); } catch (e) { console.error('[reacreditar template]', e.message); }
    return { ok: true, nombre: [a.marca, a.modelo, a.anio].filter(Boolean).join(' '), portadaOk };
}
...
function interpretarAcreditacion(tN) {
    ...
    // 1º LA FORMA EXPLÍCITA GANA (caso David E. LOZANO Porte 2026-08-05): "es de
    // <nombre> <tel>" se evalúa ANTES del atajo — un apellido Lozano en el nombre
    // jamás debe secuestrar la acreditación hacia AUTOS LOZANO.
    const m = t.match(/^(?:es de|a nombre de|acredita(?:lo)?(?: a)?|dueno:?|dueño:?)\s+([a-zñáéíóú][a-zñáéíóú .]{2,45}?)(?:\s+(\d[\d ]{9,14}))?$/i);
    if (m) {
        const nombre = m[1].trim();
        const tel = m[2] ? m[2].replace(/\D/g, '') : null;
        // "es de autos lozano" / "es de marcelo" siguen siendo la agencia
        if (/^(autos\s+lozano|marcelo(\s+lozano)?|lozano)$/i.test(nombre)) return { tipo: 'agencia', code: 'AUTOS LOZANO' };
        if (tel && tel.length >= 10) return { tipo: 'particular', nombre, tel };
        return { tipo: 'nombre_solo', nombre };   // se resuelve contra dueños conocidos
    }
    // 2º el atajo corto: "lozano"/"marcelo"/"autos lozano" como MENCIÓN, no como apellido
    if (/(marcelo|autos\s+lozano)/i.test(t) && t.length <= 80 && !/\$/.test(t)) return { tipo: 'agencia', code: 'AUTOS LOZANO' };
    if (/lozano/i.test(t) && t.length <= 20 && !/\$/.test(t)) return { tipo: 'agencia', code: 'AUTOS LOZANO' };
    return null;
}
...
async function publicarCarga(c) {
    ...
    try {
        ...
        // K_PANEL por header (la web la verifica); transición: body.key = KEY_VIEJA (armarBody). Sin literales; sin K_PANEL → falla cerrada.
            ...
            if (acred) {
                ...
                if (autoId) {
                    ...
                    await enviarWA(tel, r.ok
                        ? `🔁 Reacreditado: ${r.nombre} → ${dest.tipo === 'agencia' ? dest.code + ' (agencias verificadas)' : 'PARTICULAR — ' + dest.nombre + ' (' + String(dest.tel).slice(-10) + ')'} ${r.portadaOk ? '✓ portada re-horneada' : '⏳ portada en cola (el barredor la re-hornea)'}`
...
async function barrerCargas() {
    ...
    try { n += await sanarInventario(); } catch (e) { console.error('[sanarInventario]', e.message); }
    return n;
}

// ── SANADOR (ley del timbre; casos GLE/Mini 2026-07-18) ──
// El sync register_new_auto de Sales Brain puede PISAR una reacreditación (llegó
// 31s tarde en el GLE) o simplemente no llegar (Mazda y Mini sin fila). La verdad
// es la tabla autos (web) → el barredor compara los autos activos recientes contra
// inventario_autos y cura tipo/agencia/dueño; al curar re-hornea la portada por si
// quedó en la variante equivocada. Sin mensajes: sanado silencioso e idempotente.
async function sanarInventario() {
    const activos = await query("SELECT a.id, a.tipo_vendedor, a.agencia_nombre, c.nombre, c.apellido, c.telefono FROM autos a LEFT JOIN vendedores v ON v.id=a.vendedor_id LEFT JOIN contactos_info c ON c.id=v.contacto_id WHERE a.estado='activo' AND a.fecha_creacion > datetime('now','-7 days')");
    if (!activos.length) return 0;
    const inv = await query("SELECT fyradrive_web_id, tipo_vendedor, agencia_nombre, dueno_telefono FROM inventario_autos WHERE fyradrive_web_id IN (" + activos.map(() => '?').join(',') + ")", activos.map(a => a.id));
    const porId = {}; inv.forEach(r => { porId[r.fyradrive_web_id] = r; });
    let curados = 0;
    for (const a of activos) {
        const tipo = a.tipo_vendedor === 'agencia' ? 'agencia' : 'particular';
        const ag = tipo === 'agencia' ? (a.agencia_nombre || null) : null;
        const telV = String(a.telefono || OWNER_CARGA).replace(/\D/g, '').slice(-10);
        const fila = porId[a.id];
        const sana = fila && String(fila.tipo_vendedor || '') === tipo && String(fila.agencia_nombre || '') === String(ag || '') && String(fila.dueno_telefono || '').replace(/\D/g, '').slice(-10) === telV;
        if (sana) continue;
        const nom = [a.nombre, a.apellido].filter(Boolean).join(' ') || 'Sebastián';
        await upsertInventarioAcred(a.id, { tipo, agencia: ag, nombre: nom, tel: '521' + telV });
        try { await rehornearPortada(a.id); } catch (e) { console.error('[sanador portada]', a.id, e.message); }
        console.log('[sanarInventario] curado', a.id, '→', tipo, ag || nom);
        curados++;
    }
    return curados;
}

module.exports = { pieza, barrerCargas, publicarCarga, parseFicha, ensureCarga, ordenarFotos, destinoDe, armarBody, faltanDe, interpretarAcreditacion, duenoConocido, reacreditar, sanarInventario };
