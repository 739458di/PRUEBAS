...
function gobernarSalida(payload, ctx) {
    ...
    if (g.silencio) {
        ...
        if (payload.fotos || payload.ubicacion_auto_id != null) { const segs = [payload.segmentos[0]]; if (/ubicacion/.test(universo) && !(est0.cita_viva || est0.cita_confirmada)) segs.push(GANCHO_UBICACION); return { ...payload, segmentos: segs, _gobernado: ['solo_media'] }; }
    ...
    if (/ubicacion/.test(universo) && !(est0.cita_viva || est0.cita_confirmada)) { g.segmentos = g.segmentos.filter(x => !esGancho(x)); g.segmentos.push(GANCHO_UBICACION); }
