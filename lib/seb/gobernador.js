...
function esGancho(seg) {
    ...
    const n = norm(s);
    if (!/\?/.test(s) && !/(te agendo de una vez|te agendo cita|con que me digas|me dices tu horario|me avisas tu horario)/.test(n)) return false;   // gancho = pregunta, o imperativo de cita sin '?'
    return RE_GANCHO.test(n);
}
...
const GANCHO_UBICACION = 'Si te interesa venir a manejarlo te agendo cita 👍';
function gobernarSalida(payload, ctx) {
    const est0 = (ctx && ctx.est) || {};
    ...
    if (g.silencio) {
        ...
        if (payload.fotos || payload.ubicacion_auto_id != null) { const segs = [payload.segmentos[0]]; if (payload.ubicacion_auto_id != null && !(est0.cita_viva || est0.cita_confirmada)) segs.push(GANCHO_UBICACION); return { ...payload, segmentos: segs, _gobernado: ['solo_media'] }; }
        // JAMÁS SILENCIO ANTE UNA PREGUNTA REAL (owner 2026-09-26: "como quedaria a credito" / "es lo menos menos?" → Seb calló): si el comprador dijo algo con
        // contenido y al quitar lo repetido/ganchos no queda nada que contestarle, el cerebro NO tenía la respuesta → escala al vendedor (entrega la voz).
        const tx0 = norm((ctx && ctx.texto) || '');
        const cortesia = !tx0 || tx0.length < 4 || /^\W*(gracias|muchas gracias|ok|okey|va|vale|sale|perfecto|listo|bien|excelente|de acuerdo|jaja+|ja|mm+|si|sip|no|nel|👍|🙏|👌|hola|holi|que tal|hey|saludos|que onda|buen dia|buenos dias|buenas( tardes| noches)?)\W*(gracias)?\W*$/.test(tx0);
        if (!cortesia) return { ok: false, motivo: 'gobernador_sin_respuesta', escalar_owner: true, escala_motivo: 'no tuve respuesta para: "' + String(ctx.texto).slice(0, 80) + '"', _gobernado: ['silencio', 'escala'] };
        return { ok: false, motivo: 'gobernador_silencio', _gobernado: ['silencio'] };
    }
    // UBICACIÓN (orden owner 2026-09-22): cada vez que se manda la ubicación y NO hay cita, cierra con el gancho fijo — aunque sea seguido.
    if (payload.ubicacion_auto_id != null && !(est0.cita_viva || est0.cita_confirmada) && !g.segmentos.some(x => (esGancho(x) || /\?\s*$/.test(String(x))) && /(hora|dia|d[ií]a|hoy|manana|cuando|agend|cita|te queda|te acomoda|manejar)/.test(norm(x))) && (est0.ultimos_out || [])[ (est0.ultimos_out || []).length - 1 ] !== nkey(GANCHO_UBICACION)) { g.segmentos = g.segmentos.filter(x => !esGancho(x)); g.segmentos.push(GANCHO_UBICACION); }   // no pisa una pregunta de día/hora ya hecha, ni sale dos veces seguidas
