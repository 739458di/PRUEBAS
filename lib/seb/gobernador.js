...
function gobernarSalida(payload, ctx) {
    ...
    if (g.silencio) {
        ...
        const cortesia = !tx0 || tx0.length < 4 || /^\W*(gracias|muchas gracias|ok|okey|va|vale|sale|perfecto|listo|bien|excelente|de acuerdo|jaja+|ja|mm+|si|sip|no|nel|👍|🙏|👌)\W*(gracias)?\W*$/.test(tx0);
    ...
    if (payload.ubicacion_auto_id != null && !(est0.cita_viva || est0.cita_confirmada) && !g.segmentos.some(x => esGancho(x) && /(hora|dia|d[ií]a)/.test(norm(x))) && (est0.ultimos_out || [])[ (est0.ultimos_out || []).length - 1 ] !== nkey(GANCHO_UBICACION)) { g.segmentos = g.segmentos.filter(x => !esGancho(x)); g.segmentos.push(GANCHO_UBICACION); }   // no pisa una pregunta de día/hora ya hecha, ni sale dos veces seguidas
