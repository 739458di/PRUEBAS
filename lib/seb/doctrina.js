...
// REGLA FINAL (owner 2026-07-13, "ya no le corresponde al bot"): si el bot ESCALÓ —
// por la razón que sea (doctrina, no-claro, juez, lo que sea) — ese momento dejó de
// ser del bot. El PRIMER manual del owner después de CUALQUIER escalada — del tamaño
// que sea, hasta un "okey" — toma posesión. El bot ya levantó la mano; si el owner
// entró, el contexto es suyo.
function posesionOwner(mensajes, escalas) {
    ...
    for (let i = 0; i < (mensajes || []).length; i++) {
        ...
        ctrl = 0;
        // ¿este manual llega tras una escalada pendiente (la que sea)? → posesión
        const escPendiente = escalas.some(e => Number(e.ts) > prevOutTs && Number(e.ts) <= Number(m.ts));
        if (escPendiente) { ctrl = Number(m.ts); prevOutTs = Number(m.ts); continue; }
