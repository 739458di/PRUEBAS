...
function posesionOwner(mensajes) {
    let ctrl = 0, rescateTs = 0, rescateRespondido = false;
    for (let i = 0; i < (mensajes || []).length; i++) {
        ...
        if (ctrl && (Number(m.ts) - ctrl) < 12 * 3600000) { ctrl = Number(m.ts); continue; }
        ctrl = 0;
        ...
        if (!conIniciativa && palabras.length <= 2) continue;
        let pendiente = false;
        for (let j = i - 1; j >= 0; j--) {
