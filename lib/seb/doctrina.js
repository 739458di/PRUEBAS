...
// CONTADO (owner 2026-07-23, caso soria "¿esto es precio de contado?" → el bot
// ofreció cotización de crédito): TODO lo que traiga "contado" es dinero/política
// (pregunta de precio contado, intención de pagar de contado, descuento por
// contado) → el bot se CALLA y ESCALA. Jamás una tarjeta como respuesta.
const RE_CONTADO = /\bcontado\b/;
const RE_RIESGO = /(se me (hace|ase|aze|sale|queda|ve) (algo |muy |un poco |medio |bien )?(car[oa]|alt[oa]|pesad[oa]|elevad[oa]|mucho|fuerte)|est[aá] (muy |algo |un poco |medio )?(car[oa]|alt[oa]|elevad[oa]|pesad[oa])|l[oa] veo (muy |algo |un poco |medio )?(alt[oa]|car[oa]|elevad[oa])|muy car[oa]|carisim|no me alcanza|no me sale|no me ajusta|no completo\b|se me hace (caro|mucho|pesado|alto)|(mensualidad|pago mensual|enganche|precio)[^?]{0,24}(alt[oa]|pesad[oa]|fuerte|elevad)|fuera de mi (presupuesto|alcance)|no puedo pagar(lo)?|esta pesado)/;
...
// ══ POSESIÓN — REGLA FINAL Y DEFINITIVA (owner, 2026-07-13) ═══════════════════
// EN ETAPA 1-2 EL BOT SIGUE SIEMPRE, independientemente de todo (posesión no existe).
// EN ETAPA 3 (ráfaga #3+): si el owner CONTESTA — lo que sea, sea bug o no — el bot
// entra en posesión (herramienta pura, sin gancho; el cierre es del owner).
// Si el owner no contesta, el flujo es todo normal.
// SIN RELOJ (orden owner): la posesión es INDEFINIDA — la operación es suya hasta que
// el chat vuelva a NACER con un opener del bot (ciclo nuevo: reset / recontacto que
// arranca de cero). Mientras tanto, aunque pasen días, el contexto avanzado es suyo.
const RE_OPENER_BOT = /mucho gusto, mi nombre es sebasti/;
function posesionOwner(mensajes, escalas) {
    let ctrl = 0, rafagas = 0, prevDir = null;
    for (const m of (mensajes || [])) {
        if (m.direccion === 'out') {
            if (prevDir !== 'out') rafagas++;
            prevDir = 'out';
            if (!m.ai && rafagas >= 3) ctrl = Number(m.ts);
            else if (m.ai && ctrl && RE_OPENER_BOT.test(String(m.mensaje || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''))) ctrl = 0;
        } else prevDir = 'in';
    }
    return ctrl;
}

module.exports = { puertaEntrada, puertaSalida, DESTINOS, RE_HUMANO, RE_RIESGO, RE_CONTADO, RE_CALIENTE, RE_FRENO, esStandby, ACUSE_STANDBY, herramientaPura, UNIV_HERRAMIENTA, posesionOwner, RE_CONTROL };
