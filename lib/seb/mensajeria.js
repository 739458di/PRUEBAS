...
async function persistirSimulado({ chat, tenantId, demo, tenant, tel, textos, fotos, imagen_ref, location, manual, clave, ts }) {
    ...
    const base = Number(ts) || (t ? await require('./demo.js').ahoraChat(chat.id) : Date.now());   // universos ≠ 0 simulados (sandbox): hora de la PRUEBA de este chat, o la hora exacta del recordatorio que sale
            ...
            if (t && (opts.imagen_ref || (location && location.lat != null) || fotos.length)) {
                try {
                    ...
                    for (let i = 0; i < filasM.length; i++) {
                        const f = filasM[i], tsM = baseM + i + 1, msgIdM = 'media:' + clave + ':' + f.k;
