...
async function enviar(opts) {
    ...
    try {
        if (simulado) {
            ...
            // MEDIA VISIBLE EN FYRACHAT (orden owner 2026-09-15): el puente solo persiste el TEXTO; la captura del punto y el pin
            // se dejan aquí como renglones del hilo (tipo image 'ubic-img:<id>' / tipo location) para que el chat los pinte.
            if (t && (opts.imagen_ref || (location && location.lat != null))) {
                try {
                    ...
                    for (let i = 0; i < filasM.length; i++) {
                        ...
                        await timbreMensaje({ tenant_id: t, chat_id: Number(chat.id), telefono: tel, simulado: false, mensaje: { id: null, msg_id: msgIdM, dir: 'out', emisor: emisorNorm('out', emisorM, aiM), texto: f.tipo === 'location' ? '[ubicación]' : '[captura del punto]', ts: tsM, media: null, estado: 'enviado' } }).catch(() => { });
                    }
