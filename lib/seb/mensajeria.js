...
async function persistirSimulado({ chat, tenantId, demo, tenant, tel, textos, fotos, imagen_ref, location, manual, clave, ts }) {
    const t = Number(tenantId) || 0;
    const ai = manual ? 0 : 1;
    ...
    if (demo && tenant) {
        ...
        let i = 0; const base = Number(ts) || await DEMO.ahoraChat(chat.id);   // hora de la prueba de ESTE chat (o la hora exacta del recordatorio que sale)
    ...
    const base = await require('./demo.js').tsMono(chat.id, Number(ts) || (t ? await require('./demo.js').ahoraChat(chat.id) : Date.now()));   // universos ≠ 0 simulados (sandbox): hora de la PRUEBA de este chat, o la hora exacta del recordatorio que sale
...
// ══════════════════════════════════════════════════════════════════════════════════════════════
// ══ CANDADO DE TURNO (owner 2026-09-26: dos turnos contestaron el mismo bloque → "Perfecto, ahí nos vemos" sin nada concreto): cada turno de Seb corre con su
//    contexto (chat + último mensaje del comprador que atiende). Si entró un mensaje más nuevo, ese turno quedó VIEJO y ya no puede mandar NADA automático, por
//    ningún camino (cerebro, citas, 3 partes): el turno del mensaje nuevo contesta el bloque completo. No depende de tiempos.
const { AsyncLocalStorage } = require('async_hooks'); const TURNO = new AsyncLocalStorage();
async function turnoViejo() { const st = TURNO.getStore(); if (!st || !st.chatId || !st.inId) return false; if (st.viejo) return true; try { const r = (await query("SELECT MAX(id) m FROM mensajes WHERE conversacion_id = ? AND direccion = 'in'", [Number(st.chatId)]))[0]; if (Number(r && r.m) > Number(st.inId)) st.viejo = true; } catch (e) { } return !!st.viejo; }
async function enviar(opts) {
    ...
    if (!manual && await turnoViejo()) return { ok: false, superado: true, error: 'turno superado: entró un mensaje más nuevo del comprador' };
    ...
    // LA VOZ (voz.js): la conversación de Seb no sale si el vendedor tiene la voz, ni si nació en una generación anterior (respuesta armada antes de un cambio de voz)
    if (String(opts.accion || '') === 'seb_turno') {
        if (String(chat.estado_bot || '') === 'humano') return R(409, { error: 'voz_humano: el vendedor tiene la voz de este chat', voz: 'humano' });
        if (opts.voz_gen != null && Number(opts.voz_gen) !== (Number(chat.voz_gen) || 0)) return R(409, { error: 'voz_gen: esta respuesta nació en una generación anterior', voz_gen: Number(chat.voz_gen) || 0 });
    }
    ...
    try {
        if (simulado) {
            ...
            const u = await persistirSimulado({ chat, tenantId: t, demo, tenant, tel, textos: textos.slice(yaSalieron), fotos, imagen_ref: opts.imagen_ref || (imagen ? '[imagen]' : null), location, manual, clave, ts: opts.ts });
            out.ok = true; out.msg_id = u.msg_id; out.enviados_textos = textos.length; out.enviadas_fotos = fotos.length;
            // el renglón ya existe: se devuelve su id (1 lectura por PK lógica) para que la UI lo registre con la MISMA llave que luego trae el hilo (sin id, la burbuja optimista y la del hilo no casaban)
            let idS = u.id == null ? null : Number(u.id);
            if (idS == null && u.msg_id) { try { const mS = (await query('SELECT id FROM mensajes WHERE conversacion_id=? AND msg_id=? LIMIT 1', [chat.id, u.msg_id]))[0]; idS = mS ? Number(mS.id) : null; } catch (e) { } }
            out.mensaje = { id: idS, msg_id: u.msg_id, ts: u.ts, estado: 'enviado' };
        } else {
            ...
            // MEDIA VISIBLE EN FYRACHAT (orden owner 2026-09-15): el puente solo persiste el TEXTO; la captura del punto, el pin y LAS FOTOS
            // se dejan aquí como renglones del hilo (tipo image 'ubic-img:<id>' / '[imagen] <url>' / tipo location) para que el chat los pinte.
            if (t && (opts.imagen_ref || (location && location.lat != null) || fotos.length)) {
                try {
                    const emisorM = manual ? 'dueno' : 'asistente'; const aiM = manual ? 0 : 1; const baseM = Date.now();
                    const filasM = [];
                    if (opts.imagen_ref) filasM.push({ texto: String(opts.imagen_ref), tipo: 'image', k: 'img' });
                    if (location && location.lat != null) filasM.push({ texto: [location.name || '', location.lat, location.lng, location.maps_link || ''].join('|||'), tipo: 'location', k: 'loc' });
                    fotos.forEach((u, i) => filasM.push({ texto: '[imagen] ' + u, tipo: 'image', k: 'f' + i }));
                    for (let i = 0; i < filasM.length; i++) {
                        const f = filasM[i], tsM = await require('./demo.js').tsMono(chat.id, baseM + i + 1), msgIdM = 'media:' + clave + ':' + f.k;
                        await run("INSERT OR IGNORE INTO mensajes (conversacion_id, msg_id, ts, direccion, emisor, texto, tipo, ai_generated, created_at) VALUES (?,?,?,?,?,?,?,?,?)", [chat.id, msgIdM, tsM, 'out', emisorM, f.texto, f.tipo, aiM, Date.now()]);
                        const urlF = (String(f.texto).match(/https?:\/\/\S+/) || [])[0] || null;
                        const mediaF = f.tipo === 'image' ? { tipo: 'imagen', url: urlF || (/^ubic-img:(\d+)/.test(f.texto) ? '/api/seb-panel?action=ubic_img&auto_id=' + f.texto.slice(9) : null) } : null;
                        await timbreMensaje({ tenant_id: t, chat_id: Number(chat.id), telefono: tel, simulado: false, mensaje: { id: null, msg_id: msgIdM, dir: 'out', emisor: emisorNorm('out', emisorM, aiM), texto: f.tipo === 'location' ? '[ubicación]' : (urlF ? '' : '[captura del punto]'), ts: tsM, media: mediaF, estado: 'enviado' } }).catch(() => { });
                    }
                } catch (e) { console.error('[mensajeria] media visible:', e.message); }
            }
...
module.exports = { TURNO, turnoViejo, enviar, conClave, recibo, ensureEnvios, resolverDestino, esTelPrueba, emisorNorm, origenPuente, ORIGENES_PUENTE, tocar, timbreMensaje, _prueba };
