...
const CTX = require('./contexto.js');   // universo ambiente (TERRA MOTORS): lo que pida el 0 se redirige
function hiloDe(tel, tenantId) { const tt = CTX.remap(tenantId); return 'whatsapp:' + tel + (tt ? '#t' + tt : ''); }

// ══ DDL idempotente — UNA vez por proceso ══
const COLS_NUEVAS = [
    ['estado_json', 'TEXT'], ['canal', 'TEXT'], ['estado_bot', 'TEXT'],
    ['auto_id_activo', 'INTEGER'], ['platform', 'TEXT'], ['estado_ts', 'INTEGER'], ['voz_gen', 'INTEGER'], ['miembro_id', 'INTEGER']
];
...
// ══ EL CHAT ══
const COLS_CHAT = 'id, channel_thread_id, telefono, nombre, tenant_id, ult_msg_ts, ult_dir, is_dueno_chat, source, estado_json, canal, estado_bot, auto_id_activo, platform, estado_ts, voz_gen';
...
async function chatDe(tenantId, tel, opts) {
    ...
    const t = CTX.remap(tenantId), p = tel12(tel);
    if (!p) return null;
...
async function leerEstado(tenantId, tel) {
    ...
    const e = Object.assign({ chat }, estadoDe(chat));
    if (CTX.tenant()) {   // universo ambiente: el foco del lote (id web, o el de su delegación) → id de inventario para el cerebro
        let foco = e.auto_id_activo;
        if (!foco && chat) { try { const d = await delegacionActiva(chat.id); if (d && d.auto_id) foco = Number(d.auto_id); } catch (err) { } }
        e.auto_id_activo = CTX.aInv(foco);
    }
    return e;
}
...
async function guardarEstado(tenantId, tel, patch, opts) {
    ...
    if (CTX.tenant() && patch.auto_id_activo != null) patch = Object.assign({}, patch, { auto_id_activo: CTX.aWeb(patch.auto_id_activo) });   // el lote guarda su foco con el id de la web
    const t = CTX.remap(tenantId);
...
// ══ BARREDOR DE HUÉRFANOS @lid (FyraChat v2, 2026-09-10; cron 1/h) ══
// Causa 2 del mapa: conversaciones del universo 0 cuya identidad es un @lid (WhatsApp) y cuyo teléfono real YA
// se conoce (lid_phone_map) pero que el puente no alcanzó a fusionar (auto-curación en vivo: fusionarSiHuerfano).
// Referencia: fusionarLibretaNueva del puente. Aquí, además, se re-apuntan acciones / citas / delegaciones.
//   · sin chat real → la huérfana se RE-APUNTA al teléfono (mismo id; cero pérdidas)
//   · con chat real → mensajes → chat real (dedup por msg_id), no_leidos se suman, nombre se hereda si faltaba,
//     acciones/citas_match/citas/cita_casillas → chat real, delegaciones huérfanas se cierran, portada = último
//     mensaje real; se BORRA la huérfana.
// opts.limite (default 30) · opts.dry (solo cuenta) · opts.lids (pruebas: SOLO estos lids). Devuelve conteos.
async function fusionarLidHuerfanos(opts) {
    opts = opts || {};
    await ensureUniverso();
    const lim = Math.min(Number(opts.limite) || 30, 200);
    const rep = { candidatas: 0, reapuntadas: 0, fusionadas: 0, mensajes_movidos: 0, saltadas: 0, errores: [], dry: !!opts.dry };
    // JOIN por PK de lid_phone_map: solo las conversaciones t0 cuyo "teléfono" es un lid con mapa conocido
    const soloLids = Array.isArray(opts.lids) && opts.lids.length ? opts.lids.map(String) : null;
    const cand = soloLids
        ? await query(`SELECT c.id, c.telefono AS lid, c.nombre, c.no_leidos, l.phone
                       FROM conversaciones c JOIN lid_phone_map l ON l.lid = c.telefono
                       WHERE (c.tenant_id = 0 OR c.tenant_id IS NULL) AND c.telefono IN (${soloLids.map(() => '?').join(',')}) LIMIT ?`, soloLids.concat([lim]))
        : await query(`SELECT c.id, c.telefono AS lid, c.nombre, c.no_leidos, l.phone
                       FROM conversaciones c JOIN lid_phone_map l ON l.lid = c.telefono
                       WHERE (c.tenant_id = 0 OR c.tenant_id IS NULL) LIMIT ?`, [lim]);
    rep.candidatas = cand.length;
    for (const c of cand) {
        const phone = tel12(c.phone);
        if (!esTelMX(phone) || esTelMX(tel12(c.lid))) { rep.saltadas++; continue; }
        if (opts.dry) continue;
        try {
            const real = await _selChat(0, phone);
            const canonT = hiloDe(phone, 0);
            if (!real) {
                await run('UPDATE conversaciones SET channel_thread_id=?, telefono=?, tenant_id=0 WHERE id=?', [canonT, phone, c.id]);
                rep.reapuntadas++;
                continue;
            }
            if (Number(real.id) === Number(c.id)) { rep.saltadas++; continue; }
            const mv = await run('UPDATE OR IGNORE mensajes SET conversacion_id=? WHERE conversacion_id=?', [real.id, c.id]);
            rep.mensajes_movidos += Number(mv.rowsAffected) || 0;
            await run('DELETE FROM mensajes WHERE conversacion_id=?', [c.id]);   // los que chocaron por folio ya existían en el real
            await run('UPDATE acciones SET chat_id=? WHERE chat_id=?', [real.id, c.id]).catch(() => { });
            await run('UPDATE citas_match SET chat_id=? WHERE chat_id=?', [real.id, c.id]).catch(() => { });
            await run('UPDATE citas_match SET dueno_chat_id=? WHERE dueno_chat_id=?', [real.id, c.id]).catch(() => { });
            await run('UPDATE citas SET chat_id=? WHERE chat_id=?', [real.id, c.id]).catch(() => { });
            await run('UPDATE cita_casillas SET chat_id=? WHERE chat_id=?', [real.id, c.id]).catch(() => { });
            await run('UPDATE delegaciones SET hasta=?, motivo=? WHERE chat_id=? AND hasta IS NULL', [Date.now(), 'fusion_lid', c.id]).catch(() => { });
            const last = (await query('SELECT direccion, texto, ts, tipo FROM mensajes WHERE conversacion_id=? ORDER BY ts DESC, id DESC LIMIT 1', [real.id]))[0];
            const nombreOk = (c.nombre && c.nombre !== '.') ? c.nombre : null;
            if (last) {
                const portada = last.tipo === 'image' ? '📷 imagen' : last.tipo === 'location' ? '📍 ubicación' : String(last.texto || '');
                await run('UPDATE conversaciones SET ult_texto=?, ult_dir=?, ult_msg_ts=?, no_leidos=COALESCE(no_leidos,0)+?, nombre=COALESCE(nombre,?) WHERE id=?',
                    [portada.slice(0, 120), last.direccion, Number(last.ts), Number(c.no_leidos) || 0, nombreOk, real.id]);
            } else {
                await run('UPDATE conversaciones SET no_leidos=COALESCE(no_leidos,0)+?, nombre=COALESCE(nombre,?) WHERE id=?', [Number(c.no_leidos) || 0, nombreOk, real.id]);
            }
            await run('DELETE FROM conversaciones WHERE id=?', [c.id]);
            rep.fusionadas++;
            try { await require('./timbre.js').tocar({ entidad: 'chat', accion: 'fusion_lid', tenant_id: 0, chat_id: Number(real.id), que: 'otro', lid_chat_id: Number(c.id) }); } catch (e) { }
        } catch (e) { rep.errores.push({ id: c.id, error: e.message }); }
    }
    return rep;
}

// ══ ESPEJO DE LOTES ↔ AUTOS FYRADRIVE (orden owner 2026-09-23): "cada movimiento de cualquier lote a uno de sus autos se cambia a la par
// conmigo". Los datos ya son la MISMA fila (inventario_autos), así que editar/vender se ve al instante en todos. Lo que hay que espejar
// son los VÍNCULOS: alta → aparece en los lotes con catalogo='fyradrive'; baja → se apaga en TODOS los lotes; reactivar → se prende.
async function sincronizarCatalogoFyradrive({ inv_auto_id } = {}) {
    const tenants = await query("SELECT id, config_json FROM tenants WHERE activo = 1 AND id <> 0").catch(() => []);
    const F = tenants.filter(t => { try { return JSON.parse(t.config_json || '{}').catalogo === 'fyradrive'; } catch (e) { return false; } }).map(t => Number(t.id));
    if (!F.length) return { ok: true, catalogos: 0 };
    const now = Date.now(); let altas = 0, bajas = 0, altasRe = 0;
    // ORDEN OWNER 2026-09-24: "ningún auto de un lote es de Fyradrive hasta que se acepte la comisión". Un auto cuyo DUEÑO es un lote
    // (tenant con usuario / tipo 'lote', fuera de los catálogos Fyradrive) solo entra a Autos Fyradrive con comisiones_auto.estado = 'aceptada'.
    const LOTES = new Set(tenants.filter(t => { try { const c = JSON.parse(t.config_json || '{}'); return (c.usuario || c.tipo === 'lote') && c.catalogo !== 'fyradrive'; } catch (e) { return false; } }).map(t => Number(t.id)));
    try { await run('CREATE TABLE IF NOT EXISTS comisiones_auto (inv_auto_id INTEGER PRIMARY KEY, tenant_id INTEGER, monto INTEGER, propuesto INTEGER, estado TEXT, decidido_por TEXT, decidido_ts INTEGER, updated INTEGER)'); } catch (e) { }
    const ACEPTADAS = new Set((await query("SELECT inv_auto_id FROM comisiones_auto WHERE estado = 'aceptada'").catch(() => [])).map(r => Number(r.inv_auto_id)));
    const autos = inv_auto_id ? await query('SELECT id, estado FROM inventario_autos WHERE id = ?', [Number(inv_auto_id)]) : await query("SELECT id, estado FROM inventario_autos WHERE fyradrive_web_id IS NOT NULL");
    const links = await query('SELECT inv_auto_id, tenant_id, rol, activo FROM autos_universo' + (inv_auto_id ? ' WHERE inv_auto_id = ?' : ''), inv_auto_id ? [Number(inv_auto_id)] : []);
    const idx = new Map(links.map(l => [Number(l.inv_auto_id) + '|' + Number(l.tenant_id), l]));
    const DE_LOTE = new Set(links.filter(l => l.rol === 'dueno' && Number(l.activo) === 1 && LOTES.has(Number(l.tenant_id))).map(l => Number(l.inv_auto_id)));
    for (const a of autos) {
        const vivo = (a.estado === 'activo' || a.estado === 'vendido') && (!DE_LOTE.has(Number(a.id)) || ACEPTADAS.has(Number(a.id)));   // vendido sigue visible; inactivo/baja se apaga; de lote sin comisión aceptada NO es de Fyradrive
        for (const t of F) {
            const l = idx.get(Number(a.id) + '|' + t);
            if (vivo && !l) { await run("INSERT OR IGNORE INTO autos_universo (inv_auto_id, tenant_id, rol, origen, activo, created, updated) VALUES (?,?,'acreditado','acreditar',1,?,?)", [Number(a.id), t, now, now]); altas++; }
            else if (vivo && l && Number(l.activo) !== 1) { await run('UPDATE autos_universo SET activo = 1, updated = ? WHERE inv_auto_id = ? AND tenant_id = ?', [now, Number(a.id), t]); altasRe++; }
            else if (!vivo && l && Number(l.activo) === 1) { await run('UPDATE autos_universo SET activo = 0, updated = ? WHERE inv_auto_id = ? AND tenant_id = ?', [now, Number(a.id), t]); bajas++; }
        }
    }
    return { ok: true, catalogos: F.length, autos: autos.length, altas, reactivadas: altasRe, bajas };
}
module.exports = {
    sincronizarCatalogoFyradrive,
    ...
    backfillUniverso, fusionarLidHuerfanos
};
