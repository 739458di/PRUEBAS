...
function ensureAcciones() {
    ...
    _ens = (async () => {
        await run(`CREATE TABLE IF NOT EXISTS acciones (
            ...
            ts INTEGER NOT NULL,
            actor TEXT)`);
        try { await run('ALTER TABLE acciones ADD COLUMN sesion_id INTEGER'); } catch (e) { /* ya existe */ }   // bitácora: qué sesión de FyraChat la disparó (2026-09-10)
...
// registrar({ tenant_id, chat_id, delegacion_id, tipo, ref_id, meta, actor, ts, sesion_id }) → id | null
...
async function registrar(a) {
    try {
        ...
        const ins = await run('INSERT INTO acciones (tenant_id, chat_id, delegacion_id, tipo, ref_id, meta_json, ts, actor, sesion_id) VALUES (?,?,?,?,?,?,?,?,?)',
            [Number(a.tenant_id) || 0, a.chat_id == null ? null : Number(a.chat_id), a.delegacion_id == null ? null : Number(a.delegacion_id),
             String(a.tipo || 'accion'), a.ref_id == null ? null : Number(a.ref_id), meta, ts, a.actor || 'sistema', a.sesion_id == null ? null : Number(a.sesion_id)]);
