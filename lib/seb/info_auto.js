...
function subInfo(texto) {
    ...
    // PETICIÓN de ficha ("mándame la información/los detalles") NO es pregunta de
    // condición: la atiende la mesa con el paquete real (ficha + fotos + punto).
    if (/(m[aá]nda|manda|pasa|env[ií]a|comparte|quiero|me das|me pasas)[^?]{0,25}(info|informacion|detalles|ficha|datos)/.test(t)) return null;
    if (/(que |algun )?detalle|como esta (el|la|ese|esa)|en que estado|tiene algun|condiciones|esta bien (el|la|ese)|esta en buen|le falta algo|funciona bien|cuentame mas|que mas me (puedes|dices|cuentas)/.test(t)) return 'detalles';
...
async function infoAccion(texto, auto_id) {
    ...
    const t = norm(texto);
    const r = await query("SELECT marca, modelo, anio, kilometraje, color, transmision, fyradrive_web_id FROM inventario_autos WHERE id=?", [Number(auto_id)]);
    if (!r.length) return null;
    const a = r[0];
    const hechos = await require('./aparador.js').hechosDe(a.fyradrive_web_id);   // dueños / factura REALES; sin dato → se escala, no se inventa
    ...
    const duenoFrase = !hechos.duenos ? null : (hechos.duenos === 'único dueño' ? (/(ha tenido|ha sido|han sido|tuvo|tenido)/.test(t) ? 'Ha tenido un solo dueño' : /cuantos? (dueñ|duen)[^?]*(tiene|trae)/.test(t) ? 'Tiene un solo dueño' : 'Es de único dueño') : 'Ha tenido ' + hechos.duenos);
    ...
    switch (sub) {
        ...
        case 'papeleria':   return { sub, accion: hechos.factura === 'ok' ? (/que factura|tipo de factura/.test(t) ? 'Tiene factura de agencia, toda la papelería en regla' : 'Sí, tiene factura de agencia y toda la papelería en regla') : null };   // sin dato de factura → escala
        case 'adeudos':     return { sub, accion: adeudos };
        case 'choques':     return { sub, accion: null };      // no hay dato de choques en la base → lo ves tú (antes: 'cero choques' inventado)
        case 'servicios':   return { sub, accion: null };      // no hay dato de servicios → lo ves tú
        case 'clima':       return { sub, accion: clima };
        case 'detalles':    return { sub, accion: 'Está en excelentes condiciones; ven a verlo y lo revisas con calma, puedes traer a alguien de tu confianza' };
    }
    return null;
}
