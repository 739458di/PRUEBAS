    ...
    if (cambio) {
        ...
        if (avisar) {
            ...
            if (mDue) await avisarMiembro(tenant, mDue, txtA);
                ...
                const todos = await query('SELECT id, nombre, telefono FROM vendedores_universo WHERE tenant_id = ? AND activo = 1', [Number(tenant.id)]).catch(() => []);
                if (todos.length) { for (const m of todos) await avisarMiembro(tenant, { id: Number(m.id), nombre: m.nombre, telefono: m.telefono }, '(chat sin vendedor) ' + txtA); }
