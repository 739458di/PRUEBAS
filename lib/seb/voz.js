...
async function entregar({ tenant, chat, motivo, avisar = true, fuente = 'seb', miembro = null, forzar = false }) {
    // MARIO AGREGADOR (owner 2026-09-27: "Mario habla entre los mensajes como solucionador; lo de Seb es de Seb"): en universos con agregador, escalar NO le quita la
    // voz a Seb — solo le avisa a Mario lo que le toca. Seb sigue contestando lo que sí es suyo. Mario toma el chat completo solo con el botón (forzar).
    let agregador = false; try { const cfA = tenant.config || JSON.parse(tenant.config_json || '{}'); agregador = Number(cfA.agregador) === 1; } catch (e) { }
    if (agregador && !forzar && String(chat.canal || '') !== 'dueno') {
        if (fuente === 'vendedor' && miembro && miembro.id) { try { await run('UPDATE conversaciones SET miembro_id = ? WHERE id = ? AND miembro_id IS NULL', [Number(miembro.id), Number(chat.id)]); } catch (e) { } return { cambio: false, voz: 'seb', agregador: true }; }
        ...
        await nota(tenant, chat, '🔔 Para ti: ' + String(motivo || '').replace(/^[\s🔴🔥💰🧠]+/, '') + ' — contéstale tú cuando puedas; Seb sigue atendiendo lo demás.');
        if (avisar) { const txtA = '🔔 ' + nombreDe(chat) + ': ' + String(motivo || '').replace(/^[\s🔴🔥💰🧠]+/, '') + '\n' + linkChat(tenant.id, chat.id) + '\nContéstale directo; Seb sigue con lo demás.'; const mDue = await miembroDelChat(chat); if (mDue) await avisarMiembro(tenant, mDue, txtA); else await avisarVendedor(tenant, txtA); }
        return { cambio: false, voz: 'seb', agregador: true };
    }
