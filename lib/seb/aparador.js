...
function buscarInventario(criterio, autos) {
    ...
    if (criterio.tipo === 'camioneta') res = res.filter(a => carroceriaDe(a) === 'camioneta');
    else if (criterio.tipo) res = res.filter(a => carroceriaDe(a).includes(criterio.tipo));
...
async function redactarUbicaciones(rows) {
    ...
    if (puntos.length === 1 && conPunto.every(x => x.p)) {
        return {
            mismo: true,
            segmentos: [`Los ${rows.length === 2 ? 'dos' : rows.length} los tenemos en ${puntos[0]}`],
...
async function fichaExtensa(a) {
    ...
    if (tr) detalles.push(cap(String(tr)));
    ...
    if (w && w.motor) detalles.push('Motor ' + String(w.motor).slice(0, 20));
...
const MODELOS_SEDAN = /(sentra|versa|attitude|yaris|city|accent|rio\b|cavalier|onix|aveo|virtus|vento|jetta|mazda ?3|corolla|civic|elantra|cerato|forte|a3\b|clase a|serie 3|mustang|camry|altima|sonata|beat|spark)/;
const MODELOS_SUV_PICKUP = /(cr-?v|rav ?4|cx-?[3579]|tucson|sportage|seltos|kicks|hr-?v|x-?trail|tracker|trax|captiva|equinox|edge|escape|bronco|explorer|expedition|tahoe|suburban|cherokee|wrangler|compass|renegade|pilot|highlander|4runner|sienna|odyssey|tacoma|tundra|ranger|lobo|f-?150|silverado|ram\b|frontier|np300|l ?200|hilux|colorado|maverick|ridgeline|x[1-7]\b|q[3578]\b|gl[abces]|tiguan|teramont|touareg|murano|pathfinder|armada|durango|journey|blazer|traverse|acadia|terrain)/;
function carroceriaDe(a) {
    const c = normz(a.tipo_carroceria || '');
    if (c && c !== 'no especificado') return /suv|camioneta|pick/.test(c) ? 'camioneta' : c;
    const n = normz(a.nombre || [a.marca, a.modelo].join(' '));
    if (MODELOS_SUV_PICKUP.test(n)) return 'camioneta';
    if (MODELOS_SEDAN.test(n)) return 'sedan';
    return '';
}
...
function scoreSimilitud(ancla, a) {
    ...
    let s = 40 * (1 - dp / 0.35);
    const cb = carroceriaDe(ancla), ca = carroceriaDe(a);
    if (cb && cb === ca) s += 30;
    else if (cb && ca && cb !== ca) s -= 15;          // sedán vs camioneta NO van juntos
...
async function arranqueCarrusel({ tel, textoRaw, textoFamilia, adCtx, textosIn, nombre, esClick, duda }) {
    ...
    if (esClick) {
        ...
        apr.segmentos = [
            ...
            '¿Cuál te late conforme a lo que andas buscando? Te mando todo a detalle'
        ];
