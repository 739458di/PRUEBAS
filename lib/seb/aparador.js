...
function resolverEleccion(texto, aparador, ancla) {
    // los placeholders del puente ([imagen], [mensaje], [sticker]) no son palabras
    // del comprador: se quitan para que "[imagen] 2" se lea como "2"
    const t = normz(String(texto || '').replace(/\[[^\]]{1,20}\]/g, ' ').replace(/\s+/g, ' ').trim());
