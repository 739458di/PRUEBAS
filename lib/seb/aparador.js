...
async function descripcionDe(auto) {
    ...
    car.push('🔵 ' + (fmtDuenos(w && w.numero_duenos) || 'Único dueño'));
    if (!(w && w.factura_original) || /si|s[ií]|original|agencia/i.test(String((w && w.factura_original) || ''))) car.push('🔵 Factura original');
    else car.push('🔵 ' + String(w.factura_original).slice(0, 40));
...
async function fichaExtensa(a) {
    ...
    const du = ({ dos: '2 dueños', tres: '3 dueños', cuatro_o_mas: '4 o más dueños' }[String((w && w.numero_duenos) || '')]) || 'Único dueño';
    detalles.push(du);
    if (!(w && w.factura_original) || /si|s[ií]|original|agencia/i.test(String((w && w.factura_original) || ''))) detalles.push('Factura original');
