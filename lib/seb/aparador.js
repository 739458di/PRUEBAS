...
function scoreSimilitud(ancla, a) {
    ...
    let s = 40 * (1 - dp / 0.35);
    const cb = categoriaDe(ancla), ca = categoriaDe(a);
    if (cb === ca) s += 30;                           // misma categoría = misma intención
    else if ((cb === 'suv' && ca === 'pickup') || (cb === 'pickup' && ca === 'suv')) s += 12;
    else s -= 15;                                     // sedán vs camioneta NO van juntos
