    ...
    try {
        ...
        if (multi && multi.length >= 2) {
            ...
            try {
                ...
                if (accM && accM !== 'detalles') {
                    const idsM = multi.map(m => m.id).slice(0, 3);
                    ej.mesa = idsM; ej.foco = { id: idsM[0], nombre: multi[0].nombre, via: 'multi', ts: Date.now() };
                    ...
                    const rowsM = idsM.map(id => autosM.find(a => a.id === id)).filter(Boolean);
