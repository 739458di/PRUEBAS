...
async function arranqueCarrusel({ tel, textoRaw, textoFamilia, adCtx, textosIn, nombre, esClick, duda }) {
    ...
    if (esClick) {
        ...
        // ══ Sin certeza del clic → VARIEDAD: mundos distintos + pregunta abierta
        // (jamás comparables de una portada que quizá ni le interesa)
        const apr = await armarAparador({ ancla: null, lista: surtidoVariado(autosAp, 9), saludoNombre: nm, max: 9, nFotos: 5, ordenDado: true });
        if (!apr) return null;
        apr.segmentos = [
            `Qué tal${nm ? ' ' + nm : ''} ${saludoHora()}!`,
            'Mucho gusto, mi nombre es Sebastián Romero, para servirte',
            'Sí claro, te paso algo de lo que tenemos:\n' + apr.segmentos[1],
            `¿Qué auto estás buscando${nm ? ' ' + nm : ''}? Tenemos una gran variedad 👍`
        ];
        apr.fotos_after_index = 2;
        await guardarEstadoAparador(tel, apr.aparador, null);
        return { tipo: 'opener_variedad', segmentos: apr.segmentos, fotos: apr.fotos, fotos_after_index: apr.fotos_after_index };
    }
