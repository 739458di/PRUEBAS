...
async function arranqueCarrusel({ tel, textoRaw, textoFamilia, adCtx, textosIn, nombre, esClick, duda }) {
    ...
    if (esClick) {
        ...
        const listaAp = ancla ? similaresA(ancla, autosAp, 7) : surtidoVariado(autosAp, 9);
        const apr = await armarAparador({ ancla, lista: listaAp, saludoNombre: nm, max: ancla ? 8 : 9, nFotos: ancla ? 4 : 5, ordenDado: true });
        if (!apr) return null;
        if (!ancla) {
            // NADA identificado → variedad + gancho abierto (orden owner)
            apr.segmentos = [
