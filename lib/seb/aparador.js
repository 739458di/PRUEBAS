...
async function intentarEleccionAparador(tel, texto, convId) {
    try {
        ...
        if (multi && multi.length >= 2) {
            ...
            segs.push('¿Te late venir a verlos y manejarlos? Cuando gustes te los enseño y ya ahí decides cuál te acomoda');
            ...
            return { tipo: 'aparador_foco_multi', segmentos: segs };
        }
        ...
        if (!extra.fotos) segmentos.push('¿Te late venir a verlo y manejarlo? O si prefieres te mando primero las fotos y la ficha completa');
...
async function opcionesEnFlujo({ tel, texto }) {
    try {
        ...
        let lista = null, intro = null, tipo = null;
        if (crit) {
            ...
            lista = filtradas; tipo = 'aparador_filtrado';
            intro = 'Va, con eso que me dices estas te van 🚗';
        } else {
            // OPCIONES → acorde a su interés (parecidos al foco); sin foco → oportunidades
            const focoRow = focoId ? autos.find(a => a.id === focoId) : null;
            if (focoRow) {
                lista = autosRelacionados(focoRow, autos).filter(a => !interes.includes(a.id));
        ...
        const apr = await armarAparador({ ancla: null, lista, intro });
        if (!apr) return null;
        ...
        delete ej.foco; delete ej.pregunta;
...
async function arranqueCarrusel({ tel, textoRaw, textoFamilia, adCtx, textosIn, nombre, esClick, duda }) {
    ...
    if (esClick) {
        ...
        const apr = await armarAparador({ ancla, lista: autosAp, saludoNombre: nm });
        if (!apr) return null;
    ...
    const apr = await armarAparador({ ancla, lista, saludoNombre: nm, intro: intro === '__machote_anuncio__' ? null : intro });
    if (!apr) return null;
...
module.exports = { espiarVision, identificarAncla, parsearCriterio, buscarInventario, armarAparador, resolverEleccion, autosRelacionados, RE_RELACIONADOS, RE_NECESIDAD, inventarioActivo, portadaDe, fichaBreve, guardarEstadoAparador, intentarEleccionAparador, arranqueCarrusel, opcionesEnFlujo };
