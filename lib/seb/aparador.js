...
const U = require('./universo.js');   // ETAPA 2: estado del bot por (universo, chat)
...
async function inventarioActivo() {
    // CUOTA TURSO (2026-09-08): cacheado 15 s (lib/seb/memo.js) — un solo mensaje entrante lo pedía 5-10 veces
    const { memoQuery, INV_TTL } = require('./memo.js');
    const rows = await memoQuery(INV_TTL, "SELECT id, fyradrive_web_id, marca, modelo, version, anio, precio, kilometraje, color, tipo_carroceria, sb_convs_count, created_at FROM inventario_autos WHERE estado='activo'");
    return require('./contexto.js').filtrar(rows).map(a => ({ ...a, nombre: [a.marca, a.modelo, a.version, a.anio].filter(Boolean).join(' ') }));   // universo ambiente → solo su catálogo
}
...
// ══ TYPOS (LEY DEL OPENER, owner 2026-07-22): "cabalier" ES el Cavalier — un error
// ortográfico JAMÁS acredita que no lo tenemos. Distancia de edición contra los
// tokens del inventario: palabra ≥5 letras, 1 error (5-7) o 2 (≥8). Un solo auto →
// ese; varios del MISMO token (familia) → pregunta; señales cruzadas → nada.
const STOP_TYPOS = new Set(['hola', 'buenas', 'buenos', 'tardes', 'noches', 'informacion', 'informes', 'interesa', 'quiero', 'busco', 'necesito', 'precio', 'cuanto', 'cuesta', 'saber', 'sobre', 'tienen', 'tiene', 'venden', 'carro', 'autos', 'coche', 'camioneta', 'disponible', 'todavia', 'ahorita', 'gracias', 'porfavor', 'favor', 'credito', 'contado', 'enganche', 'mensual', 'kilometraje', 'kilometros', 'automatico', 'estandar', 'vendo', 'vender', 'comprar', 'compra', 'ocupo', 'manejan', 'buscando', 'pueden', 'podria', 'quisiera', 'ubicacion', 'donde', 'fotos', 'imagenes', 'ando', 'anuncio', 'publicacion', 'buenas']);
function distEd(a, b) {
...
function resolverConTypos(texto, autos) {
    const palabras = normz(String(texto || '').replace(/https?:\/\/\S+/gi, ' ')).split(/[^a-z0-9]+/).filter(w => w.length >= 5 && !/^\d+$/.test(w) && !STOP_TYPOS.has(w));
    if (!palabras.length) return null;
    const hits = new Map();                                  // token corregido → Map(id → auto)
    for (const a of autos) {
        const toks = normz(a.nombre).split(/\s+/).filter(w => w.length >= 5 && !/^\d+$/.test(w));
        for (const tk of toks) for (const w of palabras) {
            if (w === tk) continue;                          // exacto ya lo vio el matcher normal
            if (distEd(w, tk) <= (tk.length >= 8 ? 2 : 1)) { if (!hits.has(tk)) hits.set(tk, new Map()); hits.get(tk).set(a.id, a); }
        }
    }
    if (!hits.size) return null;
    const todos = new Map(); for (const m of hits.values()) for (const [id, a] of m) todos.set(id, a);
    if (todos.size === 1) return { auto: [...todos.values()][0] };
    if (hits.size === 1) return { familia: [...todos.values()].slice(0, 4) };
    return null;                                             // señales cruzadas → jamás adivinar
}

// ══ LEY DEL OPENER (owner 2026-07-22): pide información pero NI viene de anuncio
// NI se distingue si COMPRA o VENDE ("cómo funcionan", "info de su negocio") →
// se le pregunta directo el cajón: ¿comprar o vender?
const RE_ROL_AMBIGUO = /(como (funciona|funcionan|trabajan|le hacen)|informacion (de|sobre) (su|el|la) (negocio|lote|agencia|empresa)|^informes[?!. ]*$|que (venden|manejan|ofrecen)|mas informacion[?!. ]*$|^informacion[?!. ]*$|^mas informes[?!. ]*$)/;
function rolAmbiguo({ texto, adCtx }) {
    if (adCtx) return false;                                 // vino de un anuncio → comprador
    if (/https?:\/\//i.test(String(texto || ''))) return false;
    const t = normz(String(texto || '')).trim();
    if (/(compr|busco|vend|interesa (un|el) (auto|carro|coche)|cotiza|enganche|credito|financiam)/.test(t)) return false;
    return RE_ROL_AMBIGUO.test(t);
}
...
function pedidoFueraDeInventario(texto, autos) {
    ...
    // LEY DEL OPENER: si alguna palabra está a UN error de un token del inventario,
    // es un typo de algo que SÍ tenemos — no es puente (el flujo del typo lo atiende)
    if (resolverConTypos(texto, autos)) return null;
...
// ══ EL OJO PARA FOTOS DEL COMPRADOR (orden owner 2026-07-21): manda una foto de
// un auto ("¿tienes este?") → se identifica contra el inventario y el turno sigue
// COMO SI lo hubiera escrito. Mismo motor de visión del carrusel (bytes → Haiku).
async function identificarAutoDeImagen(urlImg, autos) {
    try {
        const ir = await fetch(urlImg, { redirect: 'follow', signal: AbortSignal.timeout(15000) });
        if (!ir.ok) return null;
        const mime = (ir.headers.get('content-type') || 'image/jpeg').split(';')[0];
        const b64 = Buffer.from(await ir.arrayBuffer()).toString('base64');
        if (b64.length < 2000) return null;
        const v = await haiku(
            'Identificas autos en fotos. PRIMERO describe qué ves, LUEGO identifica. Si la foto no trae un auto, todo null. Responde SOLO el JSON.',
            { type: 'object', properties: { descripcion: { type: 'string' }, marca: { type: ['string', 'null'] }, modelo: { type: ['string', 'null'] }, anio: { type: ['integer', 'null'] }, color: { type: ['string', 'null'] } }, required: ['descripcion', 'marca', 'modelo', 'anio', 'color'], additionalProperties: false },
            [{ type: 'image', source: { type: 'base64', media_type: mime, data: b64 } }, { type: 'text', text: 'Identifica el auto de la foto (marca, modelo, año aproximado, color).' }],
            250);
        if (!v || (!v.marca && !v.modelo)) return null;
        let m = matchSenal(v, autos || []);              // único o nada — jamás adivinar
        // desempate DE FOTO (2026-07-22, dos Sentra): la foto es señal APROXIMADA
        // (no tarjeta exacta) — con marca+modelo claros, el AÑO más cercano gana;
        // si no hay año, el COLOR. Sigue siendo hecho duro: empate real → nada.
        if (!m) {
            const anioV = Number((String(v.anio || '').match(/\b(19|20)\d{2}\b/) || [])[0]) || null;
            const tSen = normz([v.marca, v.modelo].filter(Boolean).join(' '));
            const cands = (autos || []).filter(a => {
                const toks = normz(a.nombre).split(/\s+/).filter(w => (w.length >= 3 || /[a-z]\d|\d[a-z]/.test(w)) && !/^\d+$/.test(w));
                return toks.filter(tok => tSen.includes(tok)).length >= 2;
            });
            if (cands.length === 1) m = cands[0];
            else if (cands.length > 1 && anioV) {
                const orden = cands.slice().sort((x, y) => Math.abs(Number(x.anio || 0) - anioV) - Math.abs(Number(y.anio || 0) - anioV));
                if (Math.abs(Number(orden[0].anio || 0) - anioV) < Math.abs(Number(orden[1].anio || 0) - anioV)) m = orden[0];
            }
            if (!m && cands.length > 1 && v.color) {
                const colN = normz(v.color);
                const porColor = cands.filter(a => a.color && normz(a.color).split(/\s+/).some(c => c.length >= 3 && colN.includes(c)));
                if (porColor.length === 1) m = porColor[0];
            }
        }
        return m ? { auto_id: m.id, nombre: m.nombre, vision: v } : null;
    } catch (e) { return null; }
}


// ══ ABSORBER LA IMAGEN DEL COMPRADOR (orden owner 2026-07-22, caso 7721443373:
// contestó "¿cuál Mazda?" con una FOTO + "2" y el bot se quedó mudo). LEY: una
// imagen JAMÁS deja mudo al bot. Si trae URL → se LEE (el ojo) y el nombre del
// auto identificado se suma al texto como si el comprador lo hubiera escrito;
// si no trae URL ni texto útil → se avisa al owner (nunca silencio).
async function absorberImagen({ texto, urls }) {
    const t = String(texto || '');
    const enTexto = t.match(/https?:\/\/\S+/gi) || [];
    const todas = [...(Array.isArray(urls) ? urls.filter(Boolean) : []), ...enTexto.filter(u => /\.(jpe?g|png|webp)(\?|$)|blob\.vercel|fbcdn|whatsapp/i.test(u))];
    const tieneImg = /\[(imagen|foto|sticker)[^\]]*\]/i.test(t) || todas.length > 0;
    if (!tieneImg) return { texto: t, imagen: false };
    const limpio = t.replace(/\[[^\]]{1,20}\]/g, ' ').replace(/https?:\/\/\S+/g, ' ').replace(/\s+/g, ' ').trim();
    if (!todas.length) return { texto: t, imagen: true, sinUrl: true, textoUtil: !!limpio };
    try {
        const autos = await inventarioActivo();
        const m = await identificarAutoDeImagen(todas[0], autos);
        if (m && m.nombre) return { texto: (limpio ? limpio + ' ' : '') + m.nombre, imagen: true, auto: m, textoUtil: true };
    } catch (e) { }
    return { texto: limpio || t, imagen: true, textoUtil: !!limpio };
}
...
// ── SURTIDO VARIADO (owner 2026-07-21): si no se identificó ningún auto del
// carrusel, se enseña variedad real — categorías y marcas distintas primero ──
function surtidoVariado(autos, n) {
    const pool = autos.slice().sort((x, y) => (Number(y.sb_convs_count || 0) - Number(x.sb_convs_count || 0)) || (Number(y.created_at || 0) - Number(x.created_at || 0)));
    const sel = [], cats = new Set(), marcas = new Set();
    for (const a of pool) {                                   // ronda 1: categoría y marca nuevas
        if (sel.length >= (n || 9)) break;
        const c = categoriaDe(a), m = normz(a.marca || '');
        if (!cats.has(c) && !marcas.has(m)) { sel.push(a); cats.add(c); marcas.add(m); }
    }
    for (const a of pool) {                                   // ronda 2: marca nueva
        if (sel.length >= (n || 9)) break;
        const m = normz(a.marca || '');
        if (!sel.includes(a) && !marcas.has(m)) { sel.push(a); marcas.add(m); }
    }
    for (const a of pool) { if (sel.length >= (n || 9)) break; if (!sel.includes(a)) sel.push(a); }
    return sel;
}
...
async function redactarUbicaciones(rows) {
    ...
    return {
        ...
        gancho: `¿Cuál te queda mejor para irlo a ver primero? O si gustas te los junto en ${(conPunto.find(x => x.p) || {}).p || 'un solo punto'} y ves los dos de una vez`
    };
}
...
async function descripcionDe(auto) {
    ...
    const fmtDuenos = d => ({ unico_dueno: 'Único dueño', unico: 'Único dueño', dos: '2 dueños', tres: '3 dueños', cuatro_o_mas: '4 o más dueños' }[String(d || '')] || null);
    const cap = s => String(s).charAt(0).toUpperCase() + String(s).slice(1);
    const L = [`🚗 ${auto.nombre}`];
    ...
    { const du = fmtDuenos(w && w.numero_duenos); if (du) car.push('🔵 ' + du); }   // sin dato → no se afirma nada (antes: 'Único dueño' por default)
    if (facturaOk(w && w.factura_original)) car.push('🔵 Factura original');
    else if (w && w.factura_original && !/^\s*(no|sin)\b/i.test(String(w.factura_original))) car.push('🔵 ' + String(w.factura_original).slice(0, 40));
...
// ¿la factura es original/de agencia? (anclado: 'sin factura' NO cuenta como sí)
const facturaOk = v => /^\s*(s[ií]\b|original|de agencia|agencia)/i.test(String(v || ''));
// HECHOS del auto para hablar con verdad (dueños / factura) — por id web de la tabla autos; null si no hay dato
async function hechosDe(webId) {
    try { const r = await query('SELECT numero_duenos, factura_original FROM autos WHERE id=?', [Number(webId || 0)]); const w = r[0]; if (!w) return { duenos: null, factura: null };
        const du = ({ unico_dueno: 'único dueño', unico: 'único dueño', dos: '2 dueños', tres: '3 dueños', cuatro_o_mas: '4 o más dueños' }[String(w.numero_duenos || '')]) || null;
        return { duenos: du, factura: w.factura_original ? (facturaOk(w.factura_original) ? 'ok' : 'no') : null }; } catch (e) { return { duenos: null, factura: null }; }
}
async function fichaExtensa(a) {
    ...
    { const du = ({ unico_dueno: 'Único dueño', unico: 'Único dueño', dos: '2 dueños', tres: '3 dueños', cuatro_o_mas: '4 o más dueños' }[String((w && w.numero_duenos) || '')]); if (du) detalles.push(du); }
    if (facturaOk(w && w.factura_original)) detalles.push('Factura original');
...
function resolverEleccion(texto, aparador, ancla) {
    // los placeholders del puente ([imagen], [mensaje]) y las URLS no son palabras
    // del comprador: se quitan ANTES de comparar. Caso Mar 2026-07-22: "instagram"
    // contiene "ram" y el slug "Da3O..." contiene "a3" — el link disparaba familias.
    const t = normz(String(texto || '').replace(/https?:\/\/\S+/gi, ' ').replace(/\[[^\]]{1,20}\]/g, ' ').replace(/\s+/g, ' ').trim());
    if (!Array.isArray(aparador) || !aparador.length) return null;
    // número ("2", "el 2", "la 3", "opcion 2")
    const mNum = t.match(/(?:^|\b(?:el|la|opci[oó]n|n[uú]mero)\s*)([1-6])\b/) || (/^[1-6]$/.test(t) ? [t, t] : null);
    if (mNum) {
        const it = aparador.find(x => x.n === Number(mNum[1]));
        if (it) return { auto: it, via: 'numero' };
        // no es posición de la lista → ¿es el NÚMERO DEL MODELO? ("el 3" = Mazda 3
        // frente a un CX-5) — hecho duro contra los nombres mostrados
        const porModelo = aparador.filter(x => new RegExp('\\b' + mNum[1] + '\\b').test(normz(x.nombre)));
        if (porModelo.length === 1) return { auto: porModelo[0], via: 'nombre' };
    }
    ...
    for (const it of aparador) {
        ...
        // palabras-dentro-de-palabras (owner 2026-07-23, caso "mínimo"⊃mini): los
        // tokens CORTOS (≤4) solo pegan como PALABRA COMPLETA — jamás por pedazo
        const matched = toks.filter(tok => (/^\d{1,2}$/.test(tok) || tok.length <= 4) ? new RegExp('\\b' + tok + '\\b').test(t) : t.includes(tok));
...
// ══ EL INTERIOR DE LA BÚSQUEDA (owner 2026-07-22): "similar" no es la lista — es
// QUÉ ESTÁ COMPRANDO DE VERDAD. El del BMW compra MARCA (→ Audi/Mercedes/Mazda del
// rango); el de la Sienna compra VIAJAR (→ camionetas del rango); el de la troca
// compra TRABAJO (→ pickups); el del sedán normal compra EFICIENCIA/plataforma
// (→ Toyota/Honda/Nissan/Dodge del rango); el de ultra lujo compra ULTRA LUJO.
const RE_VIAJAR = /\b(sienna|suburban|tahoe|yukon|expedition|teramont|highlander|pilot|odyssey|sequoia|carnival|pacifica|caravan)\b/;
function perfilDe(a) {
    const nom = normz([a.marca, a.nombre].filter(Boolean).join(' '));
    const precio = Number(a.precio || 0);
    if (RE_VIAJAR.test(nom)) return 'viajar';
    if (precio >= 900000) return 'ultra';
    if (MARCAS_PREMIUM.test(nom)) return 'premium';
    const cat = categoriaDe(a);
    if (cat === 'pickup') return 'pickup';
    if (cat === 'sedan') return 'sedan';
    return cat || 'otro';
}
function sonCompatibles(foco, cand) {
    const pf = perfilDe(foco), pc = perfilDe(cand);
    if (pf === pc) return true;
    if (pf === 'premium' && (pc === 'ultra' || normz(cand.marca || '') === 'mazda')) return true;  // su mundo de marca (+ Mazda de entrada)
    if (pf === 'ultra' && pc === 'premium') return true;
    if (pf === 'viajar' && categoriaDe(cand) === 'suv') return true;    // camionetas del rango
    if (pf === 'pickup' && categoriaDe(cand) === 'pickup') return true; // troca es troca (aunque sea ultra)
    return false;
}

function scoreSimilitud(ancla, a) {
    if (!sonCompatibles(ancla, a)) return 0;               // EL INTERIOR manda (perfil de compra)
    ...
    let s = 55 * (1 - dp / 0.35);                          // 2º precio
    if (normz(a.marca || '') && normz(a.marca || '') === normz(ancla.marca || '')) s += 28;   // 3º marca
    if (ancla.anio && a.anio) {
        const da = Math.abs(Number(a.anio) - Number(ancla.anio));
        if (da <= 4) s += 20 * (1 - da / 4);               // 4º año
    }
    return s;
}
const UMBRAL_SIMILITUD = 45;
function similaresA(ancla, autos, max) {
    const mismos = autos.filter(a => a.id !== ancla.id && sonCompatibles(ancla, a));
    ...
    // se completa con el MISMO MUNDO por cercanía de precio — pero DEL RANGO
    // ("no tan elevado", owner 2026-07-22): el relleno jamás pasa de ~2.2× ni baja
    // de ~0.35× del foco. Si quedan pocos, mejor lista corta que basura.
    const pb2 = Number(ancla.precio || 0);
    const resto = mismos.filter(a => !buenos.includes(a) && (!pb2 || (Number(a.precio || 0) <= pb2 * 2.2 && Number(a.precio || 0) >= pb2 * 0.35)))
        .sort((x, y) => Math.abs(Number(x.precio || 0) - pb2) - Math.abs(Number(y.precio || 0) - pb2));
    return buenos.concat(resto).slice(0, max || 7);
}

// ══ RELACIONADOS — SOLO si lo piden explícito ══
// "¿tienes más FOTOS?" es del MISMO auto, no otros autos (owner 2026-09-26: le mandaron 4 autos distintos)
const RE_MAS_DEL_AUTO = /(m[aá]s|otras?)\s+(fotos?|fotitos|im[aá]genes|imagen|videos?|detalles?|info|informaci[oó]n|datos|caracter[ií]sticas|kilometraje|km|papeles|documentos|especificaciones)|fotos?\s+m[aá]s|m[aá]s\s+de\s+(cerca|adentro|dentro)/i;
...
async function intentarEleccionAparador(tel, texto, convId) {
    try {
        const stE = await U.leerEstado(0, tel);
        if (!stE.existe) return null;
        const ej = stE.ej;
        if (!Array.isArray(ej.aparador) || !ej.aparador.length || stE.auto_id_activo) return null;
        if (RE_RELACIONADOS.test(texto) && !RE_MAS_DEL_AUTO.test(texto)) {
            ...
            if (apr2) {
                ...
                await U.guardarEstado(0, tel, { estado_json: ej });
        ...
        // lo NOMBRADO manda sobre una pregunta pendiente vieja
        if (Array.isArray(ej.pregunta) && ej.pregunta.length) {
            const invF = await inventarioActivo();
            const rF = resolverEleccion(texto, invF.map(a => ({ n: 0, id: a.id, nombre: a.nombre, color: '' })));
            if (rF && rF.auto && rF.via === 'nombre' && !ej.pregunta.includes(rF.auto.id)) { delete ej.pregunta; delete ej.pregunta_accion; delete ej.pregunta_ya; }
        }
        // la base de una pregunta VIVA son SUS candidatos completos (del inventario) —
        // intersectarla con la lista mostrada tiraba a los que no salían en ella y el
        // año se iba a buscar al aparador (caso "el 2018" → agarraba el Mazda)
        let base = ej.aparador;
        if (Array.isArray(ej.pregunta) && ej.pregunta.length) {
            const invB = await inventarioActivo();
            const itemsP = ej.pregunta.map((id, i) => { const a = invB.find(x => x.id === id); return a ? { n: i + 1, id: a.id, nombre: a.nombre, color: normz(a.color || '') } : null; }).filter(Boolean);
            if (itemsP.length) base = itemsP;
        }
        ...
        if (multi && multi.length >= 2) {
            ...
            await U.guardarEstado(0, tel, { estado_json: ej, auto_id_activo: multi[0].id });
        ...
        if (!el) return null;
        // "me pareció ver un sentra 2021" con lista sin Sentra: el AÑO empataba a los
        // mostrados y preguntaba mal — si el texto NOMBRA un auto del inventario que
        // no está en lo mostrado, la elección se hace a un lado (la mesa lo sienta)
        if (el.pregunta && el.via === 'anio_ambiguo') {
            try {
                const invA = await inventarioActivo();
                const rNombre = resolverEleccion(texto, invA.map(a => ({ n: 0, id: a.id, nombre: a.nombre, color: '' })));
                if (rNombre && rNombre.auto && rNombre.via === 'nombre') return null;
            } catch (e) { }
        }
        if (el.pregunta) {
        ...
        const auto = el.auto;
        const autosA = await inventarioActivo();
        // ══ LA MESA TAMBIÉN VOTA (owner 2026-07-22, caso "el sentra" con DOS Sentra):
        // si el nombre dicho es FAMILIA (varios en inventario) y OTRO de esa familia ya
        // está SENTADO en la mesa, jamás se adivina el de la lista → se pregunta cuál.
        if (el.via === 'nombre') {
            try {
                const { candidatosDeAuto } = require('./clasificador.js');
                const fam = candidatosDeAuto(String(texto || ''), autosA.map(a => ({ id: a.id, nombre: a.nombre, precio: a.precio })));
                // la mesa Y el interés reciente votan (más-opciones suelta la mesa
                // pero el auto platicado queda como interés — sigue vivo para él)
                const mesaIds = (Array.isArray(ej.mesa) ? ej.mesa : []).concat(Array.isArray(ej.interes) ? ej.interes.slice(-3) : []).map(Number);
                if (fam && fam.length > 1 && fam.some(c => mesaIds.includes(c.id) && c.id !== auto.id)) {
                    ej.mesa_familia = fam.slice(0, 4).map(c => c.id);
                    ej.pregunta = ej.mesa_familia.slice();
                    await U.guardarEstado(0, tel, { estado_json: ej });
                    return { tipo: 'aparador_pregunta', segmentos: ['Tenemos estos — ¿de cuál me dices?\n' + fam.slice(0, 4).map((x, i) => `${i + 1}) ${x.nombre}`).join('\n')] };
                }
            } catch (e) { }
        }
        ...
        ej.escena = [auto.id]; // LA ESCENA: lo que se le va a enseñar AHORITA
        ej.interes = (ej.interes || []).filter(x => x !== auto.id).concat([auto.id]);
        delete ej.aparador; delete ej.dudas_pendientes; delete ej.pregunta;
        await U.guardarEstado(0, tel, { estado_json: ej, auto_id_activo: auto.id });
...
async function opcionesEnFlujo({ tel, texto }) {
    try {
        const pideMas = RE_RELACIONADOS.test(texto) && !RE_MAS_DEL_AUTO.test(texto);
        ...
        const stO = await U.leerEstado(0, tel); const ej = stO.ej;
        const autos = await inventarioActivo();
        const escF = Array.isArray(ej.escena) ? ej.escena : [];
        const mesaF = Array.isArray(ej.mesa) ? ej.mesa : [];
        const intF = Array.isArray(ej.interes) ? ej.interes : [];
        const focoId = Number(stO.auto_id_activo || (ej.foco && ej.foco.id) || escF[escF.length - 1] || mesaF[mesaF.length - 1] || intF[intF.length - 1] || 0) || null;
        ...
        if (crit) {
            ...
            if (!lista || !lista.length) {
                // sin foco NO hay "similar" — variedad honesta, jamás el vaciadero
                lista = surtidoVariado(autos.filter(a => a.id !== focoId && !interes.includes(a.id)), 9);
                intro = 'Claro, mira, te paso algo de lo que tenemos 🚗';
                listaOrdenada = true;
            }
            if (!lista.length) return null;
        ...
        await U.guardarEstado(0, tel, { estado_json: ej, auto_id_activo: null, estado: 'aparador' });
...
async function arranqueCarrusel({ tel, textoRaw, textoFamilia, adCtx, textosIn, nombre, esClick, duda }) {
    ...
    if (esClick) {
        ...
        try {
            ...
            if (rNom && (rNom.auto || rNom.pregunta)) {
                // ⚖️ EL JUEZ DE DISPARO (owner 2026-07-22): el matcher disparó en un
                // PRIMER CONTACTO — evidencia que puede ser falsa. El juez razona; si
                // dice "no lo nombró", el clic sigue como genérico (variedad/ancla).
                // Sin juez o juez confirma → comportamiento determinista de siempre.
                const { juezNombroAuto } = require('./juez.js');
                const jN = await juezNombroAuto({ texto: textoRaw, candidatos: rNom.auto ? [rNom.auto] : rNom.pregunta }).catch(() => null);
                if (!jN || jN.nombro !== false) return null;
            }
        } catch (e) { }
        ...
        // ══ LEY DEL OPENER (owner 2026-07-22): un link de POST (instagram.com/p/…)
        // apunta al carrusel COMPLETO — el ojo ahí solo ve la PORTADA, jamás el clic
        // (probado: 7 personas del mismo post y 0 querían la portada). Sin certeza →
        // sin ancla. Los links de TARJETA (fb.me por tarjeta) SÍ son el clic exacto.
        const esLinkTarjeta = urlAd && !/instagram\.com\/p\//i.test(urlAd);
        if (esLinkTarjeta) ancla = (await identificarAncla(urlAd, autosAp)).ancla;
        if (ancla) {
            // ══ LEY DEL OPENER: clic EXACTO con certeza → EL FLUJO de ese auto, como
            // si lo hubiera nombrado — él lo picó: número uno con su fuegito 🔥.
            // JAMÁS comparables del ancla (mala idea — orden owner 2026-07-22).
            try {
                const { paqueteAuto, guardarMesa } = require('./mesa.js');
                const rowA = autosAp.find(a => a.id === ancla.id) || ancla;
                const ejA = (await U.leerEstado(0, tel)).ej;
                ejA.escena = [rowA.id];
                await guardarMesa(tel, ejA, [rowA.id], rowA.id);
                const pk = await paqueteAuto(rowA, { confirma: `Claro que sí — este es el del anuncio 🔥, te paso todo del ${rowA.nombre}:` });
                return { tipo: 'opener_clic_paquete', segmentos: [`Qué tal${nm ? ' ' + nm : ''} ${saludoHora()}!`, 'Mucho gusto, mi nombre es Sebastián Romero, para servirte'].concat(pk.segmentos), fotos: pk.fotos, fotos_after_index: (pk.fotos_after_index != null ? pk.fotos_after_index : 0) + 2 };
            } catch (e) { console.error('[opener clic]', e.message); }
        }
        // ══ NIVEL 2 (owner 2026-08-06): sin certeza del clic → CERO surtido, CERO
        // rollo. Pregunta DIRECTA con el machote del owner — él suelta el auto y el
        // seguimiento queda fijo. (El spam de "te paso lo que tenemos" MURIÓ.)
        return {
            tipo: 'opener_pregunta_directa',
            segmentos: [
                'Hola, cómo estás!',
                'Mucho gusto, mi nombre es Sebastián Romero',
                `Claro que sí${nm ? ' ' + nm : ''}, para poderte ayudar — ¿cuál es el auto que te interesa amigo?`
            ]
        };
    }
    ...
    if (crit) {
        ...
        // LEY DEL POST (owner 2026-07-22): un link de POST de Instagram apunta al
        // carrusel COMPLETO — el ojo ahí solo ve la PORTADA, jamás el clic (0/7
        // querían la portada). Post de IG = SIN certeza → directo a nivel 2.
        const esTarjeta3 = !/instagram\.com\/p\//i.test(urlAd);
        ancla = esTarjeta3 ? (await identificarAncla(urlAd, autosAp)).ancla : null;
        // ══ LEY BINARIA DEL OPENER (owner 2026-08-06) ══════════════════════
        // NIVEL 1: el anuncio amarra UN auto con certeza → el paquete de ESE auto
        // (seguimiento concreto; si su interés era otro, él lo dice sin tema).
        if (ancla) {
            try {
                const { paqueteAuto, guardarMesa } = require('./mesa.js');
                const rowA = autosAp.find(a => a.id === ancla.id) || ancla;
                ...
                const pk = await paqueteAuto(rowA, { confirma: `Claro que sí — este es el del anuncio 🔥, te paso todo del ${rowA.nombre}:` });
                return { tipo: 'opener_clic_paquete', segmentos: [`Qué tal${nm ? ' ' + nm : ''} ${saludoHora()}!`, 'Mucho gusto, mi nombre es Sebastián Romero, para servirte'].concat(pk.segmentos), fotos: pk.fotos, fotos_after_index: (pk.fotos_after_index != null ? pk.fotos_after_index : 0) + 2 };
            } catch (e) { console.error('[opener nivel1]', e.message); }
        }
        // NIVEL 2: sin certeza → pregunta DIRECTA (el surtido del anuncio MURIÓ)
        return {
            tipo: 'opener_pregunta_directa',
            segmentos: [
                'Hola, cómo estás!',
    ...
    const apr = await armarAparador({ ancla, lista, saludoNombre: nm, intro, max: ancla ? 8 : 6, nFotos: 4, ordenDado: !!ancla || !!crit });
    if (!apr) return null;
