...
async function ejecutar({ tel, herramientas, mesaRows, ej, convId, texto }) {
    ...
    const outFinal = { tipo: 'perro_' + (tipoOut.join('+') || 'mix'), segmentos, fotos, fotos_after_index: fotosIdx, pregunta_viva: preguntaViva, pregunta_target: preguntaTarget };
...
async function rutear({ tel, texto, historial, convId }) {
    try {
        ...
        { const ultOut = (String(historial || '').split('\n').filter(l => /^(SEB|Seb|Vendedor|VENDEDOR|Lote|BOT)/.test(l)).pop() || '');
          const RE_PIDE_FOTOS = /(foto|im[aá]gen|video|v[ií]deo|por dentro|por fuera|interior|exterior|c[oó]mo (se ve|luce|est[aá] de)|ens[eé][ñn]a|mu[eé]stra|verlo por aqu[ií])/i, RE_PIDE_UBIC = /(d[oó]nde|ubicaci|direcci|domicilio|zona|colonia|sucursal|mapa|\bpin\b|c[oó]mo llego|por d[oó]nde|queda (lejos|cerca)|en qu[eé] parte)/i;
