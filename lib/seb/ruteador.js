...
const SCHEMA = {
    type: 'object',
    properties: {
        razon: { type: 'string', description: 'UNA frase: qué quiere de verdad el comprador, leyendo su mensaje como RESPUESTA a lo último que le dijo el vendedor' },
        es_respuesta_sin_peticion: { type: 'boolean', description: 'true si el mensaje solo contesta / aplaza / piensa la última pregunta del vendedor y NO pide nada ("déjame ver", "deja checo", "ahorita te digo", "lo veo con mi esposa", "sí", "no sé")' },
        herramientas: {
            type: 'array',
            items: {
                type: 'object',
    ...
    required: ['razon', 'es_respuesta_sin_peticion', 'herramientas', 'confianza'],
    additionalProperties: false
};

const REGLAS_HERRAMIENTAS = `HERRAMIENTAS (elige 0 a 3, en orden; NO redactas nada):
...
REGLAS DE ORO:
- CONTEXTO PRIMERO: lee el mensaje como RESPUESTA a lo ÚLTIMO que dijo el vendedor en el historial. Si el vendedor acaba de preguntar algo ("¿a qué hora te coordinamos una cita?", "¿gustas venir?") y el comprador contesta algo corto que NO pide nada ("déjame ver", "deja checo", "ahorita te digo", "lo platico con mi esposa", "luego te aviso") → es_respuesta_sin_peticion = true y herramientas: []. "Ver" en "déjame ver" es "déjame pensarlo", NO "quiero ver fotos".
- Una herramienta se activa SOLO cuando el comprador PIDE esa cosa con sus palabras (fotos, ubicación, cuánto queda, qué más tienes). Jamás por una palabra suelta.
...
const SYSTEM = `Eres el RUTEADOR de un vendedor de autos usados en Monterrey. Te dan el mensaje del comprador, los autos que trae en la mesa y el historial breve. Tu ÚNICO trabajo: elegir qué herramientas ejecutar.\n\n` + REGLAS_HERRAMIENTAS;
...
async function ejecutar({ tel, herramientas, mesaRows, ej, convId, texto }) {
    ...
    let segmentos = [], fotos = null, fotosIdx = null, tipoOut = [], escala = null, preguntaViva = null, preguntaTarget = null; let ubicacionAutoId = null, pinIdx = null;
    ...
    for (const h of herramientas) {
        ...
        if (h.tool === 'traer_inventario') {
            // SEÑAL EXPLÍCITA O NADA (owner 2026-07-23, caso JHONNY): traer inventario
            // SOLO si lo PIDIÓ con sus letras — la IA jamás lo decide por inferencia.
            // Sin señal → la herramienta se descarta y el turno ESCALA (ambiguo = tuyo).
            {
                const tG = String(texto || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '');
                const señalExp = (ap.RE_RELACIONADOS.test(tG) && !(ap.RE_MAS_DEL_AUTO && ap.RE_MAS_DEL_AUTO.test(tG))) || ap.RE_NECESIDAD.test(tG) || ap.RE_BARATO.test(tG)
                    || /(que (autos|carros|opciones|camionetas|pickups?|suvs?|sedanes) (tienes|hay|manejan|tienen)|inventario|qu[eé] m[aá]s (tienes|hay)|en adelante|a partir de|ando buscando|busco (un|una)|no tienes (un|una)|tienes (un|una))/.test(tG)
                    || (h.enganche && /enganche/.test(tG)) || (h.presupuesto && /presupuesto|traigo|tengo/.test(tG));
                if (!señalExp) {
                    escala = escala || '🔴 la IA quería enseñar inventario SIN que él lo pidiera con sus letras ("' + String(texto || '').slice(0, 70) + '") — lo ves tú';
                    continue;
                }
            }
            ...
            if (fueraP) {
                ...
                if (altP.length) {
                    ...
                    if (aprP) {
                        ...
                        await U.guardarEstado(0, tel, { auto_id_activo: null }).catch(() => { });
            ...
            if (!eng) {
                // LA RÁFAGA DE DOS PREGUNTAS (caso Ezequiel): el km pedido no se traga
                try {
                    const { datoRapido } = require('./mesa.js');
                    const dqP = objs.length === 1 ? datoRapido(texto, objs[0]) : null;
                    if (dqP) segmentos.push(dqP);
                } catch (e) { }
                // "¿cuánto es el MÍNIMO de enganche?" no es "cotízame": se contesta el mínimo real del auto por año (owner 2026-09-26: le regresaba la pregunta)
                const tMin = String(texto || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
                if (objs.length === 1 && /(minimo|lo menos|desde cuanto|cuanto (es|piden|pide|seria|sale) (de |el )?(enganche|inicial)|cual es el enganche|de cuanto es el enganche|con cuanto (se puede|puedo|lo saco|minimo))/.test(tMin)) {
                    const rM = await require('./herramientas.js').enganche_minimo({ auto_id: objs[0].id }).catch(() => null);
                    if (rM && rM.ok) { segmentos.push('El enganche mínimo para este auto es de ' + rM.placeholders.enganche_minimo + '; de ahí para arriba el que gustes'); segmentos.push('Si me dices con cuánto le entrarías, te corro los números'); tipoOut.push('enganche_minimo'); continue; }
                }
            ...
            if (objs.length === 1 && herramientas.length >= 2) { const p = await ap.puntoHablado(objs[0].id); segmentos.push(p ? `Lo tenemos en ${p}` : 'Aquí te mando la ubicación'); ubicacionAutoId = objs[0].id; pinIdx = segmentos.length - 1; tipoOut.push('ubicacion'); continue; }   // pedida JUNTO con otra cosa ("fotos y dónde están"): el perro manda el pin aquí mismo, no se pierde
            ...
            for (const r of objs) {
                ...
                if (!linea) {
                    // dato que NO está en la base → jamás inventar. PERO no se mata el
                    // turno: lo que SÍ se puede servir (fotos, ubicación…) igual sale y
                    // solo el dato faltante se escala (orden owner 2026-07-21).
                    escala = `pregunta ${h.campo} del ${r.nombre} y no está en la ficha — lo ves tú`;
                    continue;
                }
    ...
    if (!segmentos.length) {
        if (escala) return { tipo: 'perro_dato_faltante', segmentos: null, escalar_owner: true, escala_motivo: escala };
        return null;
    }
    ...
    const outFinal = { tipo: 'perro_' + (tipoOut.join('+') || 'mix'), segmentos, fotos, fotos_after_index: fotosIdx, ubicacion_auto_id: ubicacionAutoId, pin_after_index: pinIdx, pregunta_viva: preguntaViva, pregunta_target: preguntaTarget };
    if (escala) { outFinal.escalar_owner = true; outFinal.escala_motivo = escala; }   // se sirve lo que hay + te aviso del faltante
    return outFinal;
}
...
async function datoFicha(r, campo) {
    ...
    // REGLA DEL NEGOCIO (owner 2026-07-21): todos son ÚNICO DUEÑO y FACTURA ORIGINAL
    // salvo que la ficha diga expresamente otra cosa.
    if (campo === 'duenos') { const v = String((w && w.numero_duenos) || ''); const m = { dos: 'Ha tenido 2 dueños', tres: 'Ha tenido 3 dueños', cuatro_o_mas: 'Ha tenido 4 o más dueños' }[v]; return m || 'Único dueño 👍'; }
    if (campo === 'factura') { const v = w && w.factura_original; if (v && !/si|s[ií]|original|agencia/i.test(String(v))) return String(cap(v)).slice(0, 60); return 'Factura original 👍'; }
    return null;
}
...
async function rutear({ tel, texto, historial, convId, clasif }) {   // clasif = lo que ya leyó el LECTOR ÚNICO (clasificador) en este turno: trae herramientas y objeción → aquí no se vuelve a llamar a la IA
    try {
        ...
        // 🔴 OBJECIÓN DE PAGO (owner 2026-07-23, caso Ezequiel "se me ase algo alta la
        // mensualidad" → el perro re-cotizó IDÉNTICO): un juicio de costo JAMÁS se
        // contesta con otra tarjeta — es momento de gol → ESCALA (doctrina).
        try {
            const { RE_RIESGO, RE_CONTADO } = require('./doctrina.js');
            const tN = t.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '');
            if (RE_RIESGO.test(tN)) {
                return { escalar_owner: true, tipo: 'riesgo_pago', segmentos: [], escala_motivo: '🔴 VENTA EN RIESGO — juicio de costo ("' + t.slice(0, 80) + '") — lo ves tú' };
            }
            // CONTADO = dinero/política: el bot se calla y ESCALA (jamás una tarjeta)
            if (RE_CONTADO.test(tN)) {
                return { escalar_owner: true, tipo: 'contado', segmentos: [], escala_motivo: '💰 habla de CONTADO ("' + t.slice(0, 80) + '") — precio/forma de pago es tuyo' };
            }
            // ══ JUEZ DE OBJECIÓN (owner 2026-07-23, caso JHONNY "sevmvhacevla"): el
            // regex no ve typos ni orden invertido. Si hay palabras de DINERO y el
            // regex no decidió, la IA razona: ¿objeta el costo o pide números?
            // LA LEY: obvio ejecuta, ambiguo ESCALA.
            const RE_DINERO = /(mensualidad|enganche|precio|pago mensual|tasa\b|car[oa]\b|alt[oa]\b|elevad|alcanz|presupuesto)/;
            if (RE_DINERO.test(tN) && !/^\s*[\d.,]+\s*(mil|k)?\s*$/.test(tN)) {
                try {
                    const { juezObjecion } = require('./juez.js');
                    const jO = (clasif && clasif.objecion && clasif.objecion !== 'no_aplica') ? { veredicto: clasif.objecion } : await juezObjecion(t);   // el lector único ya lo juzgó
                    if (jO && jO.veredicto !== 'peticion_clara') {
                        return { escalar_owner: true, tipo: 'riesgo_pago', segmentos: [], escala_motivo: '🔴 ' + (jO.veredicto === 'objecion' ? 'OBJECIÓN de costo' : 'mensaje de dinero NO claro') + ' ("' + t.slice(0, 80) + '") — lo ves tú' };
                    }
                } catch (e) { }
            }
        } catch (e) { }
        // la mesa del comprador (para que el perro sepa qué autos trae en juego)
        const stR = await U.leerEstado(0, tel); const ej = stR.ej;
        let mesa = Array.isArray(ej.mesa) ? ej.mesa.map(Number).filter(Boolean) : [];
        if (!mesa.length && stR.auto_id_activo) mesa = [Number(stR.auto_id_activo)];
        const ap = require('./aparador.js');
        const autos = await ap.inventarioActivo();
        ...
        const out = (clasif && Array.isArray(clasif.herramientas)) ? { herramientas: clasif.herramientas, confianza: clasif.confianza === 'alta' ? 'alta' : 'baja', es_respuesta_sin_peticion: clasif.es_respuesta_sin_peticion === true } : await haiku(SYSTEM, SCHEMA, contexto, 400);   // lector único: cero llamadas extra
        let hs = validar(out);
        if (!hs) return null;
        if (out.es_respuesta_sin_peticion === true) return null;   // solo contestó/aplazó nuestra pregunta: ningún botón
        // FOTOS y UBICACIÓN mandan archivos: solo con petición EXPLÍCITA en el mensaje (o si lo último nuestro fue justo ofrecerlas)
        { const ultOut = (String(historial || '').split('\n').filter(l => /^(SEB|Seb|Vendedor|VENDEDOR|Lote|BOT)/.test(l)).pop() || '');
          const { RE_PIDE_FOTOS, RE_PIDE_UBIC } = require('./regex-comunes.js');
          hs = hs.filter(h => !(h.tool === 'fotos' && !RE_PIDE_FOTOS.test(t) && !/foto|im[aá]gen|video/i.test(ultOut)) && !(h.tool === 'ubicacion' && !RE_PIDE_UBIC.test(t) && !/ubicaci/i.test(ultOut)));
          if (!hs.length) return null; }
        ...
        if (res.segmentos && res.segmentos.length) {
            ...
            await U.guardarEstado(0, tel, { estado_json: ej }).catch(() => { });
        }
...
module.exports = { REGLAS_HERRAMIENTAS, SCHEMA_HERRAMIENTAS: SCHEMA.properties.herramientas, rutear, validar, datoFicha, HERRAMIENTAS };
