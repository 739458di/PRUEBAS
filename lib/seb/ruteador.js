    ...
    properties: {
        herramientas: {
            ...
            items: {
                ...
                required: ['tool', 'enganche', 'presupuesto', 'plazo_meses', 'tipo', 'marca', 'anio_min', 'anio_max', 'similar_al_foco', 'economicos', 'campo', 'objetivo'],
...
HERRAMIENTAS:
- traer_inventario: pide ver autos del inventario — "qué similares tienes", "con ese enganche qué me alcanza", "algo más barato", "una camioneta", "qué más hay", "modelo a partir de 2022"/"2022 en adelante" (anio_min), "no más viejo que…". Params: enganche/presupuesto (pueden venir del HISTORIAL si dice "ese enganche" — y si antes dio presupuesto/año, esos criterios SIGUEN vivos: inclúyelos), tipo, marca, anio_min, anio_max, similar_al_foco (parecidos al auto que ya ve), economicos (pide lo más barato).
...
async function ejecutar({ tel, herramientas, mesaRows, ej, convId }) {
    ...
    let segmentos = [], fotos = null, fotosIdx = null, tipoOut = [], escala = null, preguntaViva = null;
    ...
    for (const h of herramientas) {
        ...
        if (h.tool === 'traer_inventario') {
            ...
            if (h.economicos && !hayCrit) {
                ...
                if (!lista.length) {
                    return { tipo: 'perro_sin_match', segmentos: ['Fíjate que con eso exacto ahorita no tengo algo que te acomode 🙏'], escalar_owner: true, escala_motivo: '💰 busca con criterio y no hay match en inventario — lo ves tú' };
                }
            ...
            if (objs.length >= 2 && h.objetivo === 'todos') {
                ...
                const fs = await ap.fotosAuto(objs[0].fyradrive_web_id, 5);
    ...
    return { tipo: 'perro_' + (tipoOut.join('+') || 'mix'), segmentos, fotos, fotos_after_index: fotosIdx, pregunta_viva: preguntaViva };
}
...
async function rutear({ tel, texto, historial, convId }) {
    try {
        ...
        if (!hs) return null;
        const res = await ejecutar({ tel, herramientas: hs, mesaRows, ej, convId });
        if (!res) return null;
