...
const TOOLS_DECL = [
    ...
    {
        name: 'cotizar',
        description: 'Cotización ESPECÍFICA con HEY Banco cuando el comprador YA dio su enganche (en pesos O en porcentaje, ej. "30%") y/o plazo. Devuelve la tarjeta formal en el hueco {{cotizacion}} — respóndela tal cual. Si el comprador dio el enganche como % (ej. "al 30%"), pásalo en enganche_pct. Si NO dio ningún enganche y solo quiere VER planes/opciones, usa la herramienta planes (no cotizar).',
        input_schema: { type: 'object', properties: { auto_id: { type: 'integer' }, enganche: { type: 'number', description: 'enganche en PESOS que dio el comprador' }, enganche_pct: { type: 'number', description: 'enganche en PORCENTAJE si el comprador lo dio así (ej. 30 para "30%")' }, plazo_meses: { type: 'integer', description: 'plazo en meses si el comprador lo dijo (opcional)' } }, required: ['auto_id'] }
    },
    {
        name: 'planes',
        description: 'TABLA de opciones de financiamiento (varios enganches × varias mensualidades) con HEY Banco. Úsala cuando el comprador quiere VER los planes / cómo quedaría / está explorando y NO te ha dado un enganche fijo. En vez de preguntarle, te ADELANTAS y le mandas las opciones para que elija o dé su enganche. Devuelve el hueco {{planes}} — respóndelo tal cual.',
        input_schema: { type: 'object', properties: { auto_id: { type: 'integer' } }, required: ['auto_id'] }
    }
];
const TOOL_IMPL = { ubicacion: H.ubicacion, autos_activos: H.autos_activos, info_auto: H.info_auto, cotizar: H.cotizar, enganche_minimo: H.enganche_minimo, planes: H.planes };
...
async function pensar({ telefono, mensaje, clasificacion, estado }) {
    ...
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
        ...
        for (const tu of toolUses) {
            ...
            // El modelo NO ve los placeholders (esos van al validador). Si la tool
            // devolvió huecos listos (p.ej. {{cotizacion}}), se lo decimos EXPLÍCITO
            // para que los use tal cual y no ofrezca "armarlo después".
            let contenido;
            if (out.ok) {
                contenido = Object.assign({}, out.datos || {});
                const huecosListos = out.placeholders ? Object.keys(out.placeholders).filter(k => out.placeholders[k] != null && out.placeholders[k] !== '') : [];
                if (huecosListos.length) {
                    contenido._huecos_listos = huecosListos.map(h => '{{' + h + '}}');
                    contenido._instruccion = 'Estos huecos YA están listos y respaldados — úsalos TAL CUAL en tu respuesta. NO digas que lo armarás después: ya está listo.';
                }
            } else {
                contenido = { error: out.error, escalar: out.escalar || false };
            }
            resultados.push({ type: 'tool_result', tool_use_id: tu.id, content: JSON.stringify(contenido) });
        }
