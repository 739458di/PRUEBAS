...
function cuandoCorto(c, now, conFecha) {   // conFecha: "el jueves 1 de octubre" (más allá de mañana "el viernes" a secas confunde un viernes en la noche)
    const rel = d => d === ymd(now) ? 'hoy' : (d === masDias(ymd(now), 1) ? 'mañana' + (conFecha ? ' ' + DIAS[dow(at(d, 12, 0))] : '') : 'el ' + (conFecha ? diaLargo(at(d, 12, 0)) : DIAS[dow(at(d, 12, 0))]));
...
const TXT = {
    ...
    vispera_hora: x => 'Qué tal' + x.n + ' ' + x.saludo + ', espero que te encuentres bien||Mañana nos vemos ' + x.alas + ' para ver ' + x.el + ', acá te esperamos||Nos hablamos mañana',
    ...
    dia_hora: x => 'Qué tal' + x.n + ' ' + x.saludo + ', cómo estás?||Hoy nos vemos ' + x.alas + ' para ver ' + x.el + ', acá te esperamos||Me avisas cuando vengas',
...
function slots(t, c, now) {
    ...
    const hL = L(now).getUTCHours(); const saludo = hL < 5 ? 'buenas noches' : (hL < 12 ? 'buen día' : (hL < 19 ? 'buenas tardes' : 'buenas noches'));
...
function planear(t, c, now) {
    if (Number(cfgDe(t).tres_partes) === 1) return [];   // 3 PARTES (Autos Fyradrive): los recordatorios y el día D los lleva cita3.js (owner 2026-09-26)
...
const { RE_RELLENO } = require('./regex-comunes.js');
...
async function leer({ t, c, now, historial, nuevo, auto, foto }) {
    ...
    // RELLENO SIN CONTENIDO ("mmm", "ah", "jaja", "..."): no dice nada de la visita → ni se lee con IA ni se contesta (orden owner 2026-09-22: el bot solo contesta lo que avanza la venta)
    if (RE_RELLENO.test(String(nuevo || '').trim())) return { evento: 'no_es_de_cita', confianza: 'alta', razon: 'compuerta: relleno sin contenido (no se contesta)', relleno: true, pregunta_comercial: '' };
    const candidatoFisico = RE_LLEGUE.test(nuevo) || RE_YAVOY.test(nuevo);
    // PREGUNTA PENDIENTE (ley 2, owner 2026-09-26: "checo mis horarios" se contestaba sin leer): si lo último que dijo el lote fue preguntarle día/hora, su respuesta SIEMPRE se lee con IA
    const ultLote = [...(historial || [])].reverse().find(h => h && h.quien === 'lote'); const pregCita = !!(ultLote && /(qu[eé] d[ií]a|a qu[eé] hora|qu[eé] horario|te (acomoda|queda bien|viene bien)|cu[aá]ndo (te|puedes|vienes|gustas))/i.test(String(ultLote.texto || '')));
    if (!pregCita && (!c || c.estado === 'pospuesta') && !candidatoFisico && !RE_TIEMPO.test(nuevo) && !/\b(ya no|cancel|compr[eé])\b/i.test(nuevo)) return { evento: 'no_es_de_cita', confianza: 'alta', razon: 'compuerta: sin visita con cuándo y sin palabra de tiempo/visita', pregunta_comercial: '' };   // SIN CUÁNDO = fuera del subsistema: se platica normal (precio, crédito…)
    const hoy = ymd(now);
    ...
    const content = 'AUTO EN FOCO: ' + (auto && auto.nombre ? auto.nombre : 'ninguno') + '\n' +
        ...
        (foto ? 'TRATO (cita de 3 partes — manda sobre cualquier fecha anterior): ' + foto + '\n' : '') +
...
async function entrante0({ tenant, chat, auto, io, textoNuevo, ahoraFijo, comercial, interceptar, foto, _T }) {
    ...
    const ev = await leer({ t, c: c0, now, historial, nuevo, auto, foto });   // la IA lee UNA vez; lo que sigue es código y se puede reintentar sin volver a leer
    ...
    // "ya vi, puedo el martes": un cuándo FUTURO jamás es "ya fue" (el lector confunde "ya vi" con "ya fui"); solo cuenta como ya_fue con un pasado explícito
    { const nF = nuevo.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''); if (ev.evento === 'ya_fue' && !/\b(ya fui|si fui|fui ayer|ayer (pase|fui)|ya pase|pase a verlo|ya lo fui a ver|ya fuimos)\b/.test(nF) && (/\b(puedo|podria|puedes|voy|ire|iria|paso|pasaria|caigo|nos vemos|te veo)\b/.test(nF) || (/^\d{4}-\d{2}-\d{2}$/.test(String(ev.dia_ini || '')) && String(ev.dia_ini) >= ymd(now)))) { ev.evento = 'agenda_o_cambio'; TZ_.ajustes.push('Trae un cuándo futuro: no puede ser "ya fue", es una nueva fecha'); if (!/^\d{4}-\d{2}-\d{2}$/.test(String(ev.dia_ini || ''))) { const m = nF.match(/\b(hoy|manana|lunes|martes|miercoles|jueves|viernes|sabado|domingo)\b/); if (m) { const DN = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado']; let d = ymd(now); if (m[1] === 'manana') d = masDias(d, 1); else if (m[1] !== 'hoy') { for (let i = 1; i <= 7; i++) { const dd = masDias(ymd(now), i); if (DN[dow(at(dd, 12, 0))] === m[1]) { d = dd; break; } } } ev.dia_ini = d; ev.dia_fin = ''; } } } }
    // HORA EXPLÍCITA (sim3 2026-09-26: "A las 11 de la mañana" leído como 'confirma' → la hora no se aplicaba y se le volvía a pedir): si la visita aún no tiene hora
    // y el mensaje trae una hora clara, ES la hora — diga lo que diga la etiqueta
    if (c0 && c0.precision !== 'hora' && ['confirma', 'incierto', 'otra_cosa', 'promete_avisar'].includes(ev.evento)) {
        const nH = nuevo.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''); const mH = nH.match(/\b(?:a las|alas|a la|como a las|tipo)\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm|de la manana|en la manana|de la tarde|en la tarde|de la noche)?\b/);
        if (mH && !/\b(no puedo|no alcanzo|no voy|ya no|mejor otro|otro dia)\b/.test(nH)) { let h = Number(mH[1]); const suf = mH[3] || ''; if (/pm|tarde|noche/.test(suf) && h < 12) h += 12; else if (!suf && h >= 1 && h <= 7) h += 12;
            if (h >= 0 && h <= 23) { ev.evento = 'agenda_o_cambio'; ev.hora_ini = String(h).padStart(2, '0') + ':' + (mH[2] || '00'); ev.hora_fin = ''; ev.franja = ''; ev.dia_fin = ''; if (!/^\d{4}-\d{2}-\d{2}$/.test(String(ev.dia_ini || ''))) ev.dia_ini = ymd(Number(c0.ini_ts)); TZ_.ajustes.push('Trae hora explícita (' + ev.hora_ini + ') y la visita no tenía hora: es la hora'); } }
    }
    ...
    // DÍAS QUE YA PASARON (orden owner 2026-09-26): la IA no decide esto, lo decide el código con la hora y el horario.
    //  · "fin de semana" = lo que QUEDA de viernes, sábado y domingo (un día cuenta si todavía no cierra); si ya no queda nada → el próximo fin de semana.
    //  · un día de la semana que es HOY pero ya cerró (o está por cerrar) = el de la PRÓXIMA semana ("viernes" dicho el viernes a las 10 pm).
    {
        const hoyD = ymd(now); const vivo = d => { if (d < hoyD) return false; const ac = abreCierra(t, d); if (!ac) return false; return d > hoyD || now < ac.cierra - 30 * MIN; };
        const nomD = String(nuevo || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        const DIAN = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado']; const nombrados = DIAN.filter(x => new RegExp('\\b' + x + '\\b').test(nomD));
        if (/fin de semana|\bfinde\b|\bfin de\b/.test(nomD) && !nombrados.length && ['agenda_o_cambio', 'confirma', 'incierto'].includes(ev.evento)) {
            const w = dow(now); const vie = masDias(hoyD, w >= 5 ? -(w - 5) : (w === 0 ? -2 : 5 - w)); let quedan = [vie, masDias(vie, 1), masDias(vie, 2)].filter(vivo);
            // (owner 2026-09-26) si ya es fin de semana —viernes de 6 pm en adelante, sábado o domingo— "el fin de semana" es el de la PRÓXIMA semana; "este fin de semana" sí es el actual
            const yaEsFinde = w === 6 || w === 0 || (w === 5 && L(now).getUTCHours() >= 18); const dijoEste = /\beste (fin|finde)\b/.test(nomD);
            if (yaEsFinde && !dijoEste) quedan = [];
            if (!quedan.length) { const v2 = masDias(vie, 7); quedan = [v2, masDias(v2, 1), masDias(v2, 2)].filter(d => abreCierra(t, d)); }
            if (quedan.length && (ev.dia_ini !== quedan[0] || String(ev.dia_fin || ev.dia_ini) !== quedan[quedan.length - 1])) { ev.dia_ini = quedan[0]; ev.dia_fin = quedan.length > 1 ? quedan[quedan.length - 1] : ''; if (ev.evento !== 'agenda_o_cambio') ev.evento = 'agenda_o_cambio'; ev.razon = '(fin de semana = lo que queda: ' + quedan.join(', ') + ') ' + (ev.razon || ''); }
        } else if (nombrados.length === 1 && /^\d{4}-\d{2}-\d{2}$/.test(String(ev.dia_ini || '')) && ev.dia_ini === hoyD && DIAN[dow(now)] === nombrados[0] && !/\b(hoy|ahorita|ahora)\b/.test(nomD) && !vivo(hoyD)) {
            const sig = masDias(hoyD, 7); ev.razon = '(' + nombrados[0] + ' ya pasó hoy → el próximo, ' + sig + ') ' + (ev.razon || ''); ev.dia_ini = sig; if (ev.dia_fin === hoyD) ev.dia_fin = '';
        }
    }
    ...
    // 3 PARTES · DESPUÉS DE CONFIRMADA: la lectura se hace UNA vez aquí, pero quien decide es cita3 (solo script; lo demás escala a Mario). null = sigue el camino normal.
    if (typeof interceptar === 'function' && c0) { const ic = await interceptar({ ev, nuevo, c: c0, now }); if (ic) { await io.sistema(notaLector + ' → 3 partes: ' + (ic.evento || (ic.silencio ? 'cortesía' : 'escala'))); return Object.assign({ manejado: true, seguir_cerebro: false, evento: ev.evento, cita: Number(c0.id), interceptado: true }, ic); } }
    ...
    // ORDEN NATURAL (orden owner 2026-09-22): primero el acuse de la cita, DESPUÉS la parte comercial del mismo mensaje (antes salía al revés y se leía volteado)
    const rC = await conReintento((intento) => aplicarComprador({ t, chat, auto, io, now, ev, nuevo, intento }));
...
async function tablero({ tenant }) {
    ...
    let cs = await query('SELECT * FROM citaf WHERE tenant_id = ? AND estado IN ' + VIVAS_SQL + ' ORDER BY ini_ts ASC', [tId]);
    if (Number(cfgDe(tenant).tres_partes) === 1) { const ok3 = new Set((await query("SELECT cita_id FROM cita3 WHERE tenant_id = ? AND estado IN ('confirmada','dia_d','pausada') AND fase IS NULL", [tId]).catch(() => [])).map(x => Number(x.cita_id))); cs = cs.filter(c => ok3.has(Number(c.id))); }   // 3 PARTES: solo las acreditadas
...
async function vivasPorChat(tId) { await asegurar(); const tresP = !!(await query('SELECT 1 FROM tenants WHERE id = ? AND config_json LIKE ?', [Number(tId), '%"tres_partes":1%']).catch(() => []))[0];
    // 3 PARTES: mientras no la confirmen los 3, NO es cita (orden owner 2026-09-26): solo cuentan las acreditadas en cita3
    const r = await query(tresP ? "SELECT f.chat_id, c3.ini_ts, f.auto_nombre, f.estado, f.confirmada_dia FROM citaf f JOIN cita3 c3 ON c3.cita_id = f.id WHERE f.tenant_id = ? AND f.estado IN ('viva','en_camino') AND c3.estado IN ('confirmada','dia_d') AND c3.fase IS NULL" : "SELECT chat_id, ini_ts, auto_nombre, estado, confirmada_dia FROM citaf WHERE tenant_id = ? AND estado IN ('viva','en_camino')", [Number(tId)]).catch(() => []); const m = {}; for (const x of r) m[Number(x.chat_id)] = { cita_ts: Number(x.ini_ts), auto: x.auto_nombre || null, estado: 'confirmada' }; return m; }
...
const GUIA_MAQ = {
    ...
    r1: 'Toque ligero a media semana, solo para seguir en contacto. Corto.', vispera_hora: 'Recordatorio de la víspera: seguro, como vendedor que da por hecho que se ven (NUNCA en forma de pregunta ni "correcto?"); cierra con "nos hablamos mañana".', vispera_sin_hora: 'Recordatorio de la víspera: saludo práctico y corto, buscando la hora.', dia_hora: 'Mensaje del mero día, seguro, como vendedor que da por hecho que se ven (NUNCA pregunta ni "correcto?"); cierra pidiendo que avise cuando venga.', dia_sin_hora: 'Mensaje del mero día, como vendedor que da por hecho que se ven; busca la hora.', dia_ultimo: 'Mensaje del mero día, como vendedor que da por hecho que se ven; busca la hora.'
};
...
// ══ 3 PARTES · PUERTA DE SALIDA DEL MOTOR (orden owner 2026-09-26): mientras la cita de 3 no esté acreditada, el motor de 2 partes no puede
//    decir "ahí nos vemos / me avisas cuando vengas" (eso da la visita por hecha con un solo voto). Sin hora exacta → se pide día y hora;
//    con hora → "en cuanto quede confirmado te aviso". Las plantillas neutras (cancela, ya fue, gracias, se complicó…) pasan tal cual.
const NEUTRAS3 = new Set(['cancela', 'ya_fue', 'gracias', 'quien_soy', 'se_complico', 'se_complico_multi', 'promete_avisar', 'auto_vendido', 'fuera_horario', 'pide_dia', 'aclaracion', 'aclaracion_duda', 'mueve_vendedor', 'se_retrasa_pregunta', 'se_retrasa_camino', 'nace_cierre']);
async function tresAntes(tC, chC, meta) {
    if (NEUTRAS3.has(meta.clave)) return null;
    const c3 = (await query('SELECT estado FROM cita3 WHERE chat_id = ? ORDER BY cita_id DESC LIMIT 1', [Number(chC.id)]).catch(() => []))[0];
    if (c3 && ['confirmada', 'dia_d', 'pausada'].includes(String(c3.estado))) return null;
    const cf = await citaViva(Number(tC.id), Number(chC.id)); if (!cf) return null;
    const n = meta.nombre ? ' ' + meta.nombre : ''; const now = Date.now() + await offsetDe(chC.id);
    const horaYa = cf.precision === 'hora' || (cf.precision === 'franja' && Number(cf.fin_ts) - Number(cf.ini_ts) <= 2 * 3600000);
    const o = cfgDe(tC).citaf_textos || {};
    if (horaYa) return (o[meta.clave] && /confirm/i.test(o[meta.clave])) ? null : 'Va' + n + ', en cuanto quede confirmado te aviso por aquí.';
    if (meta.clave === 'incierto' || meta.clave === 'avisa_al_salir' || meta.clave === 'avisa_al_salir_hoy') return 'Va' + n + ', sin presión. Cuando lo tengas me dices día y hora y lo confirmo.';   // "checo mis horarios / luego te digo": no se le presiona con la pregunta otra vez
    const ask = (cf.precision === 'franja' || cf.precision === 'dia') ? '¿A qué hora exactamente te acomoda ' + cuandoCorto(cf, now, true) + n + '? Así lo confirmo.' : '¿Qué día y a qué hora exactamente te acomoda' + n + '? Así lo confirmo.';
    const ult = String(((await query("SELECT texto FROM mensajes WHERE conversacion_id = ? AND direccion = 'out' AND emisor != 'sistema' ORDER BY id DESC LIMIT 1", [Number(chC.id)]).catch(() => []))[0] || {}).texto || '');
    if (/exactamente te acomoda|aunque sea aproximad/.test(ult)) return 'Va' + n + '. Para apartártelo necesito ' + (cf.precision === 'franja' || cf.precision === 'dia' ? 'una hora' : 'un día y una hora') + ' aunque sea aproximada; con eso lo confirmo.';   // sin ejemplo: el ejemplo le ponía palabras en la boca   // no repetir la misma pregunta palabra por palabra
    return ask;
}
function ioPara(tC, chC) {
    ...
    return {
        mandar: async (tx, ts) => {   // "||" separa burbujas. Con maquillaje encendido en el universo, la IA le pone tacto y el código verifica; si no, sale la plantilla.
            ...
            if (Number(cfgDe(tC).tres_partes) === 1) {   // 3 PARTES: antes de los 3 votos nada suena a "ya quedó" — por plantilla conocida o, si no se sabe cuál es, por su CONTENIDO
                const metaE = meta || (/(nos vemos|te esperamos|aqu[ií] te esperamos|qued[oó] confirmad|te confirmo tu cita|me avisas cuando vengas)/i.test(String(tx)) ? { clave: 'confirma_antes', nombre: '' } : null);
                if (metaE) { const r3 = await tresAntes(tC, chC, metaE); if (r3) B = [r3]; }
            }   // 3 PARTES: antes de los 3 votos nada suena a "ya quedó"
            ...
            B = B.map(x => x.charAt(0).toUpperCase() + x.slice(1));   // sin nombre del comprador las plantillas arrancaban en minúscula ("para tenerte el auto listo…")
            for (let i = 0; i < B.length; i++) await mandar({ texto: B[i], ts: ts ? ts + i * 1000 : undefined });
            return B.join('\n');
        },
        pin: async (autoInvId) => { if (Number(cfgDe(tC).tres_partes) === 1) return; /* 3 partes: la ubicación sale al acreditar (cita3) */ const pe = (await query('SELECT image_b64, lat, lng, name, maps_link FROM punto_envio WHERE auto_id = ?', [Number(autoInvId)]).catch(() => []))[0]; if (!pe) return; await mandar({ imagen: pe.image_b64 || null, imagen_ref: pe.image_b64 ? 'ubic-img:' + Number(autoInvId) : null, location: (pe.lat != null && pe.lng != null) ? { lat: pe.lat, lng: pe.lng, name: pe.name || '', maps_link: pe.maps_link || undefined } : null }); },
        vendedor: async (txt) => { if (Number(cfgDe(tC).tres_partes) === 1) return;   // 3 PARTES: los avisos a Mario los da cita3 (aquí decían "Nueva visita" antes de que existiera la cita)
            // solo cambios que alteran la realidad operativa. Sandbox: renglón gris en el hilo. Universo real: además WhatsApp al número del vendedor (el mismo desde el que salen los mensajes), por la puerta de mensajes existente.
            await nota(txt);
