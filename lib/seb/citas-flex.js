...
const TXT = {
    ...
    confirma_hoy: x => 'Perfecto' + x.n + ', aquí te esperamos hoy' + (x.cierre ? ', estamos hasta las ' + x.cierre : '') + '. Me avisas cuando vengas en camino.',
    quita_dia: x => 'Sin problema' + x.n + ', entonces te esperamos ' + x.corto + '. Cualquier cambio me avisas.',
    se_complico_multi: x => 'Sin problema' + x.n + '. ¿Te sigue quedando ' + x.otros_dias + ' o prefieres otro día?',
    vispera_avisa: x => 'Hola' + x.n + ', mañana te esperamos para ver ' + x.elauto + '. Me avisas cuando vengas en camino.',
...
function plantillaDe(c, tipo) {
    ...
    return ({ r1: 'r1', vispera: c.precision === 'hora' ? 'vispera_hora' : (c.precision === 'dias' ? 'vispera_multi' : (String(c.ultima_palabra) === 'promete_avisar' ? 'vispera_avisa' : 'vispera_sin_hora')), dia: conHora ? 'dia_hora' : 'dia_sin_hora', dia_multi: 'dia_multi', dia_ultimo: 'dia_ultimo', empujon: 'empujon', me_avisas: 'me_avisas', revision_suave: 'revision_suave', aclaracion: Number(c.en_duda) ? 'aclaracion_duda' : 'aclaracion' })[tipo] || null;
}
