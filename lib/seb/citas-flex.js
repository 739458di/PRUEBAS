...
const TXT = {
    ...
    nace: x => 'Va' + x.n + ', entonces te confirmo tu cita: ' + lc(x.cuando) + (x.lugar ? ' aquí en ' + x.lugar : '') + ' para ver ' + x.elauto + '.' + (x.recibe ? ' Te recibe ' + x.recibe + '.' : '') + (x.pin ? ' Te mando la ubicación.' : ''),
    nace_cierre: x => 'Estamos en contacto, cualquier cosa me avisas. Para servirte.',
    ...
    nace_suave: x => 'Va, claro' + x.n + '. ' + Cap(x.corto) + ' lo ves. ' + (x.hoy ? 'Estamos hasta las ' + x.cierre + ', ¿como a qué hora te esperamos?' : '¿A qué hora te esperamos?'),
    nace_suave_dias: x => 'Va, claro' + x.n + ', ' + x.corto + ' lo ves. ¿Tienes alguna hora en mente?',
    cambio: x => 'Va' + x.n + ', sin tema. Te muevo la cita: ' + lc(x.cuando) + '.',
    precisa: x => 'Va, entonces nos vemos ' + x.corto + '.' + (x.pin ? ' Te mando la ubicación.' : ''),
    ajuste: x => 'Va, sin tema. Entonces nos vemos ' + x.corto + '.',
    reagenda: x => 'Va, entonces nos vemos ' + x.corto + '.',
    angosta_hoy: x => 'Va, ' + x.corto + ' nos vemos. Hoy estamos hasta las ' + x.cierre + '.' + (x.pin ? ' Te dejo otra vez la ubicación.' : ''),
    fuera_horario: x => x.nombre_coma + 'ese día estamos ' + x.horario + '. ¿Alcanzas en ese horario?',
    r1: x => 'Qué tal' + x.n + ', buen día. Seguimos pendientes ' + (/^entre /.test(x.corto) ? '' : 'para ') + x.corto + '. Cualquier cosa me avisas.',
    vispera_hora: x => 'Qué tal' + x.n + ', mañana nos vemos ' + x.alas + ' para ver ' + x.el + ', ¿correcto?',
    vispera_sin_hora: x => 'Qué tal' + x.n + ', mañana nos vemos para ver ' + x.el + '. ¿Como a qué hora te esperamos?',
    vispera_multi: x => 'Qué tal' + x.n + ', ya casi es ' + x.ventana_nombre + '. ¿Qué día te acomoda más para ver ' + x.el + ': ' + x.dias_lista + '?',
    vispera_avisa: x => 'Qué tal' + x.n + ', mañana nos vemos para ver ' + x.el + '. Me avisas cuando vengas en camino.',
    dia_hora: x => 'Qué tal' + x.n + ', hoy nos vemos ' + x.alas + ' para ver ' + x.el + ', ¿correcto?',
    dia_sin_hora: x => 'Qué tal' + x.n + ', hoy nos vemos para ver ' + x.el + '. Estamos hasta las ' + x.cierre + ', ¿como a qué hora te esperamos?',
    dia_avisa: x => 'Qué tal' + x.n + ', hoy nos vemos para ver ' + x.el + '. Estamos hasta las ' + x.cierre + ', me avisas cuando vengas en camino.',
    dia_multi: x => 'Qué tal' + x.n + ', ¿hoy lo ves o te queda mejor ' + x.otros_dias + '? Hoy estamos hasta las ' + x.cierre + '.',
    dia_ultimo: x => 'Qué tal' + x.n + ', hoy nos vemos para ver ' + x.el + '. Estamos hasta las ' + x.cierre + ', ¿como a qué hora te esperamos?',
    empujon: x => x.nombre_coma + '¿todo bien para hoy? Si se te complicó no pasa nada, lo movemos.',
    confirma_dia: x => 'Perfecto, aquí nos vemos.',
    confirma_antes: x => 'Perfecto, ahí nos vemos.',
    ...
    ya_voy: x => 'Va, aquí te esperamos.' + (x.referencia ? ' ' + x.referencia : ''),
    ya_llegue: x => 'Va, ahorita salen a recibirte.',
    retraso_ok: x => 'Sin tema, aquí te esperamos.',
    ...
    aclaracion: x => 'Qué tal' + x.n + ', ¿sí alcanzaste a pasar ayer o todavía tienes pensado venir a ver ' + x.el + '?',
    aclaracion_duda: x => 'Qué tal' + x.n + ', ¿ya viste qué día te quedaría mejor para venir a ver ' + x.el + '?',
    ...
    quita_dia: x => 'Sin tema, entonces nos vemos ' + x.corto + '. Cualquier cambio me avisas.',
    avisa_al_salir_hoy: x => 'Va, sin presión. Hoy estamos' + (x.cierre ? ' hasta las ' + x.cierre : '') + ', me avisas cuando vengas en camino.',
    avisa_al_salir: x => 'Va, sin presión. Nos vemos ' + x.corto + ', me avisas cuando vengas en camino.',
    promete_avisar: x => 'Va' + x.n + ', sin tema. Aquí estamos para cuando gustes, me avisas.',
    cancela: x => 'Entendido' + x.n + ', gracias por avisar. Cualquier cosa aquí andamos.',
    revision_suave: x => 'Qué tal' + x.n + ', hoy también estamos por aquí hasta las ' + x.cierre + ' por si se te acomoda pasar a ver ' + x.el + '.',
...
function texto(t, clave, x) { const o = cfgDe(t).citaf_textos; const tx = (o && typeof o[clave] === 'string' && o[clave].trim()) ? o[clave].replace(/\{(\w+)\}/g, (m, k) => x[k] == null ? '' : String(x[k])) : TXT[clave](x); if (CLAVE_DE.size > 400) CLAVE_DE.clear(); CLAVE_DE.set(tx, { clave, nombre: String(x.n || '').trim() }); return tx; }
function slots(t, c, now) {
    ...
    return { el: corto_a ? 'el ' + corto_a : 'el auto', hoy: ymd(Number(c.ini_ts)) === ymd(now), dias_lista: o(dd, false), otros_dias: o(dd.filter(d => d > ymd(now)), true) || 'otro día', ventana_nombre: finde ? 'fin de semana' : 'la fecha', n: n1 ? ' ' + n1 : '', nombre_coma: n1 ? n1 + ', ' : '', auto: c.auto_nombre || '', elauto: c.auto_nombre ? 'el ' + c.auto_nombre : 'el auto', cuando: cuandoLargo(t, c), corto: cuandoCorto(c, now), hora: hm(c.ini_ts), alas: aLas(c.ini_ts), cierre: ac.cierra ? hm(ac.cierra) : '', recibe: cfgDe(t).recibe || '', referencia: cfgDe(t).referencia || '' };
}
...
const SCHEMA = {
    ...
    required: ['razon', 'evento', 'confianza', 'dia_ini', 'dia_fin', 'hora_ini', 'hora_fin', 'franja', 'alcance', 'sigue_interesado', 'avisa_dia', 'avisa_hora', 'avisara', 'minutos_tarde', 'auto_texto', 'restriccion', 'asiste', 'condicion', 'a_cuenta', 'foraneo', 'pregunta_comercial', 'motivo'],
    properties: {
        ...
        alcance: { type: 'string', enum: ['', 'hora', 'dia', 'todo'] }, sigue_interesado: { type: 'boolean' },
...
async function aplicarAgenda({ t, chat, auto, io, now, v, fuente, extras, nuevo, modo }) {
    ...
    if (!c) {   // NACE (o REVIVE ligada a la anterior)
        ...
        if (formal) { await io.mandar(texto(t, 'nace', x)); if (x.pin) { await io.pin(Number(c.auto_id)); await bitacora(c, now, 'sistema', 'pin_enviado', null); } await io.mandar(texto(t, 'nace_cierre', x)); }
...
async function entrante0({ tenant, chat, auto, io, textoNuevo, ahoraFijo, comercial, _T }) {
    ...
    if (ev.evento === 'cortesia') { await io.sistema(notaLector); await run('UPDATE citaf SET ultimo_in_ts = ? WHERE id = ?', [now, Number(c0.id)]).catch(() => { }); const xg = slots(t, c0, now); xg.n = ''; xg.viva = true; await io.mandar(texto(t, 'gracias', xg)); return { manejado: true, seguir_cerebro: false, evento: 'cortesia', cita: Number(c0.id) }; }
...
const GUIA_MAQ = {
    nace: 'Confirmación FORMAL (él dio día y hora): confírmale su cita con todos los datos del base y dile que le mandas la ubicación. Una sola burbuja. NO cierres con frases de servicio: el cierre va en otro mensaje.',
    nace_cierre: 'Cierre de servicio después de mandarle la ubicación: corto, cálido, que sepa que estamos en contacto y que avise cualquier cosa. Una burbuja.',
...
const SIN_EMOJI = s0 => String(s0 || '').replace(/[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{2B00}-\u{2BFF}\u{FE0F}\u{200D}]/gu, '').replace(/[ \t]{2,}/g, ' ').trim();
...
async function maquillar({ base, clave, nombre, historial, esRespuesta }) {
    ...
    try {
        const system = 'Eres la voz de Sebastián, vendedor de autos seminuevos en Monterrey, escribiendo por WhatsApp a un comprador. Te doy el MENSAJE BASE: es lo que el sistema NECESITA decir (su función y sus datos NO se tocan). Tú solo le pones TACTO HUMANO: es maquillaje, no funcionalidad.\n' +
            'REGLAS DURAS:\n· Conserva TODOS los datos del base tal cual (día, hora, lugar, auto, horario). No agregues ni cambies ningún dato, número, hora ni promesa, y NO repitas datos de la cita (día, hora, lugar, auto) que el base no menciona.\n· Conserva la FUNCIÓN: si el base trae una pregunta, CONSÉRVALA como pregunta con sus signos ¿?; no agregues preguntas nuevas ni peticiones nuevas (avisar, confirmar, llamar) que el base no traiga. Si el base pide que avise (cuando venga, cualquier cosa), eso se queda.\n· Prohibido hablar de precio, crédito, enganche, garantía, descuentos o cualquier cosa que no esté en el base.\n· CERO emojis. Sin signos de admiración. Nada de "estimado", "con gusto", "quedo atento", "excelente día": eso suena a robot.\n' +
            'VOZ: norteño sobrio, práctico, con intención de servir. Frases cortas. Muletillas suyas (una por mensaje, no más): "Va", "Sin tema", "Claro", "Qué tal", "Ahí nos vemos", "Estamos en contacto", "Cualquier cosa me avisas", "Para servirte", "Nos hablamos mañana". Nunca de más: si el base ya está bien, déjalo casi igual.\n' +
            'NOMBRE: úsalo solo donde suma (saludo de un recordatorio, confirmación formal, cuando se le complicó algo). Si el último mensaje del Lote ya lo traía, NO lo repitas. Si no hay nombre, no inventes.\n' +
            (dioGracias ? 'EL COMPRADOR ACABA DE DAR LAS GRACIAS: empieza con "Gracias a ti".\n' : '') + 'Refleja su registro: si escribe relajado, relajado; si escribe formal, un poco más formal.\n' +
            'BURBUJAS: normalmente 1. Usa 2 solo cuando hay dos ideas distintas (acuse + pregunta; confirmación + cierre). Máximo 3. Cada una de máximo 28 palabras.\nDevuelve { burbujas: [...] }.';
        ...
        for (const w of ['ubicaci', 'avis', 'reagend', 'mov', 'camino', 'confirm', 'llam', 'marc', 'list', 'cerca']) if (P0.includes(w) !== P.includes(w)) return dbg('petición funcional distinta: ' + w);                                                                   // la petición funcional sigue ahí
        if (todo.length > b0.length * 1.7 + 60 || B.some(x => x.length > 240) || /!/.test(todo)) return dbg('largo o con admiración');
        return B;
    ...
    return {
        mandar: async (tx, ts) => {   // "||" separa burbujas. Con maquillaje encendido en el universo, la IA le pone tacto y el código verifica; si no, sale la plantilla.
            ...
            if (meta && cfgDe(tC).citaf_maquillaje && !process.env.CITAF_SIN_MAQUILLAJE) {
                ...
                B = await maquillar({ base: tx, clave: meta.clave, nombre: meta.nombre, historial: hist, esRespuesta: !ts });
            }
