...
const TXT = {
    ...
    nace: x => 'Va pues!||Te confirmo entonces' + x.n + ': ' + lc(x.cuando) + (x.lugar ? ' aquí en ' + x.lugar : '') + ' para ver ' + x.elauto + '.' + (x.recibe ? ' Te recibe ' + x.recibe + '.' : '') + (x.pin ? ' Te mando la ubicación.' : ''),
    nace_cierre: x => 'Me avisas cualquier cosa, estamos en contacto. Para servirte.',
    ...
    r1: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Seguimos pendientes ' + (/^entre /.test(x.corto) ? '' : 'para ') + x.corto + '. Cualquier cosa me avisas.',
    vispera_hora: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Mañana nos vemos ' + x.alas + ' para ver ' + x.el + ', ¿correcto?',
    vispera_sin_hora: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Mañana nos vemos para ver ' + x.el + '. ¿Como a qué hora te esperamos?',
    vispera_multi: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Ya casi es ' + x.ventana_nombre + '. ¿Qué día te acomoda más para ver ' + x.el + ': ' + x.dias_lista + '?',
    vispera_avisa: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Mañana nos vemos para ver ' + x.el + '. Me avisas cuando vengas en camino.',
    dia_hora: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Hoy nos vemos ' + x.alas + ' para ver ' + x.el + ', ¿correcto?',
    dia_sin_hora: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Hoy nos vemos para ver ' + x.el + '. Estamos hasta las ' + x.cierre + ', ¿como a qué hora te esperamos?',
    dia_avisa: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Hoy nos vemos para ver ' + x.el + '. Estamos hasta las ' + x.cierre + ', me avisas cuando vengas en camino.',
    dia_multi: x => 'Qué tal' + x.n + ', ' + x.saludo + '||¿Hoy lo ves o te queda mejor ' + x.otros_dias + '? Hoy estamos hasta las ' + x.cierre + '.',
    dia_ultimo: x => 'Qué tal' + x.n + ', ' + x.saludo + '||Hoy nos vemos para ver ' + x.el + '. Estamos hasta las ' + x.cierre + ', ¿como a qué hora te esperamos?',
    ...
    me_avisas: x => Cap(x.saludo) + x.n + '||Cuando vengas en camino me avisas para recibirte aquí.',
    ...
    aclaracion_duda: x => 'Qué tal' + x.n + ', ' + x.saludo + '||¿Ya viste qué día te quedaría mejor para venir a ver ' + x.el + '?',
...
async function maquillar({ base, clave, nombre, historial, esRespuesta, saludo }) {
    ...
    try {
        const system = 'Eres la voz de Sebastián, vendedor de autos seminuevos en Monterrey, escribiendo por WhatsApp a un comprador. Te doy el MENSAJE BASE: es lo que el sistema NECESITA decir (su función y sus datos NO se tocan). Tú solo le pones TACTO HUMANO: es maquillaje, no funcionalidad.\n' +
            'REGLAS DURAS:\n· Conserva TODOS los datos del base tal cual (día, hora, lugar, auto, horario). No agregues ni cambies ningún dato, número, hora ni promesa, y NO repitas datos de la cita (día, hora, lugar, auto) que el base no menciona.\n· Conserva la FUNCIÓN: si el base trae una pregunta, CONSÉRVALA como pregunta con sus signos ¿?; no agregues preguntas nuevas ni peticiones nuevas (avisar, confirmar, llamar) que el base no traiga. Si el base pide que avise (cuando venga, cualquier cosa), eso se queda.\n· Prohibido hablar de precio, crédito, enganche, garantía, descuentos o cualquier cosa que no esté en el base.\n· CERO emojis. Máximo UN signo de admiración en todo el mensaje (y solo si el base lo trae o es un saludo). Si saludas, el saludo es el de ESTA hora: "' + (saludo || 'buen día') + '" (jamás otro). Nada de "estimado", "con gusto", "quedo atento", "excelente día": eso suena a robot.\n' +
            'VOZ: norteño sobrio, práctico, con intención de servir. Frases cortas. Muletillas suyas (una por mensaje, no más): "Va", "Sin tema", "Claro", "Qué tal", "Ahí nos vemos", "Estamos en contacto", "Cualquier cosa me avisas", "Para servirte", "Nos hablamos mañana". VARÍA LIGERAMENTE para que no suene a machote repetido: puedes agregar o quitar una palabra, cambiar una muletilla por otra suya ("Va" ↔ "Va pues" ↔ "Sale" ↔ "Vava"; "Sin tema" ↔ "Sin problema"; "Claro" ↔ "Sisi claro"), acomodar el orden o partir en burbujas como lo haría él. Piensa qué versión le caería mejor a ESTE comprador según cómo viene escribiendo. Nunca de más: corto siempre gana. Si en la conversación ya usaste una muletilla, usa otra.\n' +
        ...
        for (const w of ['ubicaci', 'avis', 'reagend', 'camino', 'confirm', 'llam', 'marc']) if (P0.includes(w) !== P.includes(w)) return dbg('petición funcional distinta: ' + w);                                                                   // la petición funcional sigue ahí
