...
async function entrante({ tenant, chat, auto, io, textoNuevo, ahoraFijo, comercial }) {
    await asegurar();
    ...
    if (pc) { let respondida = false; if (typeof comercial === 'function') { try { respondida = !!(await comercial(pc)); } catch (e) { respondida = false; } } if (!respondida) await io.vendedor('💬 En el mismo mensaje también preguntó: "' + pc.slice(0, 160) + '". Contéstale tú.'); }
    ...
    for (let vuelta = 0; vuelta < 60; vuelta++) {
        ...
        try {
            ...
            if (k.tipo === 'aclaracion') {   // a la mañana siguiente: UN mensaje para sacarnos de la duda. Si no contesta NO se infiere nada.
                ...
                const tx = texto(t, Number(c.en_duda) ? 'aclaracion_duda' : 'aclaracion', x); await io.sistema('⏱ ' + fechaCorta(cuando) + ' · sale: aclaración de ventana vencida'); await io.mandar(tx); await salio(tx); await bitacora(c, cuando, 'reloj', 'aclaracion_enviada', null); continue;
            }
            if (k.tipo === 'marcar') {
...
module.exports = { proyectar, cuandoAviso, activo, asegurar, ahora, offsetDe, ponerOffset, entrante, vendedor, tick, tickTodos, estado, tablero, planear, reconciliar, ventanaDe, vivasPorChat, reset, citaViva, fechaCorta, horarioDe, ioPara, situacionDe, Conflicto, _t: { at, ymd, masDias, hm, fueraDeSilencio, leer, cuandoCorto, cuandoLargo, autoDeTexto, horaDeReloj } };
