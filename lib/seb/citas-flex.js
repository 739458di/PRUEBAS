...
const TXT = {
    ...
    cambio: x => 'Listo' + x.n + ', ya quedó movida tu cita ✅\n' + (x.auto ? x.auto + '\n' : '') + x.cuando,   // SOLO cuando cambia de DÍA
    precisa: x => 'Perfecto' + x.n + ', entonces te esperamos ' + x.corto + ' ✅' + (x.auto ? '\n' + x.auto : ''),   // precisó día u hora DENTRO de lo que ya había dicho: nada se mueve, solo se afina
    ajuste: x => 'Listo' + x.n + ', ya ajusté la hora ✅\n' + (x.auto ? x.auto + '\n' : '') + x.cuando,   // mismo día, otra hora
    reagenda: x => 'Listo' + x.n + ', ya quedó tu cita ✅\n' + (x.auto ? x.auto + '\n' : '') + x.cuando,   // estaba sin fecha y dio una
    ...
    vispera_hora: x => 'Hola' + x.n + ', te recuerdo tu cita de mañana ' + x.alas + ' para ver ' + x.elauto + '. Si necesitas moverla me dices.',
    ...
    vispera_multi: x => 'Hola' + x.n + ', ya casi es ' + x.ventana_nombre + '. ¿Qué día te acomoda más para ver ' + x.elauto + ': ' + x.dias_lista + '? Es solo para tenerte el auto listo, sin compromiso.',
    dia_hora: x => 'Buen día' + x.n + '. Hoy es tu cita ' + x.alas + ' para ver ' + x.elauto + '. ¿Seguimos en pie?',
    dia_sin_hora: x => 'Buen día' + x.n + '. Hoy te esperamos para ver ' + x.elauto + ', estamos hasta las ' + x.cierre + '. ¿Sí alcanzas a venir hoy?',
    dia_multi: x => 'Buen día' + x.n + '. ¿Vienes hoy a ver ' + x.elauto + ' o te queda mejor ' + x.otros_dias + '? Hoy estamos hasta las ' + x.cierre + '.',
    ...
    avisa_al_salir_hoy: x => 'Va' + x.n + ', sin presión. Hoy estamos' + (x.cierre ? ' hasta las ' + x.cierre : '') + ', avísame cuando vengas en camino.',
    avisa_al_salir: x => 'Va' + x.n + ', sin presión. Te esperamos ' + x.corto + ', avísame cuando vengas en camino.',
...
function plantillaDe(c, tipo) {
    ...
    return ({ r1: 'r1', vispera: c.precision === 'hora' ? 'vispera_hora' : (c.precision === 'dias' ? 'vispera_multi' : 'vispera_sin_hora'), dia: conHora ? 'dia_hora' : 'dia_sin_hora', dia_multi: 'dia_multi', dia_ultimo: 'dia_ultimo', empujon: 'empujon', me_avisas: 'me_avisas', revision_suave: 'revision_suave', aclaracion: Number(c.en_duda) ? 'aclaracion_duda' : 'aclaracion' })[tipo] || null;
}
...
const EVENTOS = ['agenda_o_cambio', 'confirma', 'se_retrasa', 'avisa_al_salir', 'promete_avisar', 'ya_voy', 'ya_llegue', 'ya_fue', 'se_complico', 'cancela', 'cambia_auto', 'incierto', 'no_es_de_cita', 'otra_cosa'];
const SCHEMA = {
...
async function leer({ t, c, now, historial, nuevo, auto }) {
    ...
    const system =
        ...
        '· avisa_al_salir = el DÍA de la visita SE QUEDA IGUAL; lo único abierto es la HORA, y él avisa cuando vaya a salir o a qué hora ("yo te aviso cuando vaya", "te aviso cuando salga", "sí voy, te aviso a qué hora", "te marco cuando vaya para allá"). Típico después de que confirmó que sí viene ese día o de que dijo que llega más tarde y le preguntamos la hora. NO cambia de día ni deja la visita sin fecha.\n' +
        '· promete_avisar = deja la visita SIN CUÁNDO (ya no se sabe ni el DÍA): "yo te aviso qué día", "no sé todavía", "déjame ver y te digo", "luego te confirmo si voy". Si el día sigue firme y solo falta la hora → avisa_al_salir.\n' +
...
function situacionDe(t, c, now) {
    ...
    if (up === 'avisa_al_salir') s = 'VIENE ' + cuandoCorto(c, now).toUpperCase() + ' · hora abierta: él avisa cuando salga';
...
async function aplicarAgenda({ t, chat, auto, io, now, v, fuente, extras, nuevo }) {
    ...
    else await io.mandar(texto(t, dentro ? 'precisa' : (estadoAntes === 'pospuesta' ? 'reagenda' : (diaAntes === ymd(v.ini) ? 'ajuste' : 'cambio')), x));   // la redacción dice lo que de verdad pasó: afinar ≠ ajustar la hora ≠ mover de día
...
function grabadora(io, T) {   // misma tubería, pero anotando lo que sale para la traza
    return Object.assign({}, io, { mandar: async (tx, ts) => { T.acciones.mensajes.push(String(tx)); return io.mandar(tx, ts); }, pin: async (a) => { T.acciones.mensajes.push('📍 [ubicación: captura + pin]'); return io.pin(a); }, vendedor: async (tx) => { T.acciones.avisos_vendedor.push(String(tx)); return io.vendedor(tx); } });
}
...
async function entrante0({ tenant, chat, auto, io, textoNuevo, ahoraFijo, comercial, _T }) {
    ...
    if (c0 && ev.evento === 'promete_avisar' && ['viva', 'en_camino'].includes(c0.estado) && String(c0.ultima_palabra) !== 'ventana_vencida' && ymd(now) >= ymd(Number(c0.ini_ts)) && ymd(now) <= ymd(Number(c0.fin_ts)) && (Number(c0.confirmada_dia) || ['confirma', 'se_retrasa', 'avisa_al_salir'].includes(String(c0.ultima_palabra))) && !/\b(no s[eé]|a ver si|chance|quiz[aá]s?|tal vez|otro d[ií]a|qu[eé] d[ií]a|mejor|ma[ñn]ana|semana|si voy|si puedo)\b/i.test(nuevo)) { ev.evento = 'avisa_al_salir'; ev.razon = '(hoy ya dijo que sí viene: "yo te aviso" solo abre la hora, el día sigue) ' + (ev.razon || ''); }
    if (c0 && ev.evento === 'avisa_al_salir' && (c0.estado === 'pospuesta' || String(c0.ultima_palabra) === 'ventana_vencida')) { ev.evento = 'promete_avisar'; }
...
async function aplicarComprador({ t, chat, auto, io, now, ev, nuevo, intento }) {
    ...
    if (E === 'avisa_al_salir') {   // EL DÍA SE QUEDA · la hora queda abierta · él avisa cuando salga. No se le persigue: solo corre el fin de su ventana.
        if (c.estado === 'en_camino') { await G({}, false); return R(); }
        const antes = cuandoAviso(c); const esHoy = ymd(now) >= ymd(Number(c.ini_ts));
        const campos = { ultima_palabra: 'avisa_al_salir', en_duda: 0, suave: 0, confirmada_dia: esHoy ? 1 : Number(c.confirmada_dia) || 0, version: Number(c.version) + 1, version_ts: now };
        if (c.precision === 'hora' || c.precision === 'franja') {   // la hora anterior deja de valer; el día NO se toca
            const d = esHoy ? ymd(now) : ymd(Number(c.ini_ts)); const ac = abreCierra(t, d) || {};
            campos.ini_ts = esHoy ? now : (ac.abre || at(d, 9, 0)); campos.fin_ts = Math.max(ac.cierra || at(d, 19, 0), campos.ini_ts + 2 * H); campos.precision = 'dia';
            if (ymd(Number(c.ini0_ts)) === ymd(Number(c.fin0_ts))) { campos.ini0_ts = campos.ini_ts; campos.fin0_ts = campos.fin_ts; }
        }
        await G(campos, 'el día sigue; la hora la avisa él'); await bitacora(c, now, 'comprador', 'avisa_al_salir', { nuevo });
        await io.mandar(texto(t, esHoy ? 'avisa_al_salir_hoy' : 'avisa_al_salir', slots(t, c, now)));
        await io.vendedor('Cita actualizada — ' + nombreV + ' / ' + (c.auto_nombre || 'sin auto') + ' / ' + antes + ' → ' + cuandoAviso(c) + ' (el día sigue; él avisa cuando salga)'); return R();
    }
...
function ioPara(tC, chC) {
    ...
    return {
        mandar: (tx, ts) => mandar({ texto: tx, ts: ts || undefined }),
