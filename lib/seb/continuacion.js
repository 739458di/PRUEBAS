...
async function responderFin({ texto, nombre, auto_id, enganche, plazo }) {
    ...
    if (/(minimo|lo menos|cuanto (de |es el |es )?(el )?enganche\b|cual es el enganche|que enganche (piden|manejan|es)|de cuanto (es )?el enganche|con cuanto (menos|minimo)|desde cuanto|enganche minimo)/.test(t) && !/\d/.test(t)) {
