...
const { cotizar, planes, fotosDeAuto, bancoDeAuto } = require('./herramientas.js');
...
// #591: una Tacoma 2014 recibió "es por medio de HEY Banco" — dato falso; el cotizador
// sí acertaba (Renueva Car). NUNCA nombrar el banco sin preguntarle al auto.
// En Renueva Car la tasa NO se dice (decisión del owner) y no se presume "muy buenas tasas".
const PROCESO = b => `Es por medio de ${b}${b === 'HEY Banco' ? ', con muy buenas tasas' : ''}. Mandas tus documentos, te cotizo, solicitamos el crédito, te dicen si apruebas en menos de 2 horas, ya aprobado tú decides cuándo firmas, y el enganche lo das a la entrega — no pagas nada hasta que se te entregue el auto`;
const TASA = b => b === 'HEY Banco'
    ? 'Manejamos del 13.99% al 15%, dependiendo de tu buró e historial, con HEY Banco'
    : 'La tasa te la afino en tu cotización exacta, según tu enganche y plazo';
const BURO = b => `Es sujeto a aprobación con ${b}; estés en buró o no lo vemos${b === 'HEY Banco' ? ', y con buen historial mejora la tasa' : ''}`;
...
function universoCont(texto) {
    ...
    if (/^(si+|sii+|claro( que si)?|va+(le)?|sale( va)?|dale|de acuerdo|obvio|simon|perfecto|excelente|estoy interesad\w*|si me interesa|si quiero|me interesa|me late|si porfavor|si claro|claro que si|de una|orale)[\s.,!]*$/.test(t)) return 'interes_venir';
    return null;
}
...
async function responderFin({ texto, nombre, auto_id, enganche, plazo }) {
    ...
    if (esCotizar) {
        ...
        return { _sintonia: sintonia, segmentos: [r1, card, 'Qué te parece, con cuál opción le damos, y a su vez te voy agendando para que vengas a ver el auto?'] };
    }
    if (esRequisitos) {
        const { c, sintonia } = await conectar({ texto, accion: REQUISITOS, pool: ['Va', 'Con gusto', 'Claro'], nm });
        return { _sintonia: sintonia, segmentos: [`${c}, con gusto`, REQUISITOS, 'Gustas que te cotice, o te solicitamos de una vez la preautorización con los documentos?'] };
    }
    const banco = await bancoDeAuto(auto_id);   // HEY Banco (2018+) o Renueva Car (≤2017)
    if (esBanco) {
        const lineaBanco = banco === 'HEY Banco' ? 'Es con HEY Banco, con muy buenas tasas' : `Es con ${banco}, con muy buenas condiciones`;
    ...
    if (esTasa) {
        const { c, sintonia } = await conectar({ texto, accion: TASA(banco), pool: ['Mira', 'Claro', 'Va'], nm });
        return { _sintonia: sintonia, segmentos: [c, TASA(banco), 'Gustas que te mande un ejercicio para que veas cómo quedaría?'] };
    }
    if (esBuro) {
        const { c, sintonia } = await conectar({ texto, accion: BURO(banco), pool: ['Va', 'Mira', 'Claro'], nm });
        return { _sintonia: sintonia, segmentos: [c, BURO(banco), 'Gustas que te cotice?'] };
    }
    // info / proceso (default fin)
    const { c, sintonia } = await conectar({ texto, accion: PROCESO(banco), pool: ['Con gusto', 'Va', 'Mira'], nm });
    return { _sintonia: sintonia, segmentos: [`${c}, te explico`, PROCESO(banco), 'Gustas que te mande un ejercicio para que veas cómo quedaría?'] };
}
...
async function responderUbi({ texto, nombre, auto_id }) {
    ...
    const ganchoForaneo = 'Gustas que te agende una cita con anticipación?';

    // 1. ENVÍO EXPLÍCITO (lo único que dispara el pitch de envío) (manual §3.D)
    if (RE_ENVIO_EXP.test(t)) {
        const accion = 'sí manejamos envío';
        const { c, sintonia } = await conectar({ texto, accion, pool: ['Claro', 'Mira', 'Va'], nm });
        return { ...base, _sintonia: sintonia, segmentos: [`${c}, ${accion}`, 'La viene a ver un mecánico o conocido tuyo que te dé luz verde, y te la mandamos con garantía de viaje', 'Gustas que la revise alguien de tu confianza?'] };
    }
    // 2. "¿DE DÓNDE ERES / SON?" → preguntan por NUESTRA ubicación (NO es envío).
    if (RE_DEDONDE.test(t)) {
        ...
        const g = esForaneo ? ganchoForaneo : 'Te interesa venir a verla y manejarla?';
        return { ...base, _sintonia: sintonia, segmentos: [c, accion, g] };
    }
    ...
    // 3. FORÁNEO sin pedir envío ("soy de Saltillo", "me queda lejos", "otra ciudad") → cita anticipada.
    if (esForaneo) {
        const lugar = ciudad ? cap(ciudad) : null;
        const accion = lugar
            ? `Estamos en Monterrey, San Pedro Garza García. Como vienes de ${lugar}, te agendo con anticipación para que no hagas el viaje en balde`
            : 'Estamos en Monterrey, San Pedro Garza García, y te agendo con anticipación para que no hagas el viaje en balde';
        const { c, sintonia } = await conectar({ texto, accion, pool: ['Mira', 'Claro', 'Te cuento'], nm });
        return { ...base, _sintonia: sintonia, segmentos: [c, accion, ganchoForaneo] };
    }
...
async function responderCont({ texto, nombre, auto_id, enganche, plazo, intencion }) {
...
module.exports = { responderCont, universoCont, datosPunto, esCamioneta };
