...
async function responderCont({ texto, nombre, auto_id, enganche, plazo, intencion }) {
    ...
    // 🚩611: "mándame más información" → EL MACHOTE (ficha completa del Sales Brain) + gancho.
    if (/((mas|toda) (la )?informacion|informacion (completa|del (auto|carro|coche))|manda(r|s|rme)?(me)? (mas )?(la )?informacion|pasa(me|r)? (la )?informacion|puedes mandar (mas )?informacion)/.test(t)) {
