...
async function responderCont({ texto, nombre, auto_id, enganche, plazo, intencion }) {
    ...
    if (/((mas|toda) (la )?informacion|informacion (completa|del (auto|carro|coche))|manda(r|s|rme)?(me)? (mas )?(la )?informacion|pasa(me|r)? (la )?informacion|puedes mandar (mas )?informacion|(me puedes (dar|pasar|mandar)|dame|pasame|me das|quiero) (mas )?detalles|mas detalles de(l| la| el)|que mas me (puedes (decir|contar)|cuentas) del)/.test(t)) {
