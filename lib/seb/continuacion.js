...
function universoCont(texto) {
    ...
    if (RE_DISPONIBLE.test(t)) return 'disponible'; // 🚩841: "¿sigue disponible?" = leer ficha
    ...
    // "hola sí me interesa" DESPUÉS de una cotización = interés, no otra cotización (owner 2026-09-26: se le reenvió la misma tarjeta)
    if (/^(hola|buenas( tardes| noches| dias)?|que tal|oye)?[\s,.!]*(s+i+|claro( que si)?|va+(le)?|sale( va)?|dale|de acuerdo|obvio|simon|perfecto|excelente|estoy interesad\w*|si me interesa|si quiero|me interesa|me late|si porfavor|si claro|claro que si|de una|orale)[\s.,!]*$/.test(t)) return 'interes_venir';
...
async function responderFin({ texto, nombre, auto_id, enganche, plazo, yaCotizado }) {
    // si ya se le cotizó, los cierres no vuelven a ofrecer cotización: invitan a verlo (owner 2026-09-26: "¿por qué le vuelve a preguntar si le cotiza?")
    const ofrece = (tx) => yaCotizado ? 'Cuándo te viene bien venir a verlo y manejarlo?' : tx;
    const t = norm(texto);
    const nm = nombre ? ' ' + nombre : '';
    const tieneDato = !!(enganche || plazo) && /\d/.test(t);   // solo con un número que ÉL escribió: la IA sacaba el enganche de la tarjeta que Seb acababa de mandar y recotizaba igual
    const esCotizar = tieneDato || /((?:cotiza|coitza|cotisa|cotza|cotica|cotizc)|simulaci|simulam|corrida|como queda|cuanto (me )?queda|mensualidad|a (cuantos )?meses|de enganche|ejercicio|me interesa (a|el|con|por|en) credito|(lo|la) (quiero|llevo|llevaria|agarro|compraria) (a|con) credito|seria (a|con) credito|quiero financiarl|me interesa financiarl|(a|en) credito me interesa)/.test(t);
    ...
    if (esRequisitos) {
        ...
        return { _sintonia: sintonia, segmentos: [/con gusto/i.test(c) ? c : `${c}, con gusto`, REQUISITOS, ofrece('Gustas que te cotice mientras?')] };
    }
    ...
    if (/(minimo|lo menos|cuanto (de |del |es el |es |piden de )?(el )?enganche\b|cual es el enganche|que enganche (piden|manejan|es)|de cuanto (es )?el enganche|con cuanto (menos|minimo)|desde cuanto|enganche minimo)/.test(t) && !/\d/.test(t)) {
        ...
        if (rMin.ok) {
            ...
            return { _sintonia: sintonia, segmentos: [`${c}, ${accionM.charAt(0).toLowerCase()}${accionM.slice(1)}`, 'De ahí para arriba el que gustes', ofrece('Gustas que te cotice para que veas los números?')] };
        }
    }
    ...
    if (esBuro) {
        ...
        return { _sintonia: sintonia, segmentos: [c, BURO, ofrece('Gustas que te cotice?')] };
    }
...
async function responderUbi({ texto, nombre, auto_id }) {
    ...
    if (RE_ENVIO_EXP.test(t)) {
        return { escalar: true, motivo: 'pide ENVÍO a su ciudad (jugada de distancia — tuya)' };
    }
    // 2. "¿DE DÓNDE ERES / SON?" → dato de ubicación (📖 leer ficha).
    if (RE_DEDONDE.test(t)) {
        // 🚩 CONCORDANCIA DE UBICACIONES (caso Mazda/Alan 2026-07-13): el punto que se
        // DICE es el del AUTO (datosPunto) — jamás un texto fijo de otro punto.
        const accion = dir ? `Somos de Monterrey, Nuevo León — este auto lo tenemos en nuestro punto de ${dir}` : 'Somos de Monterrey, Nuevo León';
    ...
    if (/\b(mis? horarios?|mi agenda|mis tiempos|ver con mis|chec\w* (con )?mis|revis\w* (con )?mis|cuadrar mis|acomodar mis)\b/.test(t)) return { universo: 'freno', segmentos: ['Va, sin presión. Cuando tengas tu horario me dices qué día y a qué hora te acomoda y te agendo'] };   // SUS horarios ≠ los del lote
    if (/(a que hora (abren|cierran)|horario|que horario|que dias|dias atienden|abren (los )?(domingo|sabado)|atienden (el |los )?(domingo|sabado|fin de semana)|trabajan (el |los )?(domingo|sabado)|puedo ir (el )?(domingo|sabado))/.test(t)) {
        const accion = (await horarioDelUniverso()) || 'El horario es de 9 a 7pm, pero como son consignaciones de particulares de uso cotidiano es con cita previa; la hora no importa, igual sábado o domingo';
        const { c, sintonia } = await conectar({ texto, accion, pool: ['Mira', 'Claro', 'Va'], nm });
        return { _sintonia: sintonia, segmentos: [c, accion, 'Qué día y hora te coordino?'] };   // horario: sin pin (el pin solo cuando pregunta DÓNDE)
    }
    // 3. FORÁNEO ("soy de Saltillo", "me queda lejos") → jugada de distancia (doctrina) → TUYA.
    if (esForaneo) {
        return { escalar: true, motivo: `comprador FORÁNEO${ciudad ? ' (' + cap(ciudad) + ')' : ''} — jugada: cita anticipada vs envío, tuya` };
    }
    ...
    if (/(de que parte|en que parte|en que ciudad|de que ciudad|que (zona|ciudad)|por donde (estan|se ubican)|por que (zona|rumbo)|se encuentran|donde se encuentra|se localizan|estan ubicados|\bdnd\b|\bdnde\b|donde (se ubican|estan|quedan))/.test(t)) {
        if (!dir) return { escalar: true, motivo: 'este auto NO tiene punto de venta configurado (configúralo en puntos.html) — no hay ubicación que decir' };
        const accion = `Estamos en ${dir}`;
        const { c, sintonia } = await conectar({ texto, accion, pool: ['Claro', 'Mira', 'Te cuento'], nm });
        return { ...base, _sintonia: sintonia, segmentos: [c, accion, 'Te interesa venir a verla y manejarla?'] };
    }
    ...
    if (/(pasame la ubic|mandame la ubic|mande la ubic|enviame la ubic|comparte.*ubic|mandar.*ubic)/.test(t)) {
        if (!dir) return { escalar: true, motivo: 'este auto NO tiene punto de venta configurado (configúralo en puntos.html) — no hay pin que mandar' };
        const accion = `aquí en ${dir}`;
    ...
    if (!dir) return { escalar: true, motivo: 'este auto NO tiene punto de venta configurado (configúralo en puntos.html) — no hay ubicación que mandar' };
    const laLo = (await esCamioneta(auto_id)) ? 'la' : 'lo';
    const accionA = `te mando la ubicación, aquí es nuestro punto de venta, ${dir}`;
    ...
    return { ...base, pin_after_index: 0, _sintonia: sintonia, segmentos: [`${c}, déjame te mando la ubicación`, r2, `A qué hora te coordinamos una cita${nm}, para que ${laLo} manejes y ${laLo} veas?`] };   // el pin cae junto al anuncio, no después de la pregunta
}

// HORARIO REAL del universo ambiente (tenants.config_json.horario = { dow: [abre, cierra] }) → "Lunes a sábado de 9 am a 7 pm y domingo de 10 am a 3 pm; es con cita". null si el universo no tiene horario (número principal).
async function horarioDelUniverso() {
    try {
        const tid = Number(require('./contexto.js').tenant()) || 0; if (!tid) return null;
        const row = (await query('SELECT config_json FROM tenants WHERE id = ?', [tid]))[0]; const cfg = row && row.config_json ? JSON.parse(row.config_json) : null; const h = cfg && cfg.horario;
        if (!h || typeof h !== 'object') return null;
        const D = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado']; const hm = x => { const H = Math.floor(x), M = Math.round((x % 1) * 60); const s = (H % 12 || 12) + (M ? ':' + String(M).padStart(2, '0') : '') + (H < 12 ? ' am' : ' pm'); return s; };
        const grupos = []; for (const d of [1, 2, 3, 4, 5, 6, 0]) { const r = h[String(d)] || h[d]; const k = Array.isArray(r) ? r.join('-') : 'cerrado'; const g = grupos[grupos.length - 1]; if (g && g.k === k) g.dias.push(d); else grupos.push({ k, r: Array.isArray(r) ? r : null, dias: [d] }); }
        const partes = grupos.map(g => { const nombre = g.dias.length > 1 ? D[g.dias[0]] + ' a ' + D[g.dias[g.dias.length - 1]] : D[g.dias[0]]; return nombre + (g.r ? ' de ' + hm(g.r[0]) + ' a ' + hm(g.r[1]) : ' cerrado'); });
        const txt = partes.join(', ').replace(/, ([^,]*)$/, ' y $1'); return txt.charAt(0).toUpperCase() + txt.slice(1) + '. Es con cita para tenerte el auto listo';
    } catch (e) { return null; }
}
...
async function responderFotos({ texto, nombre, auto_id }) {
    ...
    // pidió fotos Y ubicación en el mismo mensaje ("fotos y dónde están"): va también el pin, no se pierde la segunda petición
    if (RE_UBI.test(norm(texto || ''))) return { segmentos: [`Claro${nm}, déjame te las mando`, 'Y aquí te dejo la ubicación', 'Te late venir a verlo y manejarlo?'], fotos: urls, fotos_after_index: 0, ubicacion_auto_id: auto_id, pin_after_index: 1 };
...
// ── DISPONIBILIDAD (🚩841): lectura de ficha, lista blanca también en turno 2.
// El dato sale de la BASE — un auto vendido/pausado JAMÁS se afirma (escala).
const { RE_DISPONIBLE } = require('./regex-comunes.js');
async function responderDisponible({ auto_id }) {
    if (!auto_id) return { escalar: true, motivo: 'pregunta si sigue disponible y no sé de qué auto habla' };
    const av = await query("SELECT estado, vendido_externo, vendido_fyradrive FROM inventario_autos WHERE id=?", [auto_id]).catch(() => []);
    if (!av.length) return { escalar: true, motivo: 'pregunta si sigue disponible y no pude leer el auto en inventario' };
    const disp = av[0].estado === 'activo' && !Number(av[0].vendido_externo || 0) && !Number(av[0].vendido_fyradrive || 0);
    if (!disp) return { escalar: true, motivo: 'pregunta si sigue disponible y el auto ya NO está activo (vendido/pausado) — jugada tuya' };
    return { segmentos: ['Sí, sigue disponible 👍', 'Te late venir a verlo y manejarlo?'] };
}
...
async function responderCont({ texto, nombre, auto_id, enganche, plazo, intencion, conv_id, clasif }) {
    ...
    const RE_FRENO_C = /(te aviso|les aviso|me comunico|te confirmo|lo pienso|lo platico|lo consulto|deja(me)? chec|deja(me)? veo\b(?! (el|la|lo|los|las|foto))|deja(me)? ver (si|que|como|cuando|mis|mi)\b|deja(me)? (progr\w+|cuadrar|acomodar|organizar)|deja(me)? me (acomodo|organizo|programo|cuadro)\b|me (acomodo|organizo|cuadro|ajusto) con (mis|mi|los|el)\b|acomodo mis (horarios|tiempos)|me programo|dame (chance|tiempo|oportunidad)|aguantame|esperame|(ando|estoy|he andado) (muy |bien+ |re )?(atareado|atariado|ocupado|saturado)|tengo (varios )?compromisos)/;
    // FRENO solo si no trae nada más que contestar (owner 2026-09-26: "déjame checarlo los números" + "¿qué requisitos pides?" → se contestó solo el freno)
    const restoF = t.replace(new RegExp(RE_FRENO_C.source, 'g'), ' ');
    const traePregunta = /\?/.test(String(texto || '')) || /\b(que|cual|cuales|cuanto|cuantos|como|donde|cuando|requisit\w*|papel\w*|document\w*|precio|fotos?|ubicaci\w*|kilometr\w*|km|credito|enganche|mensualidad|factura|duenos?)\b/.test(restoF);
    if (RE_FRENO_C.test(t) && !traePregunta) {
    ...
    if (!u && (enganche || plazo) && /\d/.test(String(texto || ''))) u = 'financiamiento';   // Haiku extrajo enganche/plazo → cotización (solo si ÉL escribió el número)
    ...
    else if (u === 'financiamiento') { let yaCot = false; try { if (conv_id) yaCot = !!(await require('./etapa3.js').estadoConv(conv_id)).ya_cotizado; } catch (e) { } r = await responderFin({ texto, nombre, auto_id, enganche, plazo, yaCotizado: yaCot }); }
    ...
    else if (u === 'disponible') r = await responderDisponible({ auto_id });
...
module.exports = { responderCont, universoCont, datosPunto, esCamioneta, ciudadCercana, REQUISITOS, horarioDelUniverso };
