...
async function responderFin({ texto, nombre, auto_id, enganche, plazo }) {
    ...
    if (esCotizar) {
        ...
        return { _sintonia: sintonia, segmentos: [r1, card] };
    }
    // 📤 REQUISITOS — la lista es la lista, sin CTA de venta.
    if (esRequisitos) {
        const { c, sintonia } = await conectar({ texto, accion: REQUISITOS, pool: ['Va', 'Con gusto', 'Claro'], nm });
        return { _sintonia: sintonia, segmentos: [`${c}, con gusto`, REQUISITOS] };
    }
    ...
    if (esBanco) {
        ...
        return { _sintonia: sintonia, segmentos: [c, lineaBanco] };
    }
    // PROCESO/TASA/BURÓ/APROBACIÓN = venta → ESCALA (doctrina 2026-07-09).
    if (esAprob) return { escalar: true, motivo: 'pregunta la APROBACIÓN del crédito (proceso de cierre — tuyo)', puente: 'Déjame te confirmo bien ese punto del crédito y aquí mismo te digo' };
    if (esTasa) return { escalar: true, motivo: 'pregunta la TASA (política de crédito — tuya)', puente: 'Déjame te confirmo bien ese punto del crédito y aquí mismo te digo' };
    if (esBuro) return { escalar: true, motivo: 'pregunta BURÓ (decisión de crédito — tuya)', puente: 'Déjame te confirmo bien ese punto del crédito y aquí mismo te digo' };
    // "¿Financian?" (sí/no) = hecho: SÍ + banco. El CÓMO del proceso ya es venta → escala.
    if (/(tienen|manejan|aceptan|hay|dan|cuentan con|se puede|puedo)[^?]{0,30}(financia|credito|a meses|mensualidades|financiado)|financian/.test(t)) {
        return { segmentos: [rot(['Sí, así es', 'Claro que sí']), `Es con ${banco}`] };
    }
...
async function responderCont({ texto, nombre, auto_id, enganche, plazo, intencion }) {
    ...
    // ── PUERTA DE ENTRADA (doctrina 2026-07-09): momentos de gol → puente + owner ──
    ...
    if (/(te aviso|les aviso|me comunico|te confirmo|lo pienso|lo platico|lo consulto|deja(me)? chec|deja(me)? veo\b(?! (el|la|lo|los|las|foto))|deja(me)? ver (si|que|como|cuando|mis|mi)\b|deja(me)? (progr\w+|cuadrar|acomodar|organizar)|me programo|dame (chance|tiempo|oportunidad)|aguantame|esperame|(ando|estoy|he andado) (muy |bien+ |re )?(atareado|atariado|ocupado|saturado)|tengo (varios )?compromisos)/.test(t)) {
    ...
    // ESCALA con puente desde cualquier banco (doctrina) → pasa tal cual al caller.
    if (r && r.escalar) return r;
