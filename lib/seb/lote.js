...
function identidad(texto, t, asistente) {
    if (!t || Number(t.id) === 0) return texto;
    const m = asistente ? null : marcaCorta(t); let s = String(texto || '');
    if (asistente) {   // AUTOS FYRADRIVE (3 partes): el asistente es de UN vendedor, nunca del lote ni de Fyradrive (owner 2026-09-24)
        const a = 'Seb, asistente virtual de ' + asistente;
        s = s.replace(/Mucho gusto, mi nombre es Sebasti[aá]n Romero(, para servirte)?/g, 'Soy ' + a).replace(/\b[Ss]oy Seb(asti[aá]n( Romero)?)? de Fyradrive\b/g, 'soy ' + a);
        s = s.replace(/mi nombre es Sebasti[aá]n Romero/gi, 'soy ' + a).replace(/soy Sebasti[aá]n Romero/gi, 'soy ' + a).replace(/Sebasti[aá]n Romero/g, a).replace(/[Ss]oy el asistente de [^.,!?\n]+/g, 'soy ' + a);
        return s;
    }
...
async function turno({ tenant, chat, texto, catalogo, asistente }) {
    ...
    if (!lote.fase) { await guardarLote(tenant, tel, ej, { fase: 'pregunta', intentos: 0 }); return { segmentos: [asistente ? ('Qué tal, soy Seb, asistente virtual de ' + asistente + '. Estoy aquí para responderte lo básico; si ocupas algo más a fondo, dime "hablar con ' + asistente + '". ¿Qué auto estás buscando? Dime marca, tipo de auto o presupuesto y te digo qué tenemos.') : 'Hola, soy el asistente de ' + m + '. Claro que sí, ¿qué estás buscando? Dime marca, tipo de auto o presupuesto y te digo qué tenemos.'], foco: null, escalar: false, motivo: 'pregunta_busqueda' }; }
