...
async function turnoIgnacio({ telefono, convId, desdeTs }) {
    ...
    // dueños). Un DUEÑO CONOCIDO además puede volver CUANDO SEA ("te subo otro
    // auto") — su conversación vieja no lo bloquea.
    const s = await sesionActiva(telefono);
    if (!s) {
        const abrePrimero = bursts === 0 && (esVendedorTexto(rafaga) || RE_FICHA_SENAL.test(rafaga));
        let abreConocido = false;
        if (!abrePrimero) {
            const dcG = await duenoConocido(telefono).catch(() => null);
            abreConocido = !!(dcG && (esVendedorTexto(rafaga) || RE_FICHA_SENAL.test(rafaga) || RE_OTRO_AUTO.test(rafaga)));
        }
        if (!abrePrimero && !abreConocido) return { activo: false, motivo: 'no es primer contacto de vendedor ni hay sesión' };
    }
