...
const P = {
    ...
    // "¿DÓNDE SE UBICAN?" — borrador (el owner calibra el texto)
    dondeEstamos: () => [`Trabajamos en Monterrey y toda su área metropolitana 🙌 La cita para que vean tu auto y la venta son en punto seguro que nosotros coordinamos — más adelante lo cuadramos contigo.`],
...
async function procesarMensaje({ telefono, texto, historial, ultimoManualTs }) {
    ...
    let noEntendido = false, pideInfo = false, pideCompran = false, pideUbican = false, respuestaJuez = null;
    {
        ...
        if (RE_COMPRAN.test(txt)) {
            ...
            // "¿dónde se ubican?" → llave determinista (antes caía al juez y confundía)
            pideUbican = true;
            traza.push(`[TRIGGER UBICAN regex] "${txt.match(RE_UBICAN)[0]}" → plantilla dondeEstamos`);
    ...
    if ((pideInfo || pideCompran || pideUbican) && f.length) {
        traza.push('[PLANTILLA] machote ' + (pideCompran ? 'compranAutos' : pideUbican ? 'dondeEstamos' : 'de información del owner'));
        out.segmentos = pideCompran ? P.compranAutos() : pideUbican ? P.dondeEstamos() : P.comoFunciona();
        if (hayAlgo || pideUbican) out.segmentos = out.segmentos.concat(P.progreso(s));
    }
