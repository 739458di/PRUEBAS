...
const RE_FICHA_SENAL = /(\d[\d.,]*\s*(mil\s*)?km\b|kilometraje|factura( de)? (agencia|original|endosada)|[uú]nico dueño|\d+\s*dueñ[oa]s?)/i;
...
async function procesarMensaje({ telefono, texto, historial, ultimoManualTs }) {
    ...
    if (!out.despierta && s.estado === 'recepcion' && (s.fotos || []).length && s.datos.marca == null && s.datos.precio == null && !s.datos._confirmado) {
        const esConfirm = esVendedorTexto(texto) || /^(s[ií]+|ok|va|claro|sim[oó]n|as[ií] es|correcto|exacto|aja|aj[aá])/i.test(String(texto).trim());
        if (esConfirm) {
    ...
    let noEntendido = false, pideInfo = false, pideCompran = false, respuestaJuez = null;
    {
        ...
        } else if (RE_UBICAN.test(txt)) {
            // "¿dónde se ubican?" → ESCALA MUDA determinista (orden owner 2026-07-16:
            // la ubicación la contesta ÉL; ni machote ni juez)
            noEntendido = true;
            traza.push(`[TRIGGER UBICAN regex] "${txt.match(RE_UBICAN)[0]}" → ESCALA MUDA (ubicación = del owner)`);
            const avisoU = `🟡 Vendedor ${telefono} pregunta UBICACIÓN: «${txt.slice(0, 120)}» — contéstale tú (yo sigo juntando la ficha)`;
            out.avisoOwner = out.avisoOwner ? out.avisoOwner + '\n' + avisoU : avisoU;
...
async function agregarFotos({ telefono, urls }) {
    ...
    if (!s) {
        ...
        await guardarSesion(telefono, s);
        traza.push(`[FOTOS INICIADOR 📸] sin sesión + ${(urls || []).length} foto(s) en primer contacto → Ignacio despierta a confirmar venta`);
        out.activo = true;
        out.despierta = true;
        out.avisoOwner = `🟢 Ignacio despertó con ${telefono} — mandó fotos de un auto (¿vendedor?)`;
        out.segmentos = P.fotosIniciador();
        out.checklist = { datos: s.datos, fotos: s.fotos.length, faltan: faltantes(s) };
        return out;
    }
    out.activo = true;
...
async function turnoIgnacio({ telefono, convId, desdeTs }) {
    ...
    // contacto que suena a vendedor O que abre con DATOS de ficha (km, factura,
    // dueños — así abren los vendedores; el doble candado Haiku confirma después).
    const s = await sesionActiva(telefono);
    if (!s && !(bursts === 0 && (esVendedorTexto(rafaga) || RE_FICHA_SENAL.test(rafaga)))) return { activo: false, motivo: 'no es primer contacto de vendedor ni hay sesión' };
...
module.exports = { procesarMensaje, agregarFotos, sesionActiva, resetSesion, esVendedorTexto, RE_DINERO, ensureRecepcion, publicarSesion, publicarPendiente, OWNER_RECEPCION, juezRecepcion, candadoJuez, arrancarPendiente, turnoIgnacio, aprobarSesionSimulada, plantillaPublicado: () => P.publicado() };
