...
// (caso sandbox 2026-07-18: "¿ustedes me venden el auto?" — LA pregunta del negocio
// caía al juez y se confundía → llave determinista: el pitch la contesta)
const RE_INFO = /(c[oó]mo funciona[ns]?|c[oó]mo le hacen|c[oó]mo trabajan|c[oó]mo es el (proceso|rollo|tema|asunto)|en qu[eé] consiste|c[oó]mo est[aá] el (rollo|tema|asunto)|m[aá]s informaci[oó]n|informaci[oó]n para vender|dar(me)? informes|informes para vender|qu[eé] necesito para vender|c[oó]mo puedo vender|es gratis( publicar(lo)?)?\??|tiene alg[uú]n costo|cobran algo|cu[aá]nto sale publicar|ustedes (me )?(lo )?venden|me lo venden( ustedes)?|se encargan de vender(lo|le)?|lo venden por m[ií]|ustedes lo (mueven|ofrecen|promocionan)|qui[eé]n lo vende)/i;
...
async function juezRecepcion(texto, historial) {
    // caso sandbox 2026-07-18 (v2): el MENSAJE ACTUAL va PRIMERO y etiquetado; el
    // historial va al final marcado como YA ATENDIDO — Haiku lo usaba como la
    // pregunta y escalaba con razones del pasado. Y regla nueva: si los HECHOS
    // contestan la duda CLARITO, PREFIERE responder (no escalar lo obvio).
    return haiku(
        'Eres el juez de Ignacio (recepción de VENDEDORES de autos de Fyradrive, Monterrey). Tu trabajo es decidir cómo tratar el MENSAJE ACTUAL (el primero que te paso, entre «»). El HISTORIAL VIEJO va al final y es SOLO contexto: todo lo de ahí YA FUE ATENDIDO — jamás lo re-trates ni lo uses como razón. Tu "razon" debe describir el MENSAJE ACTUAL, nada más. Acciones: "machote_info" = pregunta cómo funciona el servicio / si nosotros se lo vendemos → pitch. "machote_compran" = pregunta si le COMPRAMOS su auto directo. "responder" = la duda del MENSAJE ACTUAL se contesta con los HECHOS (1-2 frases que responden EXACTAMENTE lo que pregunta; tono amable y sobrio, sin "compa"/"carnal"; JAMÁS inventes cifras, montos, porcentajes ni plazos). REGLA: si los HECHOS contestan la duda CLARITO, PREFIERE "responder" o el machote — NO escales lo obvio. "escalar" = solo lo que los hechos NO contestan: temas legales delicados, negociación, MONTOS, UBICACIÓN física (dónde estamos/dónde sería la cita — eso lo contesta Sebastián), casos particulares del auto. Ante duda REAL → escalar. Responde SOLO el JSON.',
        { type: 'object', properties: {
            razon: { type: 'string', description: 'una línea sobre el MENSAJE ACTUAL: qué pregunta AHORITA' },
        ...
        'MENSAJE ACTUAL DEL VENDEDOR (contesta SOLO esto):\n«' + String(texto).slice(-400) + '»\n\nHECHOS (lo único que sabes):\n' + SKILL_HECHOS + '\n\nHISTORIAL VIEJO (ya atendido — solo contexto, JAMÁS lo re-trates):\n' + (historial || '(sin historial)'),
        400);
}
...
// CUOTA TURSO (2026-09-08): los ensure* corren UNA vez por instancia (antes: un DDL por
// cada llamada, varias por mensaje entrante). Las tablas ya existen.
let _ensDuenos = false, _ensRec = false, _ensTurnos = false;
async function ensureDuenos() {
    if (_ensDuenos) return;
    _ensDuenos = true;
...
// `info` (opcional): info.espero=true si este turno tuvo que ESPERAR a otro (entonces la
// libreta pre-leída por el caller ya no es fresca y turnoIgnacio la vuelve a leer).
async function tomarTurno(telefono, esperar, info) {
    if (!_ensTurnos) { _ensTurnos = true; await run("CREATE TABLE IF NOT EXISTS recepcion_turnos (telefono TEXT PRIMARY KEY, ts INTEGER)").catch(() => { }); }
...
async function procesarMensaje({ telefono, texto, historial, ultimoManualTs }) {
    ...
    if (!s) {
        ...
        if (!despierta) {
            // caso sandbox 2026-07-18: "¿compran autos?" — el doble candado no abre
            // sesión, pero la pregunta de SERVICIO se contesta igual (machote, sin
            // sesión; si luego manda su ficha, ahí sí despierta completo). Mudo jamás.
            if (RE_COMPRAN.test(texto) || RE_INFO.test(texto)) {
                out.activo = true;
                out.segmentos = RE_COMPRAN.test(texto) ? P.compranAutos() : P.comoFunciona();
                traza.push('[MACHOTE SIN SESIÓN] doble candado no abrió sesión, pero la pregunta de servicio se contesta igual');
                return out;
            }
            return out;
        }
    ...
    if (!out.despierta && s.estado === 'recepcion' && (s.fotos || []).length && s.datos.marca == null && s.datos.precio == null && !s.datos._confirmado) {
        ...
        // caso sandbox 2026-07-18: pregunta de SERVICIO ("¿cómo funcionan?", "¿compran?",
        // dinero, ubicación) es 100% compatible con un vendedor que acaba de mandar
        // fotos → JAMÁS dispara el escape; la contesta la escalera normal y la sesión sigue.
        const esPreguntaServicio = RE_INFO.test(texto) || RE_COMPRAN.test(texto) || RE_DINERO.test(texto) || RE_UBICAN.test(texto);
        // señal CLARA de comprador (determinista): pregunta por un auto PUBLICADO nuestro
        const esCompradorClaro = /(sigue disponible|todav[ií]a lo (tienen|venden)|me interesa (el|la|este|esta|ese|esa)|que publicaron|del anuncio|de la p[aá]gina|cu[aá]nto (cuesta|piden|sale el)|puedo (ir a )?verlo|informaci[oó]n (del|de la) (auto|carro|camioneta)|est[aá] a la venta)/i.test(texto);
        if (esConfirm) {
            s.datos._confirmado = 1;
            traza.push('[ESCAPE] el texto confirma vendedor → la sesión queda en firme');
        } else if (esPreguntaServicio) {
            traza.push('[ESCAPE] pregunta de servicio (compatible con vendedor) → la sesión sigue; contesta la escalera');
        } else if (esCompradorClaro) {
            await resetSesion(telefono);
            traza.push('[ESCAPE 🚪] señal CLARA de comprador (pregunta por un auto publicado) → sesión cerrada, flujo normal');
            out.activo = false;
            out.motivo = 'era comprador (fotos de anuncio)';
            return out;
        } else {
            // ambiguo → Haiku, CON el contexto de que abrió mandando fotos (sesgo vendedor)
            const c0 = await clasificarVendedorIA('[CONTEXTO: esta persona ABRIÓ el chat mandando FOTOS de un auto, sin texto — típico de quien quiere VENDER el suyo] ' + String(texto));
...
async function publicarSesion(telefono) {
    ...
    // K_PANEL por header (la web la verifica); transición: body.key = KEY_VIEJA mientras la web no se actualice. Sin literales.
    if (!process.env.K_PANEL) return { ok: false, error: 'K_PANEL no configurada', sesion: s };
...
// mensajesPre (opcional, CUOTA TURSO 2026-09-08): la libreta que el caller YA leyó en este
// mismo request ({mensaje, direccion, ts, ai}, ya filtrada por desdeTs). Se usa tal cual
// SOLO si el turno entró sin esperar; si tuvo que esperar a otro turno, se re-lee fresca
// (misma garantía de siempre: la ráfaga ya trae ambos mensajes).
async function turnoIgnacio({ telefono, convId, desdeTs, forzarVenta, mensajesPre }) {
    if (!convId) {
    ...
    const infoTurno = { espero: false };
    const turnoOk = await tomarTurno(telefono, true, infoTurno).catch(() => true);
    if (!turnoOk) return { activo: false, motivo: 'otro turno en proceso — la ráfaga lo recoge' };
    try {
        let mensajes;
        if (Array.isArray(mensajesPre) && !infoTurno.espero) {
            mensajes = mensajesPre.map(m => ({ mensaje: m.mensaje || '', direccion: m.direccion, ts: Number(m.ts), ai: Number(m.ai) || 0 }));
        } else {
            ...
            mensajes = mr.map(m => ({ mensaje: m.texto || '', direccion: m.direccion, ts: Number(m.ts), ai: Number(m.ai_generated) || 0 }));
        }
        ...
        const s = await sesionActiva(telefono);
        if (!s) {
            // forzarVenta (LEY DEL OPENER): el bot preguntó "¿comprar o vender?" y
            // contestó vender — la compuerta se abre aunque la frase sea suave.
            const abreFuerte = !!forzarVenta || esVendedorTexto(rafaga);
...
module.exports = { procesarMensaje, agregarFotos, sesionActiva, resetSesion, esVendedorTexto, RE_DINERO, ensureRecepcion, publicarSesion, publicarPendiente, juezRecepcion, candadoJuez, arrancarPendiente, turnoIgnacio, aprobarSesionSimulada, plantillaPublicado: () => P.publicado(), duenoConocido, registrarDueno, olvidarDueno };
