...
const RE_DINERO = /(cu[aá]nto me (dan|ofrecen|pagan)|comisi[oó]n|cu[aá]nto cobran|qu[eé] cobran|valuaci[oó]n|val[uú]en|val[uú]ame|cu[aá]nto vale|me lo compran ustedes|ustedes lo compran)/i;
...
function faltantes(s) {
    const f = [];
    // el PRECIO se pide con la frase del owner (2026-07-16), aparte de la ficha
    const fx = REQUISITOS.filter(k => k !== 'precio' && !s.datos[k]);
    if (fx.length) f.push('la ficha (' + fx.map(k => k === 'anio' ? 'año' : k).join(', ') + ')');
    if (!s.datos.precio) f.push('a qué precio lo quieres vender');
...
async function procesarMensaje({ telefono, texto }) {
    ...
    let noEntendido = false, pideInfo = false;
    {
    ...
    // pidió INFO → machote del owner (sustituye al saludo: ES el pitch completo);
    // si ya hay algo capturado, el progreso va de pilón para seguir jalando la ficha
    if (pideInfo && f.length) {
        traza.push('[PLANTILLA] machote de información del owner');
        out.segmentos = P.comoFunciona();
