...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // ══ SIMULADOR DE 3 PARTES (orden owner 2026-09-26): el MISMO motor real, en el universo sandbox (LAB). Nada sale a WhatsApp. ══
        //    sim3_preparar (Mario simulado + autos) · sim3_estado (los 2 chats + ronda + reloj + lo que sigue) · sim3_reloj (adelanta paso a paso) · sim3_limpiar
        if (['sim3_preparar', 'sim3_estado', 'sim3_reloj', 'sim3_limpiar'].includes(action)) {
            const tSm = await tenantDeParam(VEND_PARAM || String(SES ? SES.tenant_id : ''));
            if (!tSm || !tSm.demo || !CITA3.activo(tSm) || !(SES_DEMO || MAESTRA)) return res.status(403).json({ ok: false, error: 'El simulador solo corre en el universo de pruebas con citas de 3 partes (AUTOS FYRADRIVE SIMULADOR), con tu sesión de owner.' });
            const TELM = '5210000000096';
            const marioSim = async () => { let m = (await query('SELECT id, nombre FROM vendedores_universo WHERE tenant_id = ? AND telefono = ?', [Number(tSm.id), TELM]))[0]; if (!m) { await run("INSERT INTO vendedores_universo (tenant_id, nombre, telefono, activo, created, rol) VALUES (?, 'Mario Simulado', ?, 1, ?, 'vendedor')", [Number(tSm.id), TELM, Date.now()]); m = (await query('SELECT id, nombre FROM vendedores_universo WHERE tenant_id = ? AND telefono = ?', [Number(tSm.id), TELM]))[0]; } else await run('UPDATE vendedores_universo SET activo = 1 WHERE id = ?', [Number(m.id)]); return { id: Number(m.id), nombre: m.nombre }; };
            const chatsSim = async () => { const m = await marioSim(); return (await query("SELECT id FROM conversaciones WHERE tenant_id = ? AND (miembro_id = ? OR canal = 'dueno')", [Number(tSm.id), m.id])).map(r => Number(r.id)); };
            const msgs = async (cid) => cid ? (await query("SELECT id, direccion, emisor, texto, tipo, ts FROM mensajes WHERE conversacion_id = ? ORDER BY id DESC LIMIT 120", [Number(cid)])).reverse().map(x => ({ id: Number(x.id), dir: x.direccion, emisor: x.emisor, texto: x.tipo === 'location' ? '📍 ' + String(x.texto || '').split('|||')[0] : (x.tipo === 'image' ? '🖼 imagen' : x.texto), tipo: x.tipo, ts: Number(x.ts) })) : [];
            if (action === 'sim3_preparar') {
                const m = await marioSim();
                let cat = await autosDeTenant(tSm);
                if (cat.length < 4) {   // autos de particulares con ubicación en su ficha, acreditados SOLO en el LAB (no tocan ningún universo real)
                    const cand = await query("SELECT i.id FROM inventario_autos i WHERE i.estado = 'activo' AND i.fyradrive_web_id IS NOT NULL AND EXISTS (SELECT 1 FROM punto_envio p WHERE p.auto_id = i.id) AND NOT EXISTS (SELECT 1 FROM autos_universo a WHERE a.inv_auto_id = i.id AND a.tenant_id = ?) AND NOT EXISTS (SELECT 1 FROM autos_universo a JOIN tenants tt ON tt.id = a.tenant_id WHERE a.inv_auto_id = i.id AND a.rol = 'dueno' AND tt.id IN (9,10,11)) ORDER BY i.id DESC LIMIT ?", [Number(tSm.id), 5 - cat.length]);
                    for (const c of cand) await run('INSERT OR IGNORE INTO autos_universo (inv_auto_id, tenant_id, rol, origen, activo, created, updated) VALUES (?,?,?,?,1,?,?)', [Number(c.id), Number(tSm.id), 'acreditado', 'sim3', Date.now(), Date.now()]);
                    cat = await autosDeTenant(tSm);
                }
                const sims = await query("SELECT c.id, c.nombre, c.telefono, c.ult_msg_ts FROM conversaciones c WHERE c.tenant_id = ? AND c.miembro_id = ? AND COALESCE(c.canal,'') <> 'dueno' ORDER BY c.id DESC LIMIT 20", [Number(tSm.id), m.id]);
                return res.status(200).json({ ok: true, mario: m, universo: { id: Number(tSm.id), nombre: tSm.nombre }, autos: cat.map(a => ({ id: Number(a.id), web: a.fyradrive_web_id, nombre: [a.marca, a.modelo, a.anio].filter(Boolean).join(' '), precio: a.precio })), simulaciones: sims.map(x => ({ chat_id: Number(x.id), nombre: x.nombre, telefono: x.telefono })) });
            }
            if (action === 'sim3_limpiar' && req.method === 'POST') {
                const ids = await chatsSim(); let n = 0;
                for (const cid of ids) {
                    for (const c3 of await query('SELECT cita_id FROM cita3 WHERE chat_id = ? OR dueno_chat_id = ?', [cid, cid])) await CITA3.borrarDeCita(c3.cita_id);
                    const telC = ((await query('SELECT telefono FROM conversaciones WHERE id = ?', [cid]))[0] || {}).telefono;
                    await CITAF.borrarDeChat(cid).catch(() => { });
                    for (const [sq, a] of [['DELETE FROM mensajes WHERE conversacion_id = ?', [cid]], ['DELETE FROM seb_turnos WHERE chat_id = ?', [cid]], ['DELETE FROM acciones WHERE chat_id = ?', [cid]], ['DELETE FROM delegaciones WHERE chat_id = ?', [cid]], ['DELETE FROM chats_activos WHERE chat_id = ?', [cid]], ['DELETE FROM citaf_reloj WHERE tenant_id = ?', [CITAF.RELOJ_BASE + cid]], ['DELETE FROM conversaciones WHERE id = ?', [cid]]]) await run(sq, a).catch(() => { });
                    if (telC) { await run('DELETE FROM escalas_log WHERE telefono = ?', [telC]).catch(() => { }); await run('DELETE FROM ad_por_telefono WHERE telefono = ?', [telC]).catch(() => { }); }
                    n++;
                }
                return res.status(200).json({ ok: true, borrados: n });
            }
            const chatB = await U.chatPorId(Number(req.body.chat_id || req.query.chat_id) || 0);
            if (!chatB || Number(chatB.tenant_id) !== Number(tSm.id)) return res.status(404).json({ ok: false, error: 'chat inexistente en el simulador' });
            if (action === 'sim3_reloj' && req.method === 'POST') {
                const off = await CITAF.offsetDe(chatB.id); const ahoraV = Date.now() + off; const acc = String(req.body.accion || 'mas'); let destino;
            ...
            const off = await CITAF.offsetDe(chatB.id); const ahoraV = Date.now() + off;
            const c3 = await CITA3.porChat(tSm.id, chatB.id) || (await query('SELECT * FROM cita3 WHERE chat_id = ? ORDER BY cita_id DESC LIMIT 1', [chatB.id]))[0] || null;
            const tres = c3 ? await CITA3.estado(c3.cita_id) : null;
            const chD = c3 && c3.dueno_chat_id ? await U.chatPorId(c3.dueno_chat_id) : null;
            const cf = await CITAF.citaViva(tSm.id, chatB.id);
            const foco = await focoDe(tSm, String(chatB.telefono));
            return res.status(200).json({ ok: true, reloj: { offset_ms: off, ahora_ts: ahoraV, ahora: CITAF.fechaCorta(ahoraV) },
                comprador: { chat_id: Number(chatB.id), nombre: chatB.nombre, telefono: chatB.telefono, voz: VOZ.vozDe(chatB), foco: foco ? foco.nombre : null, mensajes: await msgs(chatB.id) },
                dueno: chD ? { chat_id: Number(chD.id), nombre: chD.nombre, telefono: chD.telefono, voz: VOZ.vozDe(chD), mensajes: await msgs(chD.id) } : null,
                cita_motor: cf ? { id: Number(cf.id), estado: cf.estado, precision: cf.precision, cuando: CITAF._t.cuandoCorto(cf, ahoraV) } : null,
                tres, proximos: (await CITA3.proximo(chatB.id)).slice(0, 6).map(p => ({ k: p.k, que: CITA3.K_TXT[String(p.k).split(':')[0]] || p.k, due: p.due, cuando: CITAF.fechaCorta(p.due) })) });
        }
        ...
        if (['citaf_estado', 'citaf_reloj', 'citaf_vendedor', 'citaf_tablero', 'citaf_calendario', 'citaf_reiniciar'].includes(action) && req.method === 'POST') {
            ...
            if (action === 'citaf_reloj') {   // RELOJ DE ESTE CLIENTE DE PRUEBA (película): avanzar EJECUTA lo que tocaba; retroceder REBOBINA (regresa mensajes y estado)
                ...
                else { await CITAF.ponerOffset(chC.id, destino - Date.now()); hechas = await CITAF.tick({ tenant: tC, ioDe, chatId: Number(chC.id), hasta: destino }); if (CITA3.activo(tC)) { try { hechas = hechas.concat((await CITA3.tick({ chatId: Number(chC.id), hasta: destino })).map(x => ({ tipo: 'tres:' + x.k, salio: true }))); } catch (e) { } } }
        ...
        if (action === 'seb_turno' && req.method === 'POST') {
            ...
            const AGENTE = !!(tS.config && Number(tS.config.agente) === 1);   // SEB AGENTE (owner 2026-09-27): un agente con herramientas reemplaza la cadena vieja del cerebro
            // MARIO AGREGADOR: si Mario tomó el chat con el botón y lleva 30 min sin escribir, Seb regresa solo (ya no hay que picar "Devolver a Seb")
            if (AGENTE && VOZ.vozDe(chS) === 'humano' && String(chS.canal || '') !== 'dueno') { try { const um = (await query("SELECT MAX(ts) t FROM mensajes WHERE conversacion_id = ? AND direccion = 'out' AND COALESCE(ai_generated,0) = 0 AND COALESCE(emisor,'') <> 'sistema'", [Number(chS.id)]))[0]; const ahoraM = Date.now() + await CITAF.offsetDe(chS.id); if (!um || !um.t || ahoraM - Number(um.t) > 30 * 60000) { await VOZ.devolver({ tenant: tS, chat: chS }); chS.estado_bot = 'seb'; } } catch (e) { } }
            ...
            // ══ 3 PARTES · "HABLAR CON MARIO" (determinista): la voz pasa al vendedor del chat en el acto ══
            if (CITA3.activo(tS)) {
                try {
                    const uiH = (await query("SELECT texto FROM mensajes WHERE conversacion_id = ? AND direccion = 'in' ORDER BY id DESC LIMIT 1", [Number(chS.id)]))[0]; const nH = String((uiH && uiH.texto) || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
                    const asisH = await asistenteDe(tS, chS.id);
                    if (/\b(hablar con|comunicame|que me (marque|llame|hable)|pasame con|quiero hablar)\b/.test(nH)) {
                        await MSJ.enviar({ tenantId: Number(tS.id), chatId: Number(chS.id), origen: 'sb', clave: 'humano:' + Number(chS.id) + ':' + Date.now(), manual: false, accion: 'seb_turno', voz_gen: VOZ_GEN, texto: 'Claro que sí, ahorita te contesta ' + (asisH || 'el vendedor') + ' personalmente.' });
                        await VOZ.entregar({ tenant: tS, chat: chS, motivo: 'pidió hablar con ' + (asisH || 'el vendedor') });
                        return res.status(200).json({ ok: true, chat_id: Number(chS.id), seb: { ok: true, modo: 'humano', tipo: 'pidio_vendedor', segmentos: 1 } });
                    }
            ...
            try {
                ...
                TURNO_IN = Number(ui.m); try { MSJ.TURNO.enterWith({ chatId: Number(chS.id), inId: TURNO_IN }); } catch (e) { }   // candado de turno en la puerta de salida
            ...
            if (TURNO_IN && !(req.body && req.body.sin_espera)) {
                for (let k = 0; k < 12; k++) { const u = await ultimoIn(); if (!u || Number(u.id) !== TURNO_IN) break; const falta = 10000 - (Date.now() - Number(u.created_at || 0)); if (!(falta > 0) || falta > 60000) break; await new Promise(r => setTimeout(r, Math.min(falta, 2500))); }   // 10 s de silencio del comprador antes de contestar (orden owner: que alcance a leer los mensajes completos)
            ...
            if (esLote && !AGENTE) {   // con el agente, las reglas de palabras que contestaban solas se quitan: el agente lee el bloque completo
                try {
                    ...
                    { const nS = ultimoInTxt.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
                      if (focoL && nS.length <= 30 && /^\W*(hola|holi|buen(os|as)?( dias| tardes| noches)?|que tal|hey|saludos|que onda)\W*$/.test(nS) && !(await query("SELECT 1 FROM mensajes WHERE conversacion_id = ? AND direccion = 'out' AND COALESCE(emisor,'') <> 'sistema' LIMIT 1", [Number(chS.id)]).catch(() => []))[0]) { /* saludo solo: se saluda SOLO si Seb no ha hablado; si ya habló, es cortesía (owner 2026-09-26: "qué tal" tras la cotización no se contesta) */ const asisG = await asistenteDe(tS, chS.id); const nomG = String(chS.nombre || '').trim().split(/\s+/)[0]; await mandarL('¡Qué tal' + (nomG && !/^\+?\d/.test(nomG) ? ' ' + nomG : '') + '! Aquí estoy para ayudarte con el ' + focoL.nombre + (asisG ? '' : '') + '. ¿Qué te gustaría saber?'); await notaL('🏬 saludo (sin escalar)'); return salirL('saludo', 1); }
                      if (focoL && nS.length <= 30 && /^\W*(hola|holi|buen(os|as)?( dias| tardes| noches)?|que tal|hey|saludos|que onda)\W*$/.test(nS)) { await notaL('🏬 saludo de cortesía (Seb ya había hablado: sin respuesta, sin escalar)'); return salirL('saludo_cortesia', 0); }
                    ...
                    if (CITAF.activo(tS) && LOTE.esAgenda(ultimoInTxt) && !(CITA3.activo(tS) && await CITA3.porChat(tS.id, chS.id))) {
            ...
            // ══ SEB AGENTE: un turno del agente con las herramientas del sistema (mismas puertas que los botones) ══
            const correrAgente = async (soloTexto) => {
                const AG = require('../lib/seb/agente.js'); const LIB_E3 = require('../lib/seb/etapa3.js');
                const ms = (await query("SELECT direccion, emisor, texto, ai_generated FROM mensajes WHERE conversacion_id = ? AND COALESCE(emisor,'') <> 'sistema' ORDER BY id DESC LIMIT 20", [Number(chS.id)])).reverse();
                let iB = ms.length; while (iB > 0 && ms[iB - 1].direccion === 'in') iB--;
                const bloque = soloTexto ? [String(soloTexto)] : ms.slice(iB).map(m => String(m.texto || '')).filter(Boolean);
                if (!bloque.length) return { out: { ok: false, motivo: 'sin_entrantes' }, enviados: [] };
                const asis = await asistenteDe(tS, chS.id); const foco = await focoDe(tS, telS);
                let duenoTipo = null; if (CITA3.activo(tS) && foco) { try { const d = await CITA3.duenoDe(tS, foco.id); duenoTipo = d && d.tipo; } catch (e) { } }
                let cita = null; try { cita = CITA3.activo(tS) ? await CITA3.fotoTexto(tS, chS) : null; if (!cita) { const cf = await CITAF.citaViva(tS.id, chS.id); if (cf) cita = 'visita ' + cf.estado + ' · ' + CITAF._t.cuandoCorto(cf, Date.now() + await CITAF.offsetDe(chS.id), true); } } catch (e) { }
                let pendienteMario = null; try { const pm = (await query("SELECT id, texto FROM mensajes WHERE conversacion_id = ? AND emisor = 'sistema' AND texto LIKE '🔔 Para ti:%' ORDER BY id DESC LIMIT 1", [Number(chS.id)]))[0]; const mm = (await query("SELECT MAX(id) m FROM mensajes WHERE conversacion_id = ? AND direccion = 'out' AND COALESCE(ai_generated,0) = 0 AND COALESCE(emisor,'') <> 'sistema'", [Number(chS.id)]))[0]; if (pm && !(mm && Number(mm.m) > Number(pm.id))) pendienteMario = String(pm.texto).replace(/^🔔 Para ti: /, '').replace(/ — contéstale.*$/, ''); } catch (e) { }
                let obligatorio = null; try { const g = require('../lib/seb/doctrina.js').puertaEntrada(bloque.join(' ')); if (g) obligatorio = String(g.motivo || '').replace(/^[\s🔴🔥💰🧠]+/, '').slice(0, 120); } catch (e) { }
                const est = await LIB_E3.estadoConv(Number(chS.id)).catch(() => ({}));
                const hist = ms.slice(Math.max(0, iB - 12), iB).map(m => ({ quien: m.direccion === 'in' ? 'Comprador' : (Number(m.ai_generated) ? 'Seb' : (asis || 'Vendedor')), texto: m.texto }));
                const ctx = { lote: LOTE.marcaCorta ? LOTE.marcaCorta(tS) : tS.nombre, asistente: asis, tres: CITA3.activo(tS), duenoTipo, foco, cita, pendienteMario, obligatorio, est, historial: hist, bloque };
                const base = 'agente:' + Number(chS.id) + ':' + Date.now(); let n = 0; const enviados = [];
                const deps = {
                    ejecutar: (acc, datos) => ejecutarAccion(tS, telS, acc, Object.assign({ via: 'agente', clave: base + ':a' + (n++) }, datos)),
                    mandarTexto: async (tx) => { const e = await MSJ.enviar({ tenantId: Number(tS.id), chatId: Number(chS.id), origen: 'sb', clave: base + ':' + (n++), manual: false, accion: 'seb_agente', voz_gen: VOZ.genDe(chS), texto: LOTE.identidad(tx, tS, asis) }); enviados.push(e); return e; },
                    nota: async (tx) => { try { if (tS.demo) await DEMO.sistema(tS, telS, tx); else { const ts = Date.now(); await run("INSERT OR IGNORE INTO mensajes (conversacion_id, msg_id, ts, direccion, emisor, texto, tipo, ai_generated, created_at) VALUES (?,?,?,?,?,?,?,?,?)", [Number(chS.id), 'agente-nota:' + ts + ':' + (n++), ts, 'out', 'sistema', tx, 'text', 1, ts]); } } catch (e) { } },
                    escalar: (motivo) => VOZ.entregar({ tenant: tS, chat: chS, motivo }),
                    ponerFoco: (inv) => ponerFoco(tS, telS, inv),
                    catalogo: () => autosDeTenant(tS),
                };
                try { const r = await AG.turno(ctx, deps); return { out: { ok: !!(r.burbujas.length || r.hechos.length), modo: 'agente', tipo: 'agente', agente: r }, enviados }; }
                catch (e) { console.error('[agente]', e.message); await deps.nota('🧯 Seb agente falló (' + e.message.slice(0, 60) + '): lo ves tú'); await deps.escalar('la IA no respondió: ' + bloque.join(' / ').slice(0, 80)); return { out: { ok: false, motivo: 'agente_error', error: e.message }, enviados }; }
            };
            ...
            if (out.ok && Array.isArray(out.segmentos)) {
                ...
                for (let i = 0; i < segs.length; i++) { await mandarS({ texto: LOTE.identidad(segs[i], tS, asisS) }); if (fotos.length && i === fi) await mandarS({ fotos }); if (!out.pin_primero && i === pi) await pin(); }
            ...
            try { if (tS.demo) await DEMO.sistema(tS, telS, nota); else { const ts = Date.now(); await run("INSERT OR IGNORE INTO mensajes (conversacion_id, msg_id, ts, direccion, emisor, texto, tipo, ai_generated, created_at) VALUES (?,?,?,?,?,?,?,?,?)", [Number(chS.id), 'seb-nota:' + ts, ts, 'out', 'sistema', nota, 'text', 1, ts]); } } catch (e) { }
                ...
                if (!out.ok && !out.escalar_owner && !/^(cortesia_silencio|relleno_silencio|en_curso_silencio|dueno|sin_entrantes|gobernador_silencio|turno_repetido|turno_superado|fuera_flujo)$/.test(String(out.motivo || ''))) {
                    ...
                    if (nE.length >= 4 && !/^\W*(gracias|muchas gracias|ok|okey|va|vale|sale|perfecto|listo|bien|excelente|de acuerdo|jaja+|ja|mm+|si|sip|no|nel|hola|holi|que tal|hey|saludos|que onda|buen dia|buenos dias|buenas( tardes| noches)?)\W*(gracias)?\W*$/.test(nE)) { out.escalar_owner = true; out.escala_motivo = 'Seb no tuvo respuesta (' + String(out.motivo || out.error || 'sin motivo').slice(0, 40) + '): "' + String(uiE.texto).slice(0, 70) + '"'; }
                }
            ...
            if (CITAF.activo(tS)) {
                ...
                if (citaF && citaF.manejado && citaF.cita && CITA3.activo(tS) && !citaF.interceptado) {   // RONDA: el cuándo que aplicó el motor (antes de la 1ª confirmación) → 1 voto del comprador; Mario confirma; dueño con 2
                    try {
                        ...
                        if (['nace', 'revive', 'cambia_dia', 'ajusta_hora', 'precisa', 'reagenda', 'ahorita', 'confirma_dia', 'abre_hora', 'quita_dia', 'angosta'].includes(String(citaF.tipo || ''))) await CITA3.propuestaComprador({ tenant: tS, chat: chS, citaId: citaF.cita, nuevo: txP });
            ...
            if (AGENTE) {
                // MARIO AGREGADOR: si Mario ya contestó a mano después de lo último que escribió el comprador, este turno es suyo → Seb no se encima
                const ult2 = (await query("SELECT direccion, ai_generated FROM mensajes WHERE conversacion_id = ? AND COALESCE(emisor,'') <> 'sistema' ORDER BY id DESC LIMIT 1", [Number(chS.id)]))[0];
                if (ult2 && ult2.direccion === 'out' && !Number(ult2.ai_generated)) return res.status(200).json({ ok: true, chat_id: Number(chS.id), seb: { ok: false, motivo: 'mario_contesto' } });
                const rA = await correrAgente(null);
                return res.status(200).json({ ok: true, chat_id: Number(chS.id), seb: Object.assign({ segmentos: (rA.enviados || []).length }, rA.out) });
            }
        ...
        async function asistenteDe(t, chatId) {   // AUTOS FYRADRIVE (3 partes): "Seb, asistente virtual de <vendedor del chat>"; en otros universos: null (identidad de lote)
            if (!CITA3.activo(t) || !chatId) return null;
            try { const r = (await query('SELECT v.nombre FROM conversaciones c JOIN vendedores_universo v ON v.id = c.miembro_id WHERE c.id = ?', [Number(chatId)]))[0]; const n = String((r && r.nombre) || '').trim().split(/\s+/)[0]; return n ? n.charAt(0).toUpperCase() + n.slice(1).toLowerCase() : null; } catch (e) { return null; }
        }
        ...
        async function delegarCore(t, body, X) {
            ...
            let primerNombre = (t.nombre || 'el vendedor').split(/\s+/)[0];
            ...
            const tresD = CITA3.activo(t) && body.miembro_nombre; if (tresD) primerNombre = String(body.miembro_nombre).trim().split(/\s+/)[0];
            const plantilla = tresD ? 'Qué tal{nombre}, soy Seb, asistente virtual de {vendedor}. Estoy aquí para ayudarte con lo básico del {auto}; si ocupas algo más a fondo, dime "hablar con {vendedor}".' : ((t.config && t.config.opener_texto) || 'Hola{nombre}, soy el asistente de {vendedor} para el {auto}.');
        ...
        if (V2.has(action)) {
            ...
            if (action === 'bot_estado' && req.method === 'POST') {
                ...
                if (TV) {   // universos: la voz se cambia por voz.js (una sola puerta, idempotente)
                    ...
                    if (est === 'humano') { const r = await VOZ.entregar({ tenant: tV, chat: c, motivo: 'tomaste el chat', avisar: false, fuente: 'vendedor', forzar: true }); return okJ({ bot: 'humano', chat_id: Number(c.id), cambio: r.cambio }); }
            ...
            if (action === 'cita3_vendedor' && req.method === 'POST') {
                ...
                const c3r = String(ch3.canal || '') === 'dueno' ? (req.body.datos && req.body.datos.cita_id ? await CITA3.porCita(req.body.datos.cita_id) : await CITA3.porDuenoChat(TV, ch3.id)) : (await CITA3.porChat(TV, ch3.id) || (await query('SELECT * FROM cita3 WHERE chat_id = ? ORDER BY cita_id DESC LIMIT 1', [ch3.id]))[0]);
                return okJ(Object.assign({}, r3, { tres: c3r ? await CITA3.estado(c3r.cita_id) : null }));
            }
            ...
            if (action === 'auto_estado' && req.method === 'POST') {
                ...
                else if (est === 'baja') { await run("UPDATE inventario_autos SET estado='inactivo', synced_at=? WHERE id=?", [now, Number(a.id)]); await run('UPDATE autos_universo SET activo=0, updated=? WHERE inv_auto_id=?', [now, Number(a.id)]); if (a.fyradrive_web_id) await run("UPDATE autos SET estado='eliminado', updated_at=datetime('now') WHERE id=?", [Number(a.fyradrive_web_id)]).catch(() => { }); }   // baja = del auto, en TODOS los lotes
                else if (est === 'activo') { await run("UPDATE inventario_autos SET estado='activo', vendido_externo=0, synced_at=? WHERE id=?", [now, Number(a.id)]); await run('UPDATE autos_universo SET activo=1, updated=? WHERE inv_auto_id=?', [now, Number(a.id)]); if (a.fyradrive_web_id) await run("UPDATE autos SET estado='activo', fecha_venta=NULL, updated_at=datetime('now') WHERE id=?", [Number(a.fyradrive_web_id)]).catch(() => { }); }
                else return err(400, 'estado inválido (vendido | baja | activo)');
                try { await U.sincronizarCatalogoFyradrive({ inv_auto_id: Number(a.id) }); } catch (e) { }   // a la par con Autos Fyradrive
            ...
            if (action === 'autos_mios') {
                ...
                // COMISIÓN DEL VENDEDOR (orden owner 2026-09-23): a los vendedores de Autos Fyradrive les toca el 20% de la comisión de Fyradrive
                // ($10,000 → $2,000; proporcional en los demás). Solo la ve el propio vendedor en su FyraChat (sesión de miembro), no el panel del lote.
                const PARTE_VENDEDOR = 0.2; const ES_VENDEDOR_FYRA = ES_LOTE_FYRADRIVE && !!(SES && SES.miembro);
                const comisionVendedorDe = r => { if (!ES_VENDEDOR_FYRA) return null; const c = comisionDe(r); if (!c || c.estado === 'rechazada') return { monto: 0, base: 0, estado: c ? c.estado : 'pendiente' }; return { monto: Math.round((Number(c.monto) || 0) * PARTE_VENDEDOR), base: Number(c.monto) || 0, estado: c.estado }; };
            ...
            const subirAutoCore = async (B, { estadoWeb, consignacion } = {}) => {
                ...
                if (invId) { try { await U.sincronizarCatalogoFyradrive({ inv_auto_id: invId }); } catch (e) { } }   // alta → a la par en Autos Fyradrive
